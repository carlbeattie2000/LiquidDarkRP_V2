...
local hookCanEditLaws = {
    canEditLaws = function(_, ply, action, args)
        if IsValid(ply) and (not RPExtraTeams[ply:Team()] or not RPExtraTeams[ply:Team()].mayor) then return false, DarkRP.getPhrase("incorrect_job", GAMEMODE.Config.chatCommandPrefix .. action) end
        return true
    end
}
...
local function placeLaws(ply, args)
    ...
    ent:Spawn()
    ent:Activate()
    if IsValid(ent) then numlaws = numlaws + 1 end
