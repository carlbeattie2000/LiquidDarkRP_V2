...
local function ShowLetter(msg)
  ...
  hook.Add("HUDPaint", "ShowLetter", function()
    if LetterAlpha < 255 then LetterAlpha = math.Clamp(LetterAlpha + 400 * FrameTime(), 0, 255); end
    ...
    if ((LocalPlayer():KeyDown(IN_USE) or LocalPlayer():GetPos():Distance(LetterPos) > 70) and CurTime() - Opened >= 3) and not LeftLetter then
      LeftLetter = true;
      SignButton:Remove();
    ...
    if LeftLetter then
      ...
      if LetterY >= ScrH() * .99 then KillLetter(); end
    end
  end);
end
