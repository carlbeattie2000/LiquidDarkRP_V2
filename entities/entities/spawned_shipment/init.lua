...
function ENT:StartSpawning()
    ...
    timer.Simple(0, function()
        self.locked = true -- when spawning through pocket it might be unlocked
    end)
...
function ENT:OnTakeDamage(dmg)
    ...
    if not self.locked then
        ...
        if self.damage <= 0 then self:Destruct() end
    end
end
...
local function calculateAmmo(class, shipment)
    ...
    if not clip1 then clip1 = clipSize end
...
function ENT:SpawnItem()
    ...
    if CustomShipments[contents] and CustomShipments[contents].spawn then
        self.USED = false
        return CustomShipments[contents].spawn(self, CustomShipments[contents])
    end
...
function ENT:StartTouch(ent)
    ...
    if not ent.IsSpawnedShipment or self:Getcontents() ~= ent:Getcontents() or self.locked or ent.locked or self.USED or ent.USED or self.hasMerged or ent.hasMerged then return end
    ...
    if self.clip2 or ent.clip2 then self.clip2 = math.floor(((ent.clip2 or 0) * entCount + (self.clip2 or 0) * selfCount) / count) end
    if self.ammoadd or ent.ammoadd then self.ammoadd = math.floor(((ent.ammoadd or 0) * entCount + (self.ammoadd or 0) * selfCount) / count) end
    ent:Remove()
end
