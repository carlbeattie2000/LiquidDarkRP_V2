...
function ENT:DecreaseAmount()
  ...
  if self.dt.amount <= 0 then self:Remove(); end
end
...
function ENT:Use(ply, caller)
  ...
  if cancarry then
    ...
    ply:AddItem(class, 1);
  elseif cancarry == nil then
    --Player attempted to pick up an item not in LDRP's item database, try DarkRP's weapon pickup code
