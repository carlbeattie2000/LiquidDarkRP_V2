...
function ENT:Initialize()
    DarkRP.ValidatedPhysicsInit(self, SOLID_VPHYSICS, string.format("The issue lies with weapon \"%s\"", self:GetWeaponClass() or "unknown"))
...
function ENT:StartTouch(ent)
    ...
    if ent.IsSpawnedWeapon ~= true or self:GetWeaponClass() ~= ent:GetWeaponClass() or self.hasMerged or ent.hasMerged then return end
    ent.hasMerged = true
    ent.USED = true
    ...
    if self.clip1 or ent.clip1 then self.clip1 = math.floor(((self.clip1 or 0) * selfAmount + (ent.clip1 or 0) * entAmount) / totalAmount) end
    if self.clip2 or ent.clip2 then self.clip2 = math.floor(((self.clip2 or 0) * selfAmount + (ent.clip2 or 0) * entAmount) / totalAmount) end
