...
function ENT:Use(ply, caller)
  ...
  if not ply:CanCarry("pot", 1) then
  ...
  if Check then
    crop = Check.crop;
    crops = Check.cropam;
    if not ply:CanCarry(crop, crops) then
    ...
    if Owner and Owner:IsValid() then
      ...
      if ply == Owner then Owner:GiveEXP("Growing", Check.exp, true); end
    end
  end
...
function ENT:StartTouch(entity)
  ...
  if self.DrugType or not b or not Tbl or not ply or not ply:IsValid() then return; end
  entity:SetNotSolid(true);
  if ply.Growing and ply.Growing[b] and ply.Growing[b] >= Tbl.max then
    ply:LiquidChat("GAME", Color(0, 200, 200), "You can only grow " .. Tbl.max .. " " .. Tbl.name .. "s at a time.");
    entity:SetNotSolid(false);
    return false;
  end

  if ply.Character.Skills.Growing.lvl < Tbl.reqlvl then
    ply:LiquidChat("GAME", Color(0, 200, 200), "You must have atleast " .. Tbl.reqlvl .. " growing to grow a " .. Tbl.name);
    entity:SetNotSolid(false);
    return false;
  end

  if Tbl.illegal and table.HasValue(Tbl.illegal, ply:Team()) then
    ply:LiquidChat("GAME", Color(0, 200, 200), "This item is illegal for you!");
    entity:SetNotSolid(false);
    return false;
  end

  if Tbl.reqteams and not table.HasValue(Tbl.reqteams, ply:Team()) then
    ply:LiquidChat("GAME", Color(0, 200, 200), "You can not grow this as your current job!");
    entity:SetNotSolid(false);
    return false;
  end
  ...
  if b == "weed seed" then
    timer.Create("MovePlant_" .. EID, math.Round(Tbl.time * .2) * 60, 5, function()
      if not ActualPlant or not ActualPlant:IsValid() then
  ...
  timer.Create("PotGrowth_" .. EID, math.Round(Tbl.time * .8) * 60, 1, function()
    if self and self:IsValid() then
      ...
      timer.Create("PotGrowth2_" .. EID, math.Round(Tbl.time * .2) * 60, 1, function()
        if self and self:IsValid() then
          self.Complete = true;
          timer.Create("KillPlant_" .. EID, math.Round(Tbl.time * .35) * 60, 1, function() if self and self:IsValid() then self.Dead = true; end end);
        end
      end);
    end
  end);
...
function ENT:OnRemove()
  ...
  if b and o and o:IsValid() and o.Growing then o.Growing[b] = o.Growing[b] - 1; end
end
