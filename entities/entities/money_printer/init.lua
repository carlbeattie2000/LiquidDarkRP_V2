...
local collSounds = {"physics/metal/metal_box_break1.wav", "physics/metal/metal_box_break2.wav", "physics/metal/metal_barrel_impact_hard1.wav", "physics/metal/metal_sheet_impact_hard6.wav"};
...
function ENT:CreateMoneybag()
  ...
  local amount = 0;
  if LDRP_SH.Printers[self:GetNWString("Upgrade")].prnt then amount = LDRP_SH.Printers[self:GetNWString("Upgrade")].prnt; end
...
function ENT:Use(ply, call)
  ...
  if self.dt.owning_ent ~= ply then
    ...
    timer.Create("StealPrinter_" .. EID, 1, 0, function()
      ...
      if not self or not self:IsValid() or not ply:GetEyeTrace().Entity or not ply:GetEyeTrace().Entity:IsValid() or ply:GetEyeTrace().Entity ~= self.Entity then
