...
function ENT:OnRemove()
  ...
  for k, v in pairs(Keypad.Passwords) do
    if Index == v.Ent then table.remove(Keypad.Passwords, k); end
  end
end

function ENT:OnRestore()
  table.insert(Keypad.Passwords, {
    Ent = self.Entity:EntIndex(),
    Pass = self.Entity.Pass
  });
end
...
local function CorrectPassword(Ent, Pass)
  ...
  for k, v in pairs(Keypad.Passwords) do
    if Ent == v.Ent then if Pass == v.Pass then return true; end end
  end
  return false;
end
...
local function RunKeypad(Repeats, Length, Delay, Key, Owner)
  for i = 0, Repeats do
    timer.Simple(Length * i + Delay * i, function() numpad.Activate(Owner, Key, false); end);
    timer.Simple(Length * (i + 1) + Delay * i, function() numpad.Deactivate(Owner, Key, false); end);
  end
end
  ...
  elseif Args[2] == "accept" then
    ...
    if (Key ~= nil) and (Key ~= -1) then
      if Simple then
        ...
        timer.Simple(Length, function() numpad.Deactivate(Owner, Key, false); end);
      else
        if InitDelay ~= 0 then
