...
function SWEP:PrimaryAttack()
  ...
  if not IsValid(trace.Entity) or (self.Owner:EyePos():Distance(trace.Entity:GetPos()) > 115) or (not trace.Entity:IsPlayer() and not trace.Entity:IsNPC()) then return; end
  if not GAMEMODE.Config.npcarrest and trace.Entity:IsNPC() then return; end
  ...
  if not jpc or jpc == 0 then
    ...
    if trace.Entity:IsNPC() then
      ...
      if not trace.Entity.Babygod then
        ...
        if self.Owner.SteamName then DB.Log(self.Owner:Nick() .. " (" .. self.Owner:SteamID() .. ") arrested " .. trace.Entity:Nick(), nil, Color(0, 255, 255)); end
      else
