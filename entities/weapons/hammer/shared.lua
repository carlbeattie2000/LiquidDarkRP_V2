...
function SWEP:PrimaryAttack()
  ...
  if self.Owner:Team() ~= TEAM_CRAFTER then
    if SERVER then self.Owner:LiquidChat("Mining", Color(100, 100, 100), "You need to be the crafter job to use this. (remember to holster the hammer first)"); end
    return;
  end
  ...
  if LP:GetEyeTrace().Entity and LP:GetEyeTrace().Entity:IsValid() and LP:GetEyeTrace().Entity:GetClass() == "crafting_table" then LDRP_SH.OpenCraftingMenu(); end
end
