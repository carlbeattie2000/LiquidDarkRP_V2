...
function SWEP:PrimaryAttack()
  ...
  if not IsValid(trace.Entity) or not trace.Entity:IsPlayer() or trace.Entity:GetPos():Distance(self.Owner:GetPos()) > 100 then return; end
  local result = "";
  for k, v in pairs(trace.Entity:GetWeapons()) do
    if v:IsValid() then result = result .. ", " .. v:GetClass(); end
  end
...
function SWEP:Succeed()
  ...
  for k, v in pairs(trace.Entity:GetWeapons()) do
    if not table.HasValue(NoStripWeapons, string.lower(v:GetClass())) then
      ...
      table.insert(stripped, {v:GetClass(), trace.Entity:GetAmmoCount(v:GetPrimaryAmmoType()), v:GetPrimaryAmmoType(), trace.Entity:GetAmmoCount(v:GetSecondaryAmmoType()), v:GetSecondaryAmmoType(), v:Clip1(), v:Clip2()});
    end
  end
  ...
  if not trace.Entity:GetTable().ConfiscatedWeapons then
    ...
    for k, v in pairs(stripped) do
      ...
      if not found then table.insert(trace.Entity:GetTable().ConfiscatedWeapons, v); end
...
function SWEP:Think()
  if self.IsWeaponChecking then
    local trace = self.Owner:GetEyeTrace();
    if not IsValid(trace.Entity) then self:Fail(); end
    if trace.HitPos:Distance(self.Owner:GetShootPos()) > 100 or not trace.Entity:IsPlayer() then self:Fail(); end
    if self.EndCheck <= CurTime() then self:Succeed(); end
  end
end
