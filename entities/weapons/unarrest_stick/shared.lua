...
function SWEP:PrimaryAttack()
  ...
  if not IsValid(trace.Entity) or not trace.Entity:IsPlayer() or (self.Owner:EyePos():Distance(trace.Entity:GetPos()) > 115) or not trace.Entity.DarkRPVars.Arrested then return; end
  trace.Entity:unArrest();
  GAMEMODE:Notify(trace.Entity, 0, 4, "You were unarrested by " .. self.Owner:Nick());
  if self.Owner.SteamName then DB.Log(self.Owner:Nick() .. " (" .. self.Owner:SteamID() .. ") unarrested " .. trace.Entity:Nick(), nil, Color(0, 255, 255)); end
end
