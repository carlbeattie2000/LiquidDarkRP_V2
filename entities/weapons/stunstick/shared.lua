...
function SWEP:OnRemove()
  if SERVER and IsValid(self:GetOwner()) then SendUserMessage("StunStickColour", self:GetOwner(), 255, 255, 255, ""); end
end
...
function SWEP:Initialize()
  ...
  self.Hit = {Sound("weapons/stunstick/stunstick_impact1.wav"), Sound("weapons/stunstick/stunstick_impact2.wav")};
  self.FleshHit = {Sound("weapons/stunstick/stunstick_fleshhit1.wav"), Sound("weapons/stunstick/stunstick_fleshhit2.wav")};
end
...
function SWEP:PrimaryAttack()
  ...
  if not trace.Entity:IsDoor() then trace.Entity:SetVelocity((trace.Entity:GetPos() - self.Owner:GetPos()) * 7); end
...
function SWEP:Reload()
  ...
  if self.LastReload and self.LastReload > CurTime() - 0.1 then
...
if CLIENT then
  local function StunStickFlash()
    ...
    hook.Add("HUDPaint", "RP_StunstickFlash", function()
      ...
      if math.Round(alpha) == 0 then hook.Remove("HUDPaint", "RP_StunstickFlash"); end
    end);
  end
