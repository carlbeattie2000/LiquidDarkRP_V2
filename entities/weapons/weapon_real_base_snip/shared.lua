...
function SWEP:PrimaryAttack()
  ...
  if (game.SinglePlayer() and SERVER) or CLIENT then self.Weapon:SetNetworkedFloat("LastShootTime", CurTime()); end
end
...
function SWEP:Reload()
  ...
  if (self.Weapon:Clip1() < self.Primary.ClipSize) and self.Owner:GetAmmoCount(self.Primary.Ammo) > 0 then
    ...
    self.MouseSensitivity = 1;
    if not CLIENT then self.Owner:DrawViewModel(true); end
  end
  return true;
end
...
function SWEP:SetScope(b, player)
  ...
  if b then
    ...
    if PlaySound then self.Weapon:EmitSound(sndZoomIn); end
  else
    --		player:DrawViewModel(true)
    if PlaySound then self.Weapon:EmitSound(sndZoomOut); end
  end
