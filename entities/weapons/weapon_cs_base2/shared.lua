...
function SWEP:SetIronsights(b)
    if b ~= self:GetIronsights() then
        ...
        if GAMEMODE.Config.ironshoot then self:SetHoldType(b and self.HoldType or self.LoweredHoldType) end
...
function SWEP:PrimaryAttack()
    ...
    if self:GetFireMode() ~= "burst" then self:SetNextPrimaryFire(CurTime() + self.Primary.Delay) end
...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    if GAMEMODE.Config.ironshoot then ang:RotateAroundAxis(ang:Right(), -15) end
...
function SWEP:Think()
    self:CalcViewModel()
    if self.Primary.ClipSize ~= -1 and not self:GetReloading() and not self:GetIronsights() and self:GetLastPrimaryAttack() + 1 < CurTime() and self:GetHoldType() == self.HoldType and GAMEMODE.Config.ironshoot then self:SetHoldType(self.LoweredHoldType) end
    if self:GetReloadEndTime() ~= 0 and CurTime() >= self:GetReloadEndTime() then
        self:SetReloadEndTime(0)
        self:SetReloading(false)
        if GAMEMODE.Config.ironshoot then self:SetHoldType(self.LoweredHoldType) end
        if CLIENT then self.hasShot = false end
    end
