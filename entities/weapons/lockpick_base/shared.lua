...
function SWEP:PrimaryAttack()
  ...
  if IsValid(e) and trace.HitPos:Distance(self.Owner:GetShootPos()) <= 100 and (e:IsDoor() or e:IsVehicle() or string.find(string.lower(e:GetClass()), "vehicle")) then
    ...
    if self.LookPickTimeReduce then Lockpicktime = Lockpicktime - self.LookPickTimeReduce; end
...
function SWEP:Succeed()
  ...
  if IsValid(e) and e.Fire then
    ...
    if e.DoorData.Owner and e.DoorData.Owner:IsPlayer() and not e:OwnedBy(self.Owner) then self.Owner:GiveEXP("Locksmith", 100, true); end
  end
...
function SWEP:Think()
  if SERVER and self.IsLockPicking then
    ...
    if trace.HitPos:Distance(self.Owner:GetShootPos()) > 100 or (not trace.Entity:IsDoor() and not trace.Entity:IsVehicle() and not string.find(string.lower(trace.Entity:GetClass()), "vehicle")) then self:Fail(); end
    if self.EndPick <= CurTime() then self:Succeed(); end
  end
end
