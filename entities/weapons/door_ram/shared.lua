...
function SWEP:PrimaryAttack()
  ...
  if not IsValid(trace.Entity) or (not trace.Entity:IsDoor() and not trace.Entity:IsVehicle() and trace.Entity:GetClass() ~= "prop_physics") then return; end
  if trace.Entity:IsDoor() and self.Owner:EyePos():Distance(trace.HitPos) > 45 then return; end
  if trace.Entity:IsVehicle() and self.Owner:EyePos():Distance(trace.HitPos) > 100 then return; end
  local a = GAMEMODE.Config.copscanunfreeze;
  local d = GAMEMODE.Config.copscanunweld;
  local b = trace.Entity:GetClass() == "prop_physics";
  local c = true;
  local Owner = FPP and trace.Entity:CPPIGetOwner(); --Make sure FPP is activated before calling an FPP function.
  if Owner then c = Owner.warranted or (Owner.DarkRPVars and Owner.DarkRPVars.wanted); end
  ...
  elseif trace.Entity:IsVehicle() then
    ...
    if driver and driver.ExitVehicle then driver:ExitVehicle(); end
...
function SWEP:GetViewModelPosition(pos, ang)
  ...
  if self.Ready then Mul = 1 - Mul; end
