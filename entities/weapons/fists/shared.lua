...
function SWEP:PrimaryAttack()
  ...
  if CLIENT then self.Weapon:EmitSound("npc/zombie/claw_miss" .. math.random(1, 2) .. ".wav"); end
  ...
  if EyeTrace.MatType == MAT_GLASS then
    if CLIENT then self.Weapon:EmitSound("physics/glass/glass_cup_break" .. math.random(1, 2) .. ".wav"); end
    return false;
  end

  if EyeTrace.HitWorld then
    if CLIENT then self.Weapon:EmitSound(final); end
    return false;
  end
  ...
  if EyeTrace.MatType == MAT_FLESH then
    ...
    if SERVER then if EyeTrace.Entity and EyeTrace.Entity:IsValid() and EyeTrace.Entity:IsPlayer() then EyeTrace.Entity:TakeDamage(ply.Character.Skills["Stamina"].lvl * 3, ply, self.Weapon); end end
    return false;
  else
    -- something else?
    if CLIENT then self.Weapon:EmitSound(final); end
