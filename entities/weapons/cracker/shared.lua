...
function SWEP:PrimaryAttack()
  ...
  if IsValid(e) and trace.HitPos:Distance(self.Owner:GetShootPos()) <= 300 and (e:GetClass() == "sent_keypad") then
    ...
    timer.Create("KeyCrackSounds", 1, CrackTime, function() self:EmitSound("buttons/blip2.wav", 50, 50); end);
...
function SWEP:Succeed()
  ...
  if IsValid(trace.Entity) and trace.Entity:GetClass() == "sent_keypad" then
    ...
    if SERVER then trace.Entity:EmitSound("buttons/button11.wav"); end
...
function SWEP:Think()
  if SERVER and self.IsCracking then
    ...
    if trace.HitPos:Distance(self.Owner:GetShootPos()) > 300 or (trace.Entity:GetClass() ~= "sent_keypad" and not trace.Entity.isFadingDoor) then self:Fail(); end
    if self.EndCrack <= CurTime() then self:Succeed(); end
  end
end
