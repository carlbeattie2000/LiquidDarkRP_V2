...
function SWEP:PrimaryAttack()
  ...
  if ply:Team() ~= TEAM_MINER then
    ply:LiquidChat("Mining", Color(100, 100, 100), "You need to be the miner job to use this. (remember to holster the pick first)");
    return;
  end
  ...
  if Tbl.lvl > Lvl then
    ply:LiquidChat("Mining", Color(100, 100, 100), "You need a higher level mining to mine this.");
    return;
  elseif not ply:CanCarry(Tbl.picked, Tbl.pickedam * self.OreMultipler) then
    ply:LiquidChat("Mining", Color(100, 100, 100), "You need to free up inventory space before mining this.");
    return;
  elseif Tbl.required then
    ply:LiquidChat("Mining", Color(100, 100, 100), "This rock requires a better pickaxe.");
  end
  ...
  timer.Create("Mining_" .. plyid, 1, Picktime, function()
    if not ent:IsValid() or not ply:IsValid() or ply:GetActiveWeapon() ~= self.Weapon or ply:GetEyeTrace().Entity ~= ent or ply:GetEyeTrace().HitPos:Distance(ply:GetPos()) > 100 then
    ...
    if SERVER and Picked == Picktime then
      ply.Picking = false;
      if (self.ChanceMultiplier == 0) or math.random(1, Tbl.minechance * self.ChanceMultiplier) == 1 then
