...
function SWEP:Think()
  ...
  if not self.viewModels or not IsValid(self.viewModels[1]) then self:createViewModels(); end
...
function SWEP:Deploy()
  if SERVER and IsValid(self.Owner) then self.Owner:DrawViewModel(false); end
end
...
function SWEP:OnRemove()
  if SERVER and IsValid(self:GetOwner()) then SendUserMessage("med_kit_model", self:GetOwner(), self); end
end
