...
function GM:FindEmptyPos(pos, ignore, distance, step, area)
  if GAMEMODE:IsEmpty(pos, ignore) and GAMEMODE:IsEmpty(pos + area, ignore) then return pos; end
  for j = step, distance, step do
    for i = -1, 1, 2 do -- alternate in direction
      local k = j * i;
      -- Look North/South
      if GAMEMODE:IsEmpty(pos + Vector(k, 0, 0), ignore) and GAMEMODE:IsEmpty(pos + Vector(k, 0, 0) + area, ignore) then return pos + Vector(k, 0, 0); end
      -- Look East/West
      if GAMEMODE:IsEmpty(pos + Vector(0, k, 0), ignore) and GAMEMODE:IsEmpty(pos + Vector(0, k, 0) + area, ignore) then return pos + Vector(0, k, 0); end
      -- Look Up/Down
      if GAMEMODE:IsEmpty(pos + Vector(0, 0, k), ignore) and GAMEMODE:IsEmpty(pos + Vector(0, 0, k) + area, ignore) then return pos + Vector(0, 0, k); end
    end
  end
  return pos;
end
