  ...
  if data then
    for k, v in pairs(data) do
      FPP.Buddies[v.steamid] = {
        name = v.name, --Put all the buddies in the table
        physgun = v.physgun,
        gravgun = v.gravgun,
        toolgun = v.toolgun,
...
function FPP.SaveBuddy(SteamID, Name, Type, value)
  if Type == "remove" then
    ...
    for k, v in pairs(player.GetAll()) do
      if v:SteamID() == SteamID then RunConsoleCommand("FPP_SetBuddy", v:UserID(), "0", "0", "0", "0", "0"); end
    end
    return;
  end

  FPP.Buddies[SteamID] = FPP.Buddies[SteamID] or {
    name = Name, -- Create if not there
    physgun = 0,
    gravgun = 0,
    toolgun = 0,
    playeruse = 0,
