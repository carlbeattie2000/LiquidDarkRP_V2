...
local function CanTouch(um)
  ...
  TouchAlpha = 255;
  timer.Simple(1, function() TouchAlpha = 0; end);
end
...
hook.Add("CanTool", "FPP_CL_CanTool", function(ply, trace, tool)
  ...
  if IsValid(trace.Entity) and trace.Entity.FPPCanTouch == false then return false; end
  if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon().GetToolObject and ply:GetActiveWeapon():GetToolObject() and (string.find(ply:GetActiveWeapon():GetToolObject():GetClientInfo("model") or "", "*") or string.find(ply:GetActiveWeapon():GetToolObject():GetClientInfo("material") or "", "*") or string.find(ply:GetActiveWeapon():GetToolObject():GetClientInfo("model") or "", "\\")) then --[[or string.find(ply:GetActiveWeapon():GetToolObject():GetClientInfo( "tool" ), "/")]]
    return false;
  end
end);

hook.Add("PhysgunPickup", "FPP_CL_PhysgunPickup", function(ply, ent) return false; end);
...
local function DrawNotice(self, k, v, i)
  ...
  if timeleft < 0.8 then ideal_x = ScrW() / 2 + w * 0.5 + 200; end
  -- Gone!
  if timeleft < 0.5 then ideal_y = ScrH() + 50; end
...
local function HUDPaint()
  ...
  if IsValid(LAEnt) and LAEnt.FPPCanTouch ~= nil and LAEnt.FPPOwner then
    ...
    if comingAroundAgain < w then comingAroundAgain = math.Min(comingAroundAgain + (RealFrameTime() * 600), w); end
