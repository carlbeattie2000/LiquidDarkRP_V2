...
function FPP.AdminMenu(Panel)
  ...
  local function addchk(label, command, plist)
    ...
    box.Button.Toggle = function()
      if not superadmin then --Hehe now you can't click it anymore non-admin!
        return;
      end
  ...
  local function addblock(pan, Type)
    ...
    AddManual.DoClick = function() Derma_StringRequest("Enter entity manually", "Enter the classname of the entity you would like to add.", nil, function(a) RunConsoleCommand("FPP_AddBlocked", Type, a); end, function() end); end;
  end
    ...
    for a, b in pairs(spawnmenu.GetTools()) do
      for c, d in pairs(spawnmenu.GetTools()[a].Items) do
        ...
        for e, f in pairs(spawnmenu.GetTools()[a].Items[c]) do
          if type(f) == "table" and string.find(f.Command, "gmod_tool") then table.insert(addnodes, {f.Text, f.ItemName}); end
        end
  ...
  SingleEditTool.DoClick = function()
    ...
    timer.Simple(1, function() if ValidPanel(SingleEditTool) then SingleEditTool:SetText("Edit/view selected tool's restrictions"); end end);
  end;
  ...
  if #FPP.multirestricttoollist.Columns ~= 1 then FPP.multirestricttoollist:AddColumn("Tool names"); end
  ...
  StartEditMultiTool.DoClick = function()
    ...
    timer.Simple(1, function() if ValidPanel(StartEditMultiTool) then StartEditMultiTool:SetText("Edit multiple tools"); end end);
  end;
  ...
  if GroupList:GetSelectedLine() and FPP.Groups[GroupList:GetSelectedLine().Columns[1]:GetValue()] then ChkAllowDefault:SetValue(FPP.Groups[GroupList:GetLine(GroupList:GetSelectedLine()).Columns[1]:GetValue()].allowdefault); end
  ...
  AddGroupBtn.DoClick = function() Derma_StringRequest("Name of the group", "What will be the name of the group?\nNOTE: YOU WILL NOT BE ABLE TO CHANGE THIS AFTERWARDS", "", function(text) RunConsoleCommand("FPP_AddGroup", text); end); end;
  ...
  AddPersonManual.DoClick = function()
    ...
    local other = menu:AddOption("other...", function() Derma_StringRequest("Enter steam ID", "Enter the Steam ID of the person you would like to add to this group.", "", function(text) RunConsoleCommand("FPP_SetPlayerGroup", text, GroupList:GetLine(GroupList:GetSelectedLine()).Columns[1]:GetValue()); end); end);
    menu:Open();
  end;
...
RetrieveRestrictedTool = function(um)
  ...
  if admin == 0 then adminsCHKboxes[1].Button:SetValue(1); end
  ...
  if admin == 1 then adminsCHKboxes[2].Button:SetValue(1); end
  ...
  if admin == 2 then adminsCHKboxes[3].Button:SetValue(1); end
    ...
    for k, v in pairs(player.GetAll()) do
      ...
      submenu:AddOption("Default", function()
        if type(tool) ~= "table" then
          ...
          for a, b in pairs(tool) do
            timer.Simple(a / 10, function() RunConsoleCommand("FPP_restricttoolplayer", b, v:UserID(), 2); end);
          end
        end
      end);
      ...
      submenu:AddOption("Allow", function()
        if type(tool) ~= "table" then
          ...
          for a, b in pairs(tool) do
            timer.Simple(a / 10, function() RunConsoleCommand("FPP_restricttoolplayer", b, v:UserID(), 1); end);
          end
        end
      end);
      ...
      submenu:AddOption("Disallow", function()
        if type(tool) ~= "table" then
          ...
          for a, b in pairs(tool) do
            timer.Simple(a / 10, function() RunConsoleCommand("FPP_restricttoolplayer", b, v:UserID(), 0); end);
          end
        end
      end);
    end
  ...
  for k, v in pairs(team.GetAllTeams()) do
    ...
    chkbx:SetDark(true);
    chkbx.Team = k;
    if table.HasValue(Teams, tostring(k)) then chkbx.Button:SetValue(true); end
    chkbx.Button.Toggle = function()
      ...
      if type(tool) ~= "table" then
        ...
        for a, b in pairs(tool) do
          timer.Simple(a / 10, function() RunConsoleCommand("FPP_restricttool", b, "team", chkbx.Team, tonum[chkbx.Button:GetChecked()]); end);
        end
      end
    end;
  ...
  for a, b in pairs(spawnmenu.GetTools()) do
    for c, d in pairs(spawnmenu.GetTools()[a].Items) do
      ...
      for g, h in pairs(weapons.Get("gmod_tool").Tool) do
        if h.Category and h.Category == d.ItemName then table.insert(addnodes, {h.Name, g}); end
      end
...
function FPP.BuddiesMenu(Panel)
  ...
  AddManual.DoClick = function() Derma_StringRequest("Add buddy manually", "Please enter the SteamID of the player you want to add in your buddies list", "", function(ID) Derma_StringRequest("Name of buddy", "What is the name of this buddy? (You can enter any name, it will change the next time you meet in a server with FPP)", "", function(Name) FPP.SetBuddyMenu(ID, Name); end); end); end;
  ...
  for k, v in SortedPairs(player.GetAll(), function(a, b) return a:Nick() > b:Nick(); end) do
    ...
    if not cantadd then
      ...
      add:SetText(v:Nick());
      add.DoClick = function() FPP.SetBuddyMenu(v:SteamID(), v:Nick()); end;
...
function FPP.PrivateSettings(Panel)
  ...
  for k, v in pairs(PrivateSettings) do
    Panel:AddControl("CheckBox", {
      Label = "I don't want to " .. k,
      Command = "FPP_PrivateSettings_" .. v
    });
  end
...
local function UpdateMenus()
  if AdminPanel then FPP.AdminMenu(AdminPanel); end
  if BuddiesPanel then FPP.BuddiesMenu(BuddiesPanel); end
end
...
function FPP.SharedMenu(um)
  ...
  local function AddChk(name, Type, value)
    ...
    if count * 20 - (row - 1) * ScrH() > ScrH() - 30 - (row - 1) * 50 then row = row + 1; end
  ...
  if #player.GetAll() ~= 1 then count = count + 1; end
  for k, v in pairs(player.GetAll()) do
    if v ~= LocalPlayer() and v:IsValid() then
      local IsShared = false;
      if table.HasValue(SharedWith, v) then IsShared = true; end
      AddChk(v, v:UserID(), IsShared);
    end
  end

  local height = count * 20;
  if row > 1 then height = ScrH() - 20; end
