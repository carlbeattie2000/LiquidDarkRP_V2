...
if cleanup then
  ...
  function cleanup.Add(ply, Type, ent)
    ...
    if FPP.AntiSpam and Type ~= "constraints" and Type ~= "stacks" and Type ~= "AdvDupe2" and (not ent.IsVehicle or not ent:IsVehicle()) then FPP.AntiSpam.CreateEntity(ply, ent, Type == "duplicates"); end
    if ent:GetClass() == "gmod_wire_expression2" then ent:SetCollisionGroup(COLLISION_GROUP_WEAPON); end
...
local function cantouchsingleEnt(ply, ent, Type1, Type2, TryingToShare)
  ...
  if IsValid(ent) and ent:IsPlayer() and not getPlySetting(ply, "FPP_PrivateSettings_Players") and Type1 == "Physgun1" then return false; end
  ...
  if not TryingToShare and ent.AllowedPlayers and table.HasValue(ent.AllowedPlayers, ply) then return not OnlyMine, Owner; end
  -- Whitelist checks
  if tobool(FPP.Settings[Type2].iswhitelist) then
    --If it's a whitelist and the entity is found in the whitelist
    if FPP.Blocked[Type1][string.lower(ent:GetClass())] then Returnal = true; end
    -- If the whitelist says you can't touch it, then you can't
    if not Returnal and (not tobool(FPP.Settings[Type2].canblocked) and (not ply:IsAdmin() or not tobool(FPP.Settings[Type2].admincanblocked))) then return false, "Blocked!"; end
  ...
  if Owner ~= ply then
    ...
    elseif IsValid(Owner) and ply:IsAdmin() and tobool(FPP.Settings[Type2].adminall) then
      -- if not world prop AND admin allowed
      return not OnlyMine, Owner;
      -- Misc entities
    ...
    elseif not IsValid(Owner) then
      ...
      elseif tobool(FPP.Settings[Type2].worldprops) then
        -- if worldprop allowed
        return not getPlySetting(ply, "FPP_PrivateSettings_" .. Restrict), world;
      end
      -- if not allowed then
      return false, world;
...
function FPP.Protect.PhysgunPickup(ply, ent)
  ...
  if why then FPP.CanTouch(ply, "FPP_PHYSGUN1", why, cantouch); end
...
function FPP.Protect.GravGunPickup(ply, ent)
  ...
  if not IsValid(ent) then -- You don't want a cross when looking at the floor while holding right mouse
    return false;
  end
  ...
  if why then FPP.CanTouch(ply, "FPP_GRAVGUN1", why, cantouch); end
...
function FPP.Protect.GravGunPunt(ply, ent)
  ...
  if not IsValid(ent) then
    DropEntityIfHeld(ent);
    return FPP.CanTouch(ply, "FPP_GRAVGUN1", "Not valid!", false);
  end
...
function FPP.Protect.PlayerUse(ply, ent)
  ...
  if why then FPP.CanTouch(ply, "FPP_PLAYERUSE1", why, cantouch); end
  if FPP.UnGhost and cantouch then FPP.UnGhost(ply, ent); end
  return cantouch;
end
...
function FPP.Protect.EntityDamage(ent, dmginfo)
  ...
  if not attacker:IsPlayer() and not ent:IsPlayer() then
    ...
    if IsValid(attackerOwner) and IsValid(entOwner) then
      ...
      if why then FPP.CanTouch(attackerOwner, "FPP_ENTITYDAMAGE1", why, cantouch); end
      if not cantouch then
        dmginfo:SetDamage(0);
        ...
        timer.Simple(1, function()
          ...
          ent.FPPAntiDamageWorld = ent.FPPAntiDamageWorld - 1;
          if ent.FPPAntiDamageWorld == 0 then ent.FPPAntiDamageWorld = nil; end
        end);
      end
      return;
    end

    if attacker == game.GetWorld() and ent.FPPAntiDamageWorld then dmginfo:SetDamage(0); end
    return;
  end
  ...
  if why then --[[and (not IsValid(attacker:GetActiveWeapon()) or (IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetClass() == "weapon_physcannon")) ]]
...
function FPP.Protect.CanTool(ply, trace, tool, ENT)
  ...
  if not ignoreGeneralRestrictTool then
    ...
    local CanGroup = true;
    if Group and ((Group.allowdefault and table.HasValue(Group.tools, tool)) or (not Group.allowdefault and not table.HasValue(Group.tools, tool))) then -- If the tool is on the BLACKLIST or -- If the tool is NOT on the WHITELIST
      CanGroup = false;
    end
  ...
  if tobool(FPP.Settings.FPP_TOOLGUN1.toggle) and IsValid(ent) then
    ...
    if why then FPP.CanTouch(ply, "FPP_TOOLGUN1", why, cantouch); end
    if not cantouch then return false; end
  end
  ...
  for k, v in pairs(EntTable) do
    ...
    if tobool(FPP.Settings.FPP_TOOLGUN1.duplicatorprotect) and (not ply:IsAdmin() or (ply:IsAdmin() and not tobool(FPP.Settings.FPP_TOOLGUN1.spawnadmincanblocked))) then
      ...
      if tobool(FPP.Settings.FPP_TOOLGUN1.spawniswhitelist) and FPP.Blocked.Spawning1[lowerClass] then setspawning = false; end
      if setspawning then
        FPP.CanTouch(ply, "FPP_TOOLGUN1", "Duplicating blocked entity", false);
        EntTable[k] = nil;
      end
...
function FPP.PlayerDisconnect(ply)
  ...
  timer.Simple(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnectedtime, function()
    if not tobool(FPP.Settings.FPP_GLOBALSETTINGS1.cleanupdisconnected) then -- Settings can change in time.
      return;
    end
    ...
    for k, v in pairs(ents.GetAll()) do
      if IsValid(v) and v.FPPOwnerID == SteamID then v:Remove(); end
    end
...
function FPP.PlayerInitialSpawn(ply)
  ...
  if FPP.DisconnectedPlayers[ply:SteamID()] then -- Check if the player has rejoined within the auto remove time
    for k, v in pairs(ents.GetAll()) do
      if IsValid(v) and v.FPPOwnerID == ply:SteamID() then v:CPPISetOwner(ply); end
    end
  end
end
...
function ENTITY:FireBullets(bullet, ...)
  ...
  if table.HasValue(blockedEffects, string.lower(bullet.TracerName)) then bullet.TracerName = ""; end
  return backup(self, bullet, ...);
end

hook.Add("EntityRemoved", "jeepWorkaround", function(ent) if IsValid(ent) and ent:IsVehicle() and ent.GetPassenger and IsValid(ent:GetPassenger(1)) then ent:GetPassenger(1):ExitVehicle(); end end);
