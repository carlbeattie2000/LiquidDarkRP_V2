...
function FPP.UnGhost(ply, ent)
  if ent.FPPAntiSpamIsGhosted then
    ...
    if phys:IsValid() then phys:EnableMotion(ent.FPPAntiSpamMotionEnabled); end
  end
end
...
function FPP.AntiSpam.CreateEntity(ply, ent, IsDuplicate)
  ...
  if phys:GetVolume() and phys:GetVolume() > math.pow(10, FPP.Settings.FPP_ANTISPAM1.bigpropsize) and not string.find(class, "constraint") and not string.find(class, "hinge") and not string.find(class, "magnet") and not string.find(class, "collision") and not blacklist[class] then
    ...
    if not IsDuplicate then ply.FPPAntiSpamLastBigProp = CurTime(); end
  ...
  if not IsDuplicate and not blacklist[class] then
    ...
    timer.Simple(ply.FPPAntiSpamCount / time, function() if IsValid(ply) then ply.FPPAntiSpamCount = ply.FPPAntiSpamCount - 1; end end);
    if ply.FPPAntiSpamCount >= FPP.Settings.FPP_ANTISPAM1.smallpropghostlimit and ply.FPPAntiSpamCount <= FPP.Settings.FPP_ANTISPAM1.smallpropdenylimit and not ent:IsVehicle() then --[[Vehicles don't like being ghosted, they tend to crash the server]]
      FPP.AntiSpam.GhostFreeze(ent, phys);
