...
local function FPP_SetSetting(ply, cmd, args)
  if ply:EntIndex() ~= 0 and not ply:IsSuperAdmin() then
    FPP.Notify(ply, "You need superadmin privileges in order to be able to use this command", false);
    return;
  end
  ...
  MySQLite.query("SELECT * FROM FPP_BLOCKED1;", function(data)
    if type(data) == "table" then
      ...
      for k, v in pairs(data) do
        if tonumber(v.id) > highest then highest = tonumber(v.id); end
        if v.var == args[1] and v.setting == args[2] then found = true; end
      end

      if not found then MySQLite.query("INSERT INTO FPP_BLOCKED1 VALUES(" .. highest + 1 .. ", " .. sql.SQLStr(args[1]) .. ", " .. sql.SQLStr(args[2]) .. ");"); end
    else
      --insert
...
local function AddBlockedModel(ply, cmd, args)
  ...
  if FPP.BlockedModels[model] then
    FPP.Notify(ply, "This model is already in the black/whitelist", false);
    return;
  end
...
local function RetrieveBlockedModels()
  ...
  MySQLite.query("SELECT * FROM FPP_BLOCKEDMODELS1;", function(data)
    if not data or #data == 0 then FPP.generateDefaultBlocked(); end
    for k, v in pairs(data or {}) do
...
local function AddGroup(ply, cmd, args)
  ...
  if not args[1] then -- Args: 1 = name, optional: 2 = allowdefault
...
local function RemoveGroup(ply, cmd, args)
  ...
  if not args[1] then -- Args: 1 = name
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
...
local function GroupChangeAllowDefault(ply, cmd, args)
  ...
  if not args[2] then -- Args: 1 = groupname, 2 = new value 1/0
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
...
local function GroupAddTool(ply, cmd, args)
  ...
  if not args[2] then -- Args: 1 = groupname, 2 = tool
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
...
local function GroupRemoveTool(ply, cmd, args)
  ...
  for k, v in pairs(FPP.Groups[name].tools) do
    if v == tool then table.remove(FPP.Groups[name].tools, k); end
  end
...
local function PlayerSetGroup(ply, cmd, args)
  ...
  if not args[2] then -- Args: 1 = player, 2 = group
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
...
local function SendRestrictedTools(ply, cmd, args)
  ...
  if FPP.RestrictedTools[args[1]] and FPP.RestrictedTools[args[1]].team then teamrestrict = table.concat(FPP.RestrictedTools[args[1]]["team"], ";"); end
...
local function SetBuddy(ply, cmd, args)
  ...
  ply.Buddies[buddy] = {
    physgun1 = util.tobool(args[2]),
    gravgun1 = util.tobool(args[3]),
    toolgun1 = util.tobool(args[4]),
    playeruse1 = util.tobool(args[5]),
    entitydamage1 = util.tobool(args[6])
  };
end
...
local function CleanupDisconnected(ply, cmd, args)
  if ply:EntIndex() ~= 0 and not ply:IsAdmin() then
    FPP.Notify(ply, "You can't clean up", false);
    return;
  end
  ...
  if args[1] == "disconnected" then
    for k, v in pairs(ents.GetAll()) do
      ...
      if Owner and not IsValid(Owner) then v:Remove(); end
    end
  ...
  for k, v in pairs(ents.GetAll()) do
    local Owner = v:CPPIGetOwner();
    if Owner == Player(args[1]) and not v:IsWeapon() then v:Remove(); end
  end
...
local function SetToolRestrict(ply, cmd, args)
  if ply:EntIndex() ~= 0 and not ply:IsSuperAdmin() then
    FPP.Notify(ply, "You can't set tool restrictions", false);
    return;
  end

  if not args[3] then --FPP_restricttool <toolname> <type(admin/team)> <toggle(1/0)>
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
...
local function RestrictToolPerson(ply, cmd, args)
  ...
  if not args[3] then --FPP_restricttoolperson <toolname> <userid> <disallow, allow, remove(0,1,2)>
    FPP.Notify(ply, "Invalid argument(s)", false);
    return;
  end
