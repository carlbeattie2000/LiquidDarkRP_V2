...
function createBtn(target, x, y, w, h, text, onclick, paint)
  ...
  if paint then paint(button); end
end
...
net.Receive("OpenF4Menu", function()
  ...
  if InitContentRequested and InitContentRequested ~= "" then
    ...
    selectedTab = foundResult["loadtab"](mainWindowPanel) or MENU_CONFIG.tabButtons[1]["loadtab"](mainWindowPanel);
    selectedTabIndex = -1;
  else
...
local function KeysMenu(um)
  ...
  function Frame:Think()
    ...
    if not IsValid(ent) or (not ent:IsDoor() and not string.find(ent:GetClass(), "vehicle")) or ent:GetPos():Distance(LocalPlayer():GetPos()) > 200 then self:Close(); end
  ...
  local Entiteh = "door";
  if Vehicle then Entiteh = "vehicle"; end
  ...
  if trace.Entity:OwnedBy(LocalPlayer()) then
    if not trace.Entity.DoorData then -- Don't open the menu when the door settings are not loaded yet
      return;
    end
    ...
    AddOwner.DoClick = function()
      ...
      for k, v in pairs(player.GetAll()) do
        if not trace.Entity:OwnedBy(v) and not trace.Entity:AllowedToOwn(v) then menu:AddOption(v:Nick(), function() LocalPlayer():ConCommand("say /ao " .. v:UserID()); end); end
      end

      if menu.Panels and #menu.Panels == 0 then menu:AddOption("Noone available", function() end); end
      menu:Open();
    end;
    ...
    RemoveOwner.DoClick = function()
      local menu = DermaMenu();
      for k, v in pairs(player.GetAll()) do
        if (trace.Entity:OwnedBy(v) and not trace.Entity:IsMasterOwner(v)) or trace.Entity:AllowedToOwn(v) then menu:AddOption(v:Nick(), function() LocalPlayer():ConCommand("say /ro " .. v:UserID()); end); end
      end
    ...
    if not trace.Entity:IsMasterOwner(LocalPlayer()) then RemoveOwner.m_bDisabled = true; end
    ...
    if LocalPlayer():IsSuperAdmin() and not Vehicle then
      ...
      SetCopsOnly.DoClick = function()
        local menu = DermaMenu();
        menu:AddOption("No group", function()
          RunConsoleCommand("say", "/togglegroupownable");
          Frame:Close();
        end);

        for k, v in pairs(RPExtraTeamDoors) do
          menu:AddOption(k, function()
            RunConsoleCommand("say", "/togglegroupownable " .. k);
            Frame:Close();
          end);
        end
  ...
  elseif not trace.Entity:OwnedBy(LocalPlayer()) and trace.Entity:IsOwnable() and not trace.Entity:IsOwned() and not trace.Entity.DoorData.NonOwnable then
    ...
    if LocalPlayer():IsSuperAdmin() then
      ...
      SetCopsOnly.DoClick = function()
        ...
        menu:AddOption("No group", function()
          RunConsoleCommand("say", "/togglegroupownable");
          if ValidPanel(Frame) then Frame:Close(); end
        end);
