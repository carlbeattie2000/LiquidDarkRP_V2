...
if not LDRP_Theme.CurrentSkin then LDRP_Theme.CurrentSkin = "Dark Skin"; end
...
GWEN.CreateTextureBorder = function(_x, _y, _w, _h, l, t, r, b, colMod)
  local modifyFunc = function(x, y, w, h, col)
    ...
    local newCol = Color(col.r - (col.r - skinCol.r), col.g - (col.g - skinCol.g), col.b - (col.b - skinCol.b), col.a - (col.a - skinCol.a));
...
function RefreshSkin()
  ...
  function SKIN:PaintPanelList(panel)
    if panel.m_bBackground then draw.RoundedBox(4, 0, 0, panel:GetWide(), panel:GetTall(), self.bg_color_dark); end
  end
  ...
  function SKIN:PaintButtonDown(panel, w, h)
    if not panel.m_bBackground then return; end
    if panel.Depressed or panel:IsSelected() then self.tex.Scroller.DownButton_Down(0, 0, w, h); end
    if panel:GetDisabled() then self.tex.Scroller.DownButton_Dead(0, 0, w, h); end
    if panel.Hovered then self.tex.Scroller.DownButton_Hover(0, 0, w, h); end
  ...
  function SKIN:SchemeTab(panel)
    ...
    local ExtraInset = 10;
    if panel.Image then ExtraInset = ExtraInset + panel.Image:GetWide(); end
  ...
  function SKIN:DrawNumberWangIndicatorText(panel, wang, x, y, number, alpha)
    ...
    if number / dec == math.ceil(number / dec) then col = self.text_highlight; end
  ...
  function SKIN:LayoutNumberWangIndicator(panel)
    ...
    if panel.m_bTop then y = y - panel.Height - wang:GetTall(); end
    panel:SetPos(x, y);
