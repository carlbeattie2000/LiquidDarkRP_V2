...
function LDRP.GetPaycheck(ply, cmd, args)
  if not LDRP_SH.UsePaycheckLady then
    ply:ChatPrint("Paycheck lady is not used but in the map.");
    return;
  end
...
function LDRP.BankCMD(ply, cmd, args)
  ...
  local Type = args[1];
  local Item = args[2];
  if not Type or not Item or (Type ~= "money" and not LDRP_SH.AllItems[Item]) or Item == "curcash" then return; end
  if Type == "bank" then
  ...
  elseif Type == "money" then
    ...
    if not Cash then
      ply:LiquidChat("BANK", Color(0, 192, 10), "Specify a number to deposit or withdraw.");
      return;
    end
...
function LDRP.BankUpgrade(ply, cmd, args)
  ...
  if upgradeType == "balanceChanged" then
    ...
    if playerBalance < LDRP.AccountUpgradeOptionValues[rateType]["min"] then
      ...
      if not newSelectedType or not newInterestRate then
  ...
  if newLevel == "Upgrade Account" then
