...
function LDRP.PrinterUpgrade()
  ...
  if ent and ent:IsValid() and ent:GetClass() == "money_printer" and ent.dt.owning_ent and ent.dt.owning_ent == LocalPlayer() and LocalPlayer():GetPos():Distance(ent:GetPos()) <= 300 then
    ...
    if not Upgrade or Upgrade == "" then return; end
...
function LDRP_SH.OpenNPCWindow(name, mdl, says, sayings)
  ...
  SayingList.Paint = function() draw.RoundedBox(6, 0, 0, w * .98, h * .42, LDRPColorMod(27, 27, 27, 30)); end;
...
function LDRP.MoreInfoToggle()
  ...
  if not timer.Exists("ReSizeInfoWindow") then
...
function LDRP.HUDPaint()
  ...
  if ShowHP == 1 then
    local Pos1 = 162;
    if ShowPC ~= 1 then Pos1 = 4.32; end
    if NiceHP > LH then
      NiceHP = NiceHP - 1;
  ...
  if GetConVarNumber("cl_showammo") == 1 then
    ...
    if ShowHP ~= 1 then Pos1, Pos2, Pos4 = 2.7, 4.95, 13.95; end
    if ShowPC ~= 1 then Pos3 = 4.32; end
...
hook.Add("HUDShouldDraw", "Hides the old HUD", function(c) return not table.HasValue(LDRP.BlockHUD, c); end);
...
function LDRP.CreateMeterHUD(text, time)
  ...
  local function MeterHUDLocal()
    ...
    draw.SimpleTextOutlined(text .. math.Round(time - (CurTime() - MeterStart), 0), "HUDNumber", ScrW() * .5, (LDRP.MeterHeight * .5) + 10, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0));
  end

  hook.Add("HUDPaint", "Shows a progress meter", MeterHUDLocal);
  timer.Create("MeterHUDRemove", time, 1, function() hook.Remove("HUDPaint", "Shows a progress meter"); end);
end
...
function LDRP.InputLockReason()
  ...
  LD.Enter.Paint = function() draw.RoundedBox(8, 0, 0, 492, 20, Color(70, 0, 0, 210)); end;
...
function LDRP.SendLockdown(um)
  ...
  if not Reason or Reason == "" then Reason = "No reason!"; end
...
if not UseFadmin then
  ...
  function LDRP.Scoreboard()
    ...
    for k, v in pairs(player.GetAll()) do
      ...
      Info:SetSize(16, 16);
      Info.DoClick = function() LDRP.PlayerInfo(v); end;
