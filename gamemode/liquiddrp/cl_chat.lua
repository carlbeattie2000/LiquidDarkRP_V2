...
local DoShit = function()
  ...
  ChatBox.SettingsMenu.ExpandButton.DoClick = function(self)
    ...
    if not ChatBox.SettingsMenu.Expanded then
      ...
      timer.Simple(0.6, function() ChatBox.SettingsMenu.Expanding = false; end);
    else
  ...
  ChatBox.SettingsMenu.Settings.Font.OnSelect = function(self, index, value, data) RunConsoleCommand("pchat_font", value); end;
  ...
  ChatBox.SettingsMenu.Settings.URLSettings.Think = function(self)
    if not (ChatBox.SettingsMenu.Settings.URLSettings.Expanded == ChatBox.SettingsMenu.Settings.OpenURLs:GetChecked()) and not ChatBox.SettingsMenu.Settings.URLSettings.Expanding then
      ...
      timer.Simple(0.2, function() ChatBox.SettingsMenu.Settings.URLSettings.Expanding = false; end);
    end
  end;
  ...
  ChatBox.TextEntry.OnEnter = function() ChatBox:SendMessage(); end;
  ChatBox.TextEntry.OnKeyCodeTyped = function(self, Key)
    ...
    elseif Key == KEY_UP then
      if ChatBox.PrevMessage then ChatBox.PrevMessage:DoClick(); end
  ...
  ChatBox.PrevMessage.Paint = function(self, w, h) SKIN:PaintNumberUp(ChatBox.PrevMessage, w, h); end;
  ChatBox.PrevMessage.DoClick = function(self)
    if not self:IsVisible() then return; end
    if not ChatBox.NextMessage:IsVisible() then ChatBox.NextMessage:SetVisible(true); end
    if ChatBox.CurMessage < 2 then SentMessages[1] = ChatBox.TextEntry:GetText(); end
    ChatBox.CurMessage = math.min(#SentMessages, ChatBox.CurMessage + 1);
    ChatBox.TextEntry:SetText(SentMessages[ChatBox.CurMessage]);
    if ChatBox.CurMessage == #SentMessages then self:SetVisible(false); end
  end;
  ...
  ChatBox.NextMessage.Paint = function(self, w, h) SKIN:PaintNumberDown(self, w, h); end;
  ChatBox.NextMessage.DoClick = function(self)
    if not self:IsVisible() then return; end
    if not ChatBox.PrevMessage:IsVisible() then ChatBox.PrevMessage:SetVisible(true); end
  ...
  HTMLPanel.FinishedURL = function(self) HTMLFrame:SetTitle(self.PageTitle); end;
  ...
  function chat.AddText(...)
    ...
    for _, v in pairs({...}) do
      ...
      if type(v) == "table" then CurColor = v; end
      if type(v) == "Angle" then v = "Angle(" .. v.p .. ", " .. v.y .. ", " .. v.r .. ")"; end
      if type(v) == "Vector" then v = "Vector(" .. v.x .. ", " .. v.y .. ", " .. v.z .. ")"; end
      if type(v) == "boolean" then v = v and "true" or "false"; end
      if type(v) == "Entity" then v = "[" .. v:EntIndex() .. "]" .. v:GetClass(); end
    ...
    if ShouldPrint then PrintColor(...); end
  end

  FindMetaTable("Player").ChatPrint = function(self, Str) chat.AddText(Str); end;
  ...
  local CheckForLinks = function(Player, Text, Team, Dead)
    ...
    if not Text or not Pattern then return; end
  ...
  ChatBox.ComeBack = function(self)
    ...
    self.CurMessage = 1;
    if #SentMessages > 1 then self.PrevMessage:SetVisible(true); end
  ...
  local OpenChat = function(Player, Bind, Pressed)
    if ChatBox.Visible then --Chatbox is open
      if Bind == "cancelselect" then --Close chat on Esc
        ChatBox.GoAway();
        return true;
      end

      if Bind ~= "toggleconsole" then --Don't do anything
        return true;
      end
  ...
  ChatBox.GoAway = function()
    ...
    timer.Simple(0, function() self.Team = false; end);
    for k, v in pairs(ChatBox.Messages:GetCanvas():GetChildren()) do
      v:SetCursor("arrow");
    end
...
hook.Add("HUDShouldDraw", "Disable old CB", function(hud) if hud == "CHudChat" then return false; end end);
