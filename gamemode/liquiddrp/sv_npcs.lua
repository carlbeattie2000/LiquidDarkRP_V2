...
function LDRP_SH.CreateNPC(name, pos, ang)
  if LDRP_SH.ShopPoses[name] or not LDRP_SH.AllNPCs[name] then return; end
  if not LDRP_SH.UsePaycheckLady and name == "Paycheck Lady" then
  ...
  if ang then NewNPC:SetAngles(ang); end
  NewNPC:Spawn();
end
...
function LDRP.UseNPC(ply, ent)
  if ent and ent:IsValid() and ent:GetClass() == "shop_base" and ent.ShopType and not ply.CanUse then
    local b = LDRP_SH.AllNPCs[ent.ShopType];
    if b then
      ply.CanUse = true;
      timer.Simple(2, function() ply.CanUse = nil; end);
      umsg.Start(b.uname, ply);
      if b.NeedTeam and not table.HasValue(b.NeedTeam, ply:Team()) then umsg.Float(b.NeedTeam[1]); end
      umsg.End();
    end
  end
end
...
function LDRP.SetNPCPos(ply, cmd, args)
  ...
  local Type = args[1];
  if not LDRP_SH.AllNPCs[Type] then
    ply:PrintMessage(2, "Type does not exist.");
    return;
  end
...
function LDRP.DrugDealerCMD(ply, cmd, args)
  if not args or not args[1] or not args[2] or ply:Team() ~= TEAM_DRUGDEALER then return; end
  if not LDRP_SH.ShopPoses["Drug Dealer"] or ply:GetPos():Distance(LDRP_SH.ShopPoses["Drug Dealer"]) > 300 then return; end
  local Do = args[1];
  local What = args[2];
  if Do == "buy" then
    if What == "weed" then
      if LDRP.SellingWeed then
        ...
        if not ply:CanCarry("weed seed", Amount) then
    ...
    elseif What == "shrooms" then
      if LDRP.SellingShrooms then
        ...
        if not ply:CanCarry("mushroom spore", Amount) then
          ply:LiquidChat("DRUG DEALER", Color(0, 180, 0), "You need to free up inventory space.");
          return;
        end
...
function LDRP.BailNPC(ply, cmd, args)
  if not LDRP_SH.ShopPoses["Bail NPC"] or ply:GetPos():Distance(LDRP_SH.ShopPoses["Bail NPC"]) > 300 then return; end
...
function LDRP.StoreCMD(ply, cmd, args)
  ...
  if not Type or not ItemTbl then return; end
  if Type == "buy" then
