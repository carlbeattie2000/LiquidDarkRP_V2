...
function LDRP.ConfirmVender(Type, Item, Cost)
  ...
  local CantSell = (Type == "sell") and (not LocalPlayer().Inventory[item] or LocalPlayer().Inventory[item] < 1);
  ...
  YesButton.DoClick = function()
    ...
    timer.Simple(.4, function() ConfirmWindow:Close(); end);
  end;
...
function LDRP.RulesMenu()
  ...
  Window.Paint = function()
    ...
    if Loading then draw.SimpleTextOutlined("Loading Webpage", "HUDNumber", w * .5, h * .5, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0)); end
  end;
  ...
  HTML.FinishedURL = function() Loading = false; end;
  HTML.OpeningURL = function() Loading = true; end;
  return "";
end
...
function LDRP.BankMenu(ply, cmd, args)
  ...
  function MainBankBackground:Paint()
    ...
    for k, v in pairs(LocalPlayer().Bank) do
      ...
      if v and v >= 1 then BankWeight = BankWeight + (LDRP_SH.AllItems[k].weight * v); end
    end
  ...
  function BankItemsList:Think()
    for k, v in pairs(LocalPlayer().Bank) do
      ...
      timer.Simple(.001, function()
        if not BankItemsList:IsValid() then return; end
  ...
  function InvItemsList:Think()
    for k, v in pairs(LocalPlayer().Inventory) do
      ...
      timer.Simple(.001, function()
        if not InvItemsList:IsValid() then return; end
  ...
  MainBankBackground:NewButton("Deposit Money", 4, 522, 592, 20, function()
    local am = tonumber(InputCash:GetValue());
    if am and am > 0 then
      RunConsoleCommand("_bnk", "money", -am);
    else
      LocalPlayer():ChatPrint("Please enter a valid number.");
    end
  end);

  MainBankBackground:NewButton("Withdraw Money", 4, 548, 592, 20, function()
    local am = tonumber(InputCash:GetValue());
    if am and am > 0 then
      RunConsoleCommand("_bnk", "money", am);
    else
