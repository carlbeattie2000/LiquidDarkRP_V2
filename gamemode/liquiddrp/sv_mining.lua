...
function LDRP.SpawnRock(Type, Position, Override, NoQuery)
  ...
  if Count >= Tbl.max and not Override then return; end
  ...
  table.insert(LDRP.AllRockPositions, {
    ["Position"] = Position,
    ["Type"] = Type
  });
...
local function GetAvailPoses()
  ...
  for _, v in ipairs(LDRP.AllRockPositions) do
    ...
    if not table.HasValue(LDRP_SH.SpawnedRockPositions, v.Position) then table.insert(available, v); end
  end
  return available;
end
...
concommand.Add("ldrp_spawnrock", function(ply, cmd, args)
  ...
  local Type = args[1];
  if not LDRP_SH.Rocks[Type] then
...
timer.Create("RandomRockSpawn", 400, 0, function()
  ...
  if numPoses >= 8 or #availPoses == 0 then --Too many rocks, or no open positions.
    return;
  end
