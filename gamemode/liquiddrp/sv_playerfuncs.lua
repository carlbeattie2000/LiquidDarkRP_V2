...
function meta:UIDToFile(fil)
  if self and self:IsValid() and fil then return "LiquidDRP/" .. fil .. "_" .. self:UniqueID(); end
end
...
function meta:SavePlayer(savewhat)
  if self and self:IsValid() and self.Inited then
    if not file.IsDir("LiquidDRP", "DATA") then file.CreateDir("LiquidDRP"); end
    if savewhat and self.savewat ~= nil then
      if self[savewhat] then file.Write("LiquidDRP/" .. savewhat .. "_" .. self:UniqueID() .. ".txt", von.serialize(self[savewhat])); end
    else
      for k, v in pairs(SaveTables) do
        if self[v] then file.Write("LiquidDRP/" .. v .. "_" .. self:UniqueID() .. ".txt", von.serialize(self[v])); end
      end
    end
  end
end
...
function meta:GiveLevel(skill, am)
  if self:IsValid() and self.Character and self.Character.Skills[skill] then
    ...
    umsg.Float(cur.lvl);
    umsg.End();
    if LDRP_SH.AllSkills[skill].LvlFunction then LDRP_SH.AllSkills[skill].OnLevelUp(self); end
