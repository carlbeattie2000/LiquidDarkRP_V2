...
function LDRP.TradingMenu(tradingwith)
  ...
  TM.Window.Think = function() if not tradingwith or not tradingwith:IsValid() then TM.Window:Close(); end end;
  ...
  TM.Inventory.Paint = function() draw.RoundedBox(6, 0, 0, TM.Inventory:GetWide(), TM.Inventory:GetTall(), LDRP.ColorMod(-30, -30, -30, -40)); end;
  ...
  function TM.Inventory:Think() -- This is probably a horrible way to do this, but look at half of the DarkRP code and see how much shittier that is instead of complaining about my code
    for k, v in pairs(LocalPlayer().Inventory) do
      ...
      timer.Simple(.00001, function() if TM.Inventory:IsValid() then TM.Inventory:Rebuild(); end end);
    end
  end
  ...
  TM.ChatBox.Paint = function() draw.RoundedBox(6, 0, 0, TM.ChatBox:GetWide(), TM.ChatBox:GetTall(), LDRP.ColorMod(-30, -30, -30, -40)); end;
  ...
  function TM.ChatBox:AddChat(text, color, ByTrader)
    ...
    chat.PlaySound();
    timer.Simple(.001, function() TM.ChatBox.VBar:AddScroll(TM.ChatBox.VBar.BarSize); end);
  end
  ...
  function TM.MyTrade:Think()
    local Chk = not MoneyIconTrade or not MoneyIconTrade:IsValid();
    if not FreshCash and LocalPlayer().Trading["cashh"] and LocalPlayer().Trading["cashh"] >= 1 then FreshCash = LocalPlayer().Trading["cashh"]; end
    ...
    if Chk and LocalPlayer().Trading["cashh"] and LocalPlayer().Trading["cashh"] >= 1 then
      ...
      TM.ConfirmTrade:SetDisabled(true);
      RunConsoleCommand("__trd", "decline");
    elseif not Chk and (not LocalPlayer().Trading["cashh"] or LocalPlayer().Trading["cashh"] <= 0) then
    ...
    for k, v in pairs(LocalPlayer().Trading) do
      ...
      timer.Simple(.00001, function() if TM.MyTrade:IsValid() then TM.MyTrade:Rebuild(); end end);
    end
  end
  ...
  function TM.TheirTrade:Think()
    local Chk = not MoneyIconTheir or not MoneyIconTheir:IsValid();
    if not FreshCash2 and LocalPlayer().BeingTraded["cashh"] and LocalPlayer().BeingTraded["cashh"] >= 1 then FreshCash2 = LocalPlayer().BeingTraded["cashh"]; end
    ...
    if Chk and FreshCash2 and FreshCash2 >= 1 then
      ...
      TM.ConfirmTrade:SetDisabled(true);
      RunConsoleCommand("__trd", "decline");
    elseif not Chk and (not FreshCash2 or FreshCash2 <= 0) then
    ...
    for k, v in pairs(LocalPlayer().BeingTraded) do
      ...
      timer.Simple(.00001, function() if TM.TheirTrade:IsValid() then TM.TheirTrade:Rebuild(); end end);
    end
  end
  ...
  function TM.ChatEntry:OnEnter()
    ...
    self:SetText("");
    self:RequestFocus();
    timer.Simple(0, function() TM.ChatBox.VBar:AddScroll(TM.ChatBox.VBar.BarSize); end);
  end
  ...
  TM.AcceptTrade.DoClick = function() TM.ConfirmTrade:SetDisabled(false); end;
  ...
  TM.ConfirmTrade:SetDisabled(true);
  TM.ConfirmTrade.DoClick = function() RunConsoleCommand("__trd", "accept"); end;
...
function LDRP.ReceiveChat(um)
  if ChatBox and ChatBox:IsValid() then
    ...
    local s = false;
    if string.sub(Txt, 1, 3) == "PlY" then
      s = true;
      Txt = string.sub(Txt, 4, string.len(Txt));
    end
