...
local function SavePlayersBeforeShutDown()
  ...
  table.foreachi(plys, function(_, v)
    ...
    for __, w in ipairs(v:GetWeapons()) do
      ...
      if v:CanCarry(wepType) then v:AddItem(wepType, 1); end
    end
...
function LDRP.InitializeSelf(ply, cmd, args)
  ...
  if ply:LiquidFile("Inventory") then
    ...
    for k, v in pairs(Tbl) do -- Remove unexisting items and recalculate weight
      ...
      if v >= 1 then InvWeight = InvWeight + (Check[k].weight * v); end
    end
  else
    ply.Inventory = {};
  end
  ...
  if ply:LiquidFile("Character") then
    ...
    ply.Character.InvWeight = {
      ["cur"] = 0,
      ["allowed"] = 25
    };
  end

  if not ply.Character.BankWeight then
    ply.Character.Bank = {
      ["curcash"] = 0
    };

    ply.Character.BankWeight = {
      ["cur"] = 0,
      ["allowed"] = 40
    };

    ply.Character.InterestRate = {
      ["cur"] = 0
    };
  end

  if not ply.Character.InterestRate["lastcollected"] then ply.Character.InterestRate["lastcollected"] = os.time(); end
  timer.Simple(3, function()
    if not ply:IsValid() then return; end
  ...
  timer.Simple(2, function()
    if not ply:IsValid() then return; end
    if not file.Exists("liquiddrp/didtutorial_" .. ply:UniqueID() .. ".txt", "DATA") then
  ...
  for k, v in pairs(LDRP_SH.AllSkills) do
    if not ply.Character.Skills[k] then
...
function LDRP.ItemCMD(ply, cmd, args)
  if RPArrestedPlayers[ply:SteamID()] then
    ply:ChatPrint("You are in jail!");
    return;
  end

  if not args or not args[1] or not args[2] or args[3] then return; end
  if not ply:Alive() then
    ply:ChatPrint("You must be alive to do this!");
    return;
  end
  ...
  if b and ply:HasItem(it) then
    if args[1] == "drop" then
      ...
      local DroppedItem;
      ply:AddItem(it, -1);
      if b.realent and not b.iswep then
      ...
      if CPPI then --Support for FPP
        DroppedItem:CPPISetOwner(ply);
      end
