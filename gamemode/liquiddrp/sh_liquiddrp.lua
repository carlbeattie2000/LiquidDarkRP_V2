...
function LDRP.AddNewPrinter(name, model, cost, prints, material, color)
  if not LDRP_SH.First then LDRP_SH.First = name; end
...
function LDRP.AddNewItem(Name, description, model, weight, clr, mat, realent, usefunc, usename, fakemdl)
  ...
  if string.sub(Name, 1, 6) == "weapon" then LDRP_SH.AllItems[name].iswep = true; end
  LDRP_SH.AllItems[name].nicename = Name;
  LDRP_SH.AllItems[name].mdl = model;
  if CLIENT and model:EndsWith(".png") then LDRP_SH.AllItems[name].Material = Material(model, "noclamp smooth"); end
  ...
  if realent then LDRP_SH.AllItems[name].realent = realent; end
  if fakemdl then LDRP_SH.AllItems[name].fakemdl = fakemdl; end
end
...
LDRP.AddNewItem("Weed Bag", "A dub sac of bud. Let's get stoned.", "models/katharsmodels/contraband/zak_wiet/zak_wiet.mdl", .03, Color(255, 255, 255, 255), "", nil, function(ply)
  ...
  timer.Create("WeedTrip_" .. UID, 5, 4, function()
    if not ply:IsValid() then timer.Remove("WeedTrip_" .. UID); end
    am = am - 1;
...
LDRP.AddNewItem("Carrot", "Eat it for 2 HP", "models/props/carrot.mdl", .1, Color(255, 255, 255, 255), "", nil, function(ply)
  ply:ChatPrint("Yum. A carrot.");
  ply:EmitSound("LiquidDRP/eating.wav");
  ply:SetHealth(math.Clamp(ply:Health() + 2, 1, 100));
end, "Eat");
...
LDRP.AddNewItem("Melon", "Eat it for 10 HP", "models/props_junk/watermelon01.mdl", .15, Color(255, 255, 255, 255), "", nil, function(ply)
  ply:ChatPrint("MMMMM!! MELONS!!!!");
  ply:EmitSound("LiquidDRP/eating.wav");
  ply:SetHealth(math.Clamp(ply:Health() + 14, 1, 100));
end, "Eat");
...
function LDRP.AddWeapon(class, nicename, description, model, weight, color, mat)
  LDRP.AddNewItem(class, description, model, weight, color or Color(255, 255, 255, 255), mat, class, function(ply)
...
function LDRP.AddSkill(name, description, mdl, exptable, costtbl, lvlfunc)
  ...
  if type(func) == "function" then LDRP_SH.AllSkills[name].LvlFunction = true; end
end
  ...
  if CLIENT then
    local function NPCUMSG(um)
      ...
      if z == (nil or 0) then
        ...
        LDRP_SH.OpenNPCWindow(name, mdl, "I only talk to " .. team.GetName(z) .. "s.", {
          [LeaveMessages[math.random(1, #LeaveMessages)]] = function() end
        });
      end
    end
...
LDRP.AddCustomNPC("Mayor Elections", "models/mossman.mdl", "SendMayorElectionMenu");
LDRP_SH.AllStores = {};
...
LDRP.CreateStore("General Store", "models/humans/group01/male_09.mdl", "Welcome to the General Store!", {
  ["Carrot Seed"] = LDRP_SH.CarrotSeedPrice,
  ["Melon Seed"] = 75,
  ["Pistol Ammo"] = 100,
  ["Rifle Ammo"] = 140,
  ["Shotgun Ammo"] = 140
}, {
  ["Carrot"] = LDRP_SH.CarrotBuyPrice,
  ["Melon"] = 50
});

LDRP.CreateStore("Miner", "models/Humans/Group02/male_02.mdl", "I'm too lazy to mine rocks. Do it for me.", {
  ["Pickaxe"] = 120,
  ["hammer"] = 200
}, {
  ["Stone"] = 500,
...
LDRP.CreateCraftItem("Pickaxe Upgrade #1", "models/weapons/w_stone_pickaxe.mdl", 8, 1, 100, {
  ["gold"] = 5,
  ["stone"] = 5,
  ["pickaxe"] = 1
}, {
...
LDRP.CreateCraftItem("Pickaxe Diamond", "models/weapons/w_stone_pickaxe.mdl", 8, 1, 100, {
  ["diamond"] = 20,
  ["pickaxe1"] = 2
}, {
  ["pickaxediamond"] = 1
...
LDRP.CreateCraftItem("Pickaxe Upgrade VIP", "models/weapons/w_stone_pickaxe.mdl", 12, 1, 130, {
  ["ruby"] = 4,
  ["stone"] = 4,
  ["pickaxe1"] = 1
}, {
...
function LDRP.CreateOutfit(name, description, model, weight, clr, mat, realent, usefunc)
  LDRP.AddNewItem(name, description, model, weight, clr, mat, realent, usefunc or function(ply)
    ply:SetModel(model);
    ply:SetHealth(400);
...
function AddAdminControl(name, description, func, sv)
  ...
  if sv then
    local function AdminFunc(ply, cmd, args)
      if not ply:IsUserGroup("Moderator") and not ply:IsAdmin() then return; end
      func(ply, cmd, args);
    end
...
if SERVER then
  ...
  concommand.Add("ldrp_addvip", function(ply, cmd, args)
    ...
    if not args[1] then
      ply:ChatPrint("Command format: ldrp_addvip 'name'");
      return;
    end

    local Target = {};
    for k, v in pairs(player.GetAll()) do
      if string.find(string.lower(v:GetName()), string.lower(args[1])) then table.insert(Target, v); end
    end

    if table.Count(Target) > 1 then
      ply:ChatPrint("More than 1 target found! Be more specific on the name.");
    else
      if not Target[1]:IsValid() then
        ply:ChatPrint("Player isn't valid.");
        return;
      end
