...
function LDRP.TradingCMD(ply, cmd, args)
  ...
  elseif args[1] == "item" then
    if ply.Trading and ply.Trading:IsValid() then
      ...
      if LDRP_SH.AllItems[args[2]] and ply:HasItem(args[2], num) then
        if not ply.Trading:CanCarry(args[2], num) then
  ...
  elseif args[1] == "accept" then
    if ply.Trading and ply.Trading:IsValid() and not ply.Confirmed then
      ply.Confirmed = true;
      if not ply.Trading.Confirmed then
        umsg.Start("SendTradeChat", ply);
      ...
      for k, v in pairs(ply.Trading.TradeItems) do
        if v < 1 then continue; end
        if k == "cashh" then
          if ply.Trading:CanAfford(v) then
            ply.Trading:AddMoney(-v);
            ply:AddMoney(v);
          end
        ...
        if ply.Trading:HasItem(k, v) then
          ply.Trading:AddItem(k, -v);
          ply:AddItem(k, v);
        end
      end
  ...
  elseif args[1] == "decline" then
    if ply.Trading and ply.Trading:IsValid() and ply.Confirmed then ply.Confirmed = false; end
  ...
  elseif args[1] == "f4_send" then
    ...
    for i, v in ipairs(playersConnected) do
      if v:Nick() == tradeWithPlayerName then playerEntity = v; end
    end

    ply.TradedWith = nil;
    local Trade = playerEntity;
    if Trade and Trade:IsValid() and Trade:IsPlayer() then
      ply:LiquidChat("TRADING", Color(0, 192, 10), "Sent a trade request to " .. Trade:GetName());
      Trade:LiquidChat("TRADING", Color(0, 192, 10), ply:GetName() .. " has created a trade request. Type /accept in chat to accept.");
      ply.TradedWith = Trade;
      Trade.TradeRequest = ply;
