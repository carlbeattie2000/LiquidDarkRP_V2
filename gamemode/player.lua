...
local function RPName(ply, args)
  ...
  local allowed = {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', 'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'z', 'x', 'c', 'v', 'b', 'n', 'm', ' ', '(', ')', '[', ']', '!', '@', '#', '$', '%', '^', '&', '*', '-', '_', '=', '+', '|', '\\'};
...
function meta:RestorePlayerData()
  ...
  DB.RetrievePlayerData(self, function(data)
    ...
    if not data then DB.CreatePlayerData(self, info.rpname, info.wallet, info.salary); end
  end, function()
...
function meta:ChangeTeam(t, force)
  ...
  if self:Team() == TEAM_MAYOR and tobool(GetConVarNumber("DarkRP_LockDown")) then UnLockdown(self); end
  ...
  if self.DarkRPVars.HasGunlicense then self:SetDarkRPVar("HasGunlicense", false); end
  if TEAM.Haslicense and GAMEMODE.Config.license then self:SetDarkRPVar("HasGunlicense", true); end
  ...
  if GAMEMODE.Config.removeclassitems then
    ...
    for k, v in pairs(ents.FindByClass("pot")) do
      ...
      if owner and owner:IsValid() and owner == self and v.DrugType and v.DrugType == "weed seed" or v.DrugType == "mushroom" then v:Remove(); end
    end
  end
...
function meta:UpdateJob(job)
  ...
  if LDRP_SH.ChangeJobFuncs[job] then LDRP_SH.ChangeJobFuncs[job](self); end
end
    ...
    if not RPArrestedPlayers[self:SteamID()] then
      DB.RetrieveSalary(self, function(amount)
        ...
        if amount == 0 or not amount then
          ...
          self.CurCheck = 0;
          OldCheck = nil;
          if OldCheck and LDRP_SH.StackChecks then self.CurCheck = self.CurCheck + OldCheck; end
        end
      end);
    else
...
local function JailPos(ply)
  ...
  if (ply:Team() == TEAM_CHIEF and GAMEMODE.Config.chiefjailpos) or ply:HasPriv("rp_commands") then
    ...
    if GAMEMODE.Config.chiefjailpos then str = "Chief or " .. str; end
    Notify(ply, 1, 4, str);
  end
  return "";
end
...
local function AddJailPos(ply)
  ...
  if (ply:Team() == TEAM_CHIEF and GAMEMODE.Config.chiefjailpos) or ply:HasPriv("rp_commands") then
    ...
    if GAMEMODE.Config.chiefjailpos then str = LANGUAGE.chief_or .. str; end
    Notify(ply, 1, 4, str);
  end
  return "";
end
...
function meta:UnownAll()
  for k, v in pairs(ents.GetAll()) do
    if v:IsOwnable() and v:OwnedBy(self) == true then v:Fire("unlock", "", 0.6); end
  end
  ...
  for k, v in pairs(player.GetAll()) do
    if v:GetTable().Ownedz then
      for n, m in pairs(v:GetTable().Ownedz) do
        if IsValid(m) and m:AllowedToOwn(self) then m:RemoveAllowed(self); end
      end
    end
  end
