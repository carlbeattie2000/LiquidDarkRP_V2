...
function LDRP_Hitman.RemoveHit(um)
  ...
  for k, v in pairs(LDRP_Hitman.Hits) do
    if string.lower(v:UniqueID()) == playa then LDRP_Hitman.Hits[k] = nil; end
  end
end
...
function LDRP_Hitman.HitMenu()
  ...
  B.Close.DoClick = function()
    if not B.Closin then
      B.Opened = CurTime();
      B.Closin = true;
      timer.Simple(.5, function() B.BG:Close(); end);
    end
  end;

  local cl = Color(110, 110, 110, 210);
  B.Close.Paint = function(s)
    local x, y = s:GetSize();
    draw.RoundedBox(8, 0, 0, x, y, Color(cl.r, cl.g, cl.b, cl.a * B.OpenMath));
    draw.SimpleTextOutlined("X", "Trebuchet24", x * .5, y * .5, Color(255, 255, 255, 200 * B.OpenMath), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 200 * B.OpenMath));
  end;
  ...
  B.List.Paint = function(s)
    local x, y = s:GetSize();
    draw.RoundedBox(10, 0, 0, x, y, Color(40, 40, 40, 200 * B.OpenMath));
  end;
  ...
  B.List.Think = function()
    for k, v in pairs(LDRP_Hitman.Hits) do
      if not Inlist[k] or Inlist[k] ~= v then
    ...
    for k, v in pairs(Inlist) do
      if not LDRP_Hitman.Hits[k] then
