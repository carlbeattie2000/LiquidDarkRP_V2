...
function LDRP.ProtectSaveInfo(ply)
  if not ply:IsValid() or not ply.Protect then return; end
...
function LDRP.CanTouch(ply, ent)
  ...
  return (not Valid) or (Valid and Valid == ply) or (Valid and Valid.Protect and Valid.Protect.Friends[ply:UniqueID()]) or (not ent.dt);
end
...
hook.Add("PlayerDisconnect", "LDRP Prop protect", function(ply)
  ...
  timer.Create("RemoveProps_" .. ply:UniqueID(), LDRP_Protector.DiscoTime or 120, 1, function()
    for k, v in pairs(ents.GetAll()) do
      if v:IsValid() and v.Owner then v:Remove(); end
    end
  end);
end);
...
hook.Add("PhysgunPickup", "LDRP Prop protect", function(ply, ent)
  ...
  if not CanTouch then
    if not ply.LastNotify or CurTime() - ply.LastNotify > 3 then
      ply.LastNotify = CurTime();
...
hook.Add("PhysgunDrop", "LDRP Prop protect", function(ply, ent)
  ...
  if not LDRP.CanTouch(ply, ent) then
    if not ply.LastNotify or CurTime() - ply.LastNotify > 3 then
      ply.LastNotify = CurTime();
      Notify(ply, 1, 5, "This is not your prop!");
    end
...
hook.Add("CanTool", "LDRP Prop protect", function(ply, tr, mode)
  ...
  if table.HasValue(LDRP_Protector.Restrict.Tools, STL(mode)) then
    Notify(ply, 1, 5, "This tool is restricted!");
    return false;
  end
end);

hook.Add("PlayerUse", "LDRP Prop protect", function(ply, ent) if LDRP_Protector.UseProtect and ent:GetClass() ~= "shop_base" and not LDRP.CanTouch(ply, ent) then return false; end end);
hook.Add("PlayerSpawnedProp", "LDRP Prop protect", function(ply, mdl, ent)
  local EntMdl = ent:GetModel();
  if table.HasValue(LDRP_Protector.Restrict.Models, STL(EntMdl)) then
    ent:Remove();
    Notify(ply, 1, 5, "This model is restricted!");
    return false;
  end
...
hook.Add("PlayerInitialSpawn", "LDRP Prop protect", function(ply)
  ...
  if timer.Exists("RemoveProps_" .. UID) then timer.Remove("RemoveProps_" .. UID); end
  if not file.Exists("protector/info_" .. UID .. ".txt", "DATA") then
    ply.Protect = {};
    ply.Protect.Friends = {};
    LDRP.ProtectSaveInfo(ply);
  else
...
concommand.Add("_protector", function(ply, cmd, args)
  ...
  if Type == "friend" then
    ...
    if ply.Protect.Friends[args[2]] then
      ...
      for k, v in pairs(player.GetAll()) do
        if string.lower(v:UniqueID()) == string.lower(args[2]) then Ply = v; end
      end

      if not Ply or not Ply:IsValid() then
        Notify(ply, 5, 5, "That is not a valid player.");
        return;
      end
  ...
  elseif Type == "useprotect" then
    if not ply:IsAdmin() or ply:IsSuperAdmin() and not ply:IsSuperAdmin() then return; end
    LDRP_Protector.UseProtect = not LDRP_Protector.UseProtect;
