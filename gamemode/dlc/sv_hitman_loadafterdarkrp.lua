...
function LDRP_Hitman.PlaceHit(ply, arglong)
  ...
  if ply:Team() == TEAM_HITMAN then
    ...
    if not arglong or not args or not args[1] or args[2] or args[1] == "" then
      ply:ChatPrint("Wrong format. Format: '/hit playersname'. This will cost $" .. LDRP_SH_Hitman.HitPrice);
      return "";
    end

    local playa = {};
    for k, v in pairs(player.GetAll()) do
      if string.find(string.lower(v:Name()), string.lower(args[1])) then table.insert(playa, v); end
    end

    if playa[2] then
      Notify(ply, 1, 3, "Multiple players found. Please type out the player's full name");
      return "";
    elseif not playa[1] then
    ...
    if LDRP_SH_Hitman.MultiHits then
      for k, v in pairs(LDRP_Hitman.Hits) do
        if v == ply2 then
          Notify(ply, 1, 3, "There is already a hit on this player.");
          return "";
        end
      end
    end

    if not ply:CanAfford(LDRP_SH_Hitman.HitPrice) then
...
hook.Add("DoPlayerDeath", "Removes hits, gives cash", function(victim, attk, dmginfo)
  if IsValid(attk) and type(attk) == "Player" and attk:Team() == TEAM_HITMAN and table.HasValue(LDRP_Hitman.Hits, victim) then
    ...
    for k, v in pairs(LDRP_Hitman.Hits) do
      if v == victim then LDRP_Hitman.Hits[k] = nil; end
    end
  end
end);

AddChatCommand("/buddies", function(ply)
  ply:ConCommand("protector_buddies");
  return "";
end);
