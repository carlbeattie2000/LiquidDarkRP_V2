...
function LDRP.MapSetupCMD(ply, cmd, args)
  ...
  if not ent or not ent:IsValid() or ent:GetClass() == "shop_base" or ent:IsWorld() or ent:IsPlayer() then return; end
  if args[1] == "addprop" then
    if LDRP.SavedInMap[ent] then
      ...
      LDRP.SavedProps[N] = {};
      if ent:GetClass() ~= "prop_physics" then LDRP.SavedProps[N].class = ent:GetClass(); end
  ...
  elseif args[1] == "addremove" then
    ...
    ply:PrintMessage(HUD_PRINTCONSOLE, "Usage: ldrp_map _command_ \nModify the game world. Syntax:");
    ply:PrintMessage(HUD_PRINTCONSOLE, "ldrp_map addprop \n\tAdds or removes the current prop you are" .. " looking at so that it will be loaded if the server is" .. " restarted.");
    ply:PrintMessage(HUD_PRINTCONSOLE, "ldrp_map addremove \n\tAdds the current prop you are looking" .. " at to the remove list so it is removed from the map" .. " whenever the map is loaded.");
  end
end
...
hook.Add("InitPostEntity", "LDRP.ProcessSavedProps", function()
  timer.Simple(1.5, function()
    for k, v in pairs(ents.GetAll()) do
      if v:IsValid() and not v:IsWorld() and table.HasValue(LDRP.SavedRemove, v:GetPos()) then v:Remove(); end
    end
