...
function AddCustomShipment(name, model, entity, price, category, Amount_of_guns_in_one_shipment, Sold_seperately, price_seperately, noshipment, classes, shipmodel)
  if not name or not model or not entity or not price or not Amount_of_guns_in_one_shipment or (Sold_seperately and not price_seperately) then
    ...
    print(text);
    hook.Add("PlayerSpawn", "ShipmentError", function(ply) if ply:IsAdmin() then ply:ChatPrint("WARNING: " .. text); end end);
    return;
  end
...
function AddEntity(name, entity, model, mat, clr, price, category, max, command, classes, CustomCheck)
  ...
  tblEnt.name = name;
  if type(tblEnt.allowed) == "number" then tblEnt.allowed = {tblEnt.allowed}; end
  local corrupt = checkValid(tblEnt, validEntity);
  if corrupt then ErrorNoHalt("Corrupt Entity \"" .. (name or "") .. "\": element " .. corrupt .. " is corrupt.\n"); end
  if SERVER and FPP then FPP.AddDefaultBlocked(blockTypes, tblEnt.ent); end
...
function AddAgenda(Title, Manager, Listeners)
  if not Manager then
    hook.Add("PlayerSpawn", "AgendaError", function(ply) if ply:IsAdmin() then ply:ChatPrint("WARNING: Agenda made incorrectly, there is no manager! failed to load!"); end end);
    return;
  end

  DarkRPAgendas[Manager] = {
    Title = Title,
    Listeners = Listeners
  };
end
...
local combinedItems = {DarkRPEntities, CustomShipments};
function categorizeStoreItems()
  DarkStoreItemsCategorized = {};
  for i = 1, #combinedItems do
    for ent_i = 1, #combinedItems[i] do
      if combinedItems[i][ent_i]["category"] ~= nil and not DarkStoreItemsCategorized[combinedItems[i][ent_i]["category"]] then DarkStoreItemsCategorized[combinedItems[i][ent_i]["category"]] = {}; end
