...
R_CASES.boosterCaseItems = {
  {
    ["type"] = "ent",
    ["item"] = "uranium_printer"
  },
  {
    ["type"] = "ent",
    ["item"] = "theif_outfit"
...
function CreateIcon(panel, model, sizex, sizey, mat, clr, clickfunc, campos, lookat) -- Liquid DarkRP CreateIcon
  ...
  icon:SetModel(model);
  if mat then icon.Entity:SetMaterial(mat); end
  if clr then icon:SetColor(clr); end
  ...
  iconbutton:SetText("");
  iconbutton.OnCursorEntered = function() Hovered = true; end;
  iconbutton.OnCursorExited = function()
    Hovered = false;
    Pressed = false;
  end;

  iconbutton.OnMousePressed = function() Pressed = true; end;
...
function pmeta:Name()
  return GAMEMODE.Config.allowrpnames and self.DarkRPVars and self.DarkRPVars.rpname or self:SteamName();
end
...
function pmeta:HasItem(name, am)
  if not LocalPlayer().Inventory then return false; end
  local Check = LocalPlayer().Inventory[name] or 0;
...
function FindPlayer(info)
  ...
  -- Find by RP Name
  for k, v in pairs(pls) do
    if string.find(string.lower(v.DarkRPVars.rpname or ""), string.lower(tostring(info))) ~= nil then return v; end
  end

  -- Find by Partial Nick
  for k, v in pairs(pls) do
    if string.find(string.lower(v:Name()), string.lower(tostring(info))) ~= nil then return v; end
  end
  return nil;
end
...
if UseFadmin then
  ...
  hook.Add("PostGamemodeLoaded", "FAdmin_DarkRP", function()
    ...
    FAdmin.StartHooks["DarkRP"] = function()
      ...
      -- Warrant
      FAdmin.ScoreBoard.Player:AddActionButton("Warrant", "FAdmin/icons/Message", Color(0, 0, 200, 255), function(ply)
        local t = LocalPlayer():Team();
        return t == TEAM_POLICE or t == TEAM_MAYOR or t == TEAM_CHIEF;
      end, function(ply, button) Derma_StringRequest("Warrant reason", "Enter the reason for the warrant", "", function(Reason) LocalPlayer():ConCommand("darkrp /warrant " .. ply:UserID() .. " " .. Reason); end); end);

      --wanted
      FAdmin.ScoreBoard.Player:AddActionButton(function(ply) return ((ply.DarkRPVars.wanted and "Unw") or "W") .. "anted"; end, function(ply) return "FAdmin/icons/jail", ply.DarkRPVars.wanted and "FAdmin/icons/disable"; end, Color(0, 0, 200, 255), function(ply)
        local t = LocalPlayer():Team();
        return t == TEAM_POLICE or t == TEAM_MAYOR or t == TEAM_CHIEF;
      end, function(ply, button)
        if not ply.DarkRPVars.wanted then
          Derma_StringRequest("wanted reason", "Enter the reason to arrest this player", "", function(Reason) LocalPlayer():ConCommand("darkrp /wanted " .. ply:UserID() .. " " .. Reason); end);
        else
      ...
      FAdmin.ScoreBoard.Player:AddActionButton("Ban from job", "FAdmin/icons/changeteam", Color(200, 0, 0, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "rp_commands", ply); end, teamban);
      FAdmin.ScoreBoard.Player:AddActionButton("Unban from job", function() return "FAdmin/icons/changeteam", "FAdmin/icons/disable"; end, Color(200, 0, 0, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "rp_commands", ply); end, teamban);
    end;
  end);
end
...
local function DoSpecialEffects(Type)
  ...
  if toggle then
    if thetype == "motionblur" then
      hook.Add("RenderScreenspaceEffects", thetype, function() DrawMotionBlur(0.05, 1.00, 0.035); end);
...
local function RPSelectwhohearit()
  ...
  hook.Add("HUDPaint", "RPinstructionsOnSayColors", function()
    ...
    for k, v in pairs(playercolors) do
      if v.Nick then draw.WordBox(2, w, h + k * 20, v:Nick(), "ScoreboardText", Color(0, 0, 0, 120), Color(255, 255, 255, 255)); end
    end
  end);

  hook.Add("Think", "RPGetRecipients", function()
    if not Messagemode then
      RPStopMessageMode();
      hook.Remove("Think", "RPGetRecipients");
      return;
    end
      ...
      if v ~= LocalPlayer() then
        ...
        elseif HearMode == "speak" and distance < 550 and not table.HasValue(playercolors, v) then
          if GAMEMODE.Config.dynamicvoice then
            if CL_IsInRoom(v) then table.insert(playercolors, v); end
          else
            table.insert(playercolors, v);
          end
...
function GM:ChatTextChanged(text)
  ...
  if string.sub(string.lower(text), 1, 3) == "/pm" then
    ...
    if string.find(plyname, " ") then plyname = string.sub(plyname, 1, string.find(plyname, " ") - 1); end
    HearMode = "pm";
    playercolors = {};
    if plyname ~= "" and FindPlayer(plyname) then playercolors = {FindPlayer(plyname)}; end
  ...
  if old ~= HearMode then playercolors = {}; end
end

function GM:PlayerStartVoice(ply)
  if ply == LocalPlayer() and LocalPlayer().DarkRPVars and IsValid(LocalPlayer().DarkRPVars.phone) then return; end
  isSpeaking = true;
  LocalPlayer().DarkRPVars = LocalPlayer().DarkRPVars or {};
...
function GM:PlayerEndVoice(ply) --voice/icntlk_pl.vtf
  if LocalPlayer().DarkRPVars and IsValid(LocalPlayer().DarkRPVars.phone) then
    ...
    timer.Simple(0.2, function() if IsValid(LocalPlayer().DarkRPVars.phone) then LocalPlayer():ConCommand("+voicerecord"); end end);
...
local function UpdateDoorData(um)
  ...
  if string.match(tostring(value), "Entity .([0-9]*)") then value = Entity(string.match(value, "Entity .([0-9]*)")); end
  if string.match(tostring(value), "Player .([0-9]*)") then value = Entity(string.match(value, "Player .([0-9]*)")); end
...
local function RetrievePlayerVar(entIndex, var, value, tries)
  ...
  value = tonumber(value) or value;
  if string.match(stringvalue, "Entity .([0-9]*)") then value = Entity(string.match(stringvalue, "Entity .([0-9]*)")); end
  if string.match(stringvalue, "^Player .([0-9]+).") then value = player.GetAll()[tonumber(string.match(stringvalue, "^Player .([0-9]+)."))]; end
  if stringvalue == "NULL" then value = NULL; end
...
local ConnectMessages = {"%s just joined the server", "%s just joined. Everyone hide!", "Welcome, %s. Good luck, your need it.", "Guess who's here!! Yep you guess it, %s arrived!", "Welcome, %s. You know what we do here right?", "Welcome, %s. Ready to obliterate everyone?"};
local WelcomeBackMessages = {"%s where have you been? We've missed you.", "Welcome back, %s. You look stronger... oh no.", "Oh look who decided to return....%s everyone.", "Snap, %s's back. You got that money you owe me?"};
...
hook.Add("player_connect_client", "AnnouceJoin", function(data)
  ...
  if lastKnownName and (name == lastKnownName) then
    if GetConVar("rebbelion_joinmessages2") and GetConVar("rebbelion_joinmessages2"):GetBool() then chat.AddText(unpack({Color(51, 255, 102), "Player joined ", Color(255, 255, 255), string.format(table.Random(WelcomeBackMessages), name)})); end
    return;
  end

  lastKnownName = name;
  if GetConVar("rebbelion_joinmessages") and GetConVar("rebbelion_joinmessages"):GetBool() then chat.AddText(unpack({Color(51, 255, 102), "Player joined ", Color(255, 255, 255), string.format(table.Random(ConnectMessages), name)})); end
end);
