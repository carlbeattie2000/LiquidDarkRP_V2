...
function TabsCrafting(parent)
	...
	local function populateCraftingTab()
		...
		if IsValid(craftingScrollPanel) then

			craftingScrollPanel:Clear()
		...
		for k, v in pairs(LDRP_SH.CraftItems) do
			...
			local craftingItemIcon = CreateIcon(
				craftingItemPanel, v.icon, 
				craftingItemPanel:GetWide()*.22, 
				craftingItemPanel:GetTall(), 
				modelDetails.mat || nil, 
				modelDetails.clr || nil, 
				function() end)
			...
			table.insert(markupObjs, {obj = {}, parent = craftingRecipeNeededContainer, recipe = v.recipe})
