  ...
  local function IncludeFolder(fol)
    ...
    for _, folder in SortedPairs(folders, true) do
      if folder ~= "." and folder ~= ".." then
        ...
        for _, File in SortedPairs(file.Find(fol .. folder .. "/cl_*.lua", "LUA"), true) do
          include(fol .. folder .. "/" .. File);
          print("Including clientside file " .. File);
        end
      end
    end
  end
...
function FAdmin.FindPlayer(info)
  ...
  for k, v in pairs(pls) do
    for _, PlayerInfo in pairs(InfoPlayers) do
      ...
      if tonumber(PlayerInfo) == v:UserID() and not table.HasValue(PlayersFound, v) then table.insert(PlayersFound, v); end
      -- Find by Steam ID
      if (PlayerInfo == v:SteamID() or v:SteamID() == "UNKNOWN") and not table.HasValue(PlayersFound, v) then table.insert(PlayersFound, v); end
      -- Find by Partial Nick
      if string.find(string.lower(v:Name()), string.lower(tostring(PlayerInfo)), 1, true) ~= nil and not table.HasValue(PlayersFound, v) then table.insert(PlayersFound, v); end
...
function FAdmin.IsEmpty(vector)
  ...
  for k, v in pairs(ents.FindInSphere(vector, 35)) do
    if v:IsNPC() or v:IsPlayer() or v:GetClass() == "prop_physics" then b = false; end
  end
  return a and b;
end
...
hook.Add("CanTool", "EntityCanTool", function(ply, trace, mode) if trace.Entity.CanTool and not FPP then return trace.Entity:CanTool(ply, trace, mode); end end);
...
hook.Add("OnPhysgunFreeze", "EntityPhysgunFreeze", function(weapon, physobj, ent, ply, ...) if ent.OnPhysgunFreeze and not FPP then return ent:OnPhysgunFreeze(weapon, physobj, ent, ply, ...); end end);
local IP = "";
if SERVER then
...
FindMetaTable("Player").FAdmin_GetGlobal = function(self, setting) return self.GlobalSetting and self.GlobalSetting[setting]; end;
if SERVER then
  local SetTypes = {
    Angle = "Angle",
    boolean = "Bool",
  ...
  function FAdmin.SetGlobalSetting(setting, value)
    if FAdmin.GlobalSetting[setting] == value then -- If the value didn't change, we don't need to resend it.
      return;
    end
  ...
  getmetatable(Player(0)).FAdmin_SetGlobal = function(self, setting, value)
    ...
    if self.GlobalSetting[setting] == value then -- If the value didn't change, we don't need to resend it.
      return;
    end
...
timer.Simple(0, function()
  for k, v in pairs(FAdmin.StartHooks) do
    if type(k) ~= "string" then FAdmin.StartHooks[k] = nil; end
  end
