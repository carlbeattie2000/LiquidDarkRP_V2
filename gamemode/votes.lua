...
function Vote:handleNewVote(ply, choice)
  ...
  if voteCount >= #player.GetAll() - excludeCount then self:handleEnd(); end
end
...
function GM.vote:create(question, voteType, target, time, callback, excludeVoters, fail)
  ...
  if target:IsPlayer() then GAMEMODE:Notify(target, 1, 4, LANGUAGE.vote_started); end
...
function GM.vote.HandleVoteEnd(id, OnePlayer)
  ...
  for a, b in pairs(player.GetAll()) do
    if not b:GetTable().VotesVoted then b:GetTable().VotesVoted = {}; end
