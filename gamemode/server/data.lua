...
local teamSpawns = {};
local jailPos = {};
local createSpawnPos, setUpNonOwnableDoors, setUpTeamOwnableDoors, setUpGroupDoors, createJailPos;
DB.Prefix = "ldrp";
local CustomTables = {};
...
function DarkRP.initDatabase()
  ...
  if MySQLite.CONNECTED_TO_MYSQL then MySQLite.queueQuery([[
  ...
  for k, v in ipairs(CustomTables) do
    ...
    for a, b in pairs(v["table_columns"]) do
      ...
      if b["allow_null"] == nil or (b["allow_null"] ~= nil and b["allow_null"] == false) then query = query .. " NOT NULL"; end
      if b["is_pk"] ~= nil and b["is_pk"] == true then query = query .. " PRIMARY KEY"; end
      --If we're on the last column, finish the query, otherwise add a comma and space.
      if a == #v["table_columns"] then
        query = query .. ");";
      else
        query = query .. ", ";
      end
  ...
  MySQLite.commit(function()
    ...
    if MySQLite.CONNECTED_TO_MYSQL then updateQuery = [[show tables like "]] .. DB.Prefix .. [[_cvars";]]; end
  ...
  for i = 0, count, 1000 do -- SQLite selecting limit
    MySQLite.query([[SELECT ]] .. DB.Prefix .. [[_wallets.steam, amount, salary, name FROM ]] .. DB.Prefix .. [[_wallets
      ...
      for k, v in pairs(data or {}) do
        ...
        MySQLite.query([[INSERT INTO ]] .. DB.Prefix .. [[_player VALUES(]] .. uniqueID .. [[,]] .. ((v.name == "NULL" or not v.name) and "NULL" or MySQLite.SQLStr(v.name)) .. [[,]] .. ((v.salary == "NULL" or not v.salary) and GAMEMODE.Config.normalsalary or v.salary) .. [[,]] .. v.amount .. [[);]]);
      end
...
function DB.StoreTeamSpawnPos(t, pos)
  ...
  MySQLite.query([[INSERT INTO ]] .. DB.Prefix .. [[_position VALUES(NULL, ]] .. MySQLite.SQLStr(map) .. [[, "T", ]] .. pos[1] .. [[, ]] .. pos[2] .. [[, ]] .. pos[3] .. [[);]], function()
...
function DB.RemoveTeamSpawnPos(t, callback)
  ...
  for k, v in pairs(teamSpawns) do
    if tonumber(v.team) == t then teamSpawns[k] = nil; end
  end
...
function DB.RetrieveTeamSpawnPos(ply)
  ...
  if teamSpawns then
    for k, v in pairs(teamSpawns) do
      if v.map == map and tonumber(v.team) == t then table.insert(returnal, Vector(v.x, v.y, v.z)); end
    end
...
function DB.RetrieveRPNames(ply, name, callback)
  MySQLite.query("SELECT COUNT(*) AS count FROM " .. DB.Prefix .. "_player WHERE rpname = " .. MySQLite.SQLStr(name) .. ";", function(r) callback(tonumber(r[1].count) > 0); end);
end

function DB.RetrievePlayerData(ply, callback, failed, attempts)
  attempts = attempts or 0;
  if attempts > 3 then return failed(); end
  MySQLite.query("SELECT rpname, wallet, salary FROM " .. DB.Prefix .. "_player WHERE uid = " .. ply:UniqueID() .. ";", callback, function() DB.RetrievePlayerData(ply, callback, failed, attempts + 1); end);
end
...
function DB.CreatePlayerData(ply, name, wallet, salary)
  ...
  MySQLite.query([[REPLACE INTO ]] .. DB.Prefix .. [[_player VALUES(]] .. ply:UniqueID() .. [[, ]] .. MySQLite.SQLStr(name) .. [[, ]] .. salary .. [[, ]] .. wallet .. ");");
end
...
function DB.RetrieveSalary(ply, callback)
  ...
  if ply:getDarkRPVar("salary") then -- First check the cache.
    return callback and callback(ply:getDarkRPVar("salary"));
  end
...
function DB.StoreDoorOwnability(ent)
  ...
  MySQLite.query([[REPLACE INTO ]] .. DB.Prefix .. [[_door VALUES(]] .. ent:DoorIndex() .. [[, ]] .. MySQLite.SQLStr(map) .. [[, ]] .. (ent.DoorData.title and MySQLite.SQLStr(ent.DoorData.title) or "NULL") .. [[, ]] .. "NULL" .. [[, ]] .. (ent.DoorData.NonOwnable and 1 or 0) .. [[);]]);
end
  ...
  MySQLite.query("SELECT idx, title, isLocked, isDisabled FROM " .. DB.Prefix .. "_door WHERE map = " .. MySQLite.SQLStr(string.lower(game.GetMap())) .. ";", function(r)
    ...
    for _, row in pairs(r) do
      ...
      if IsValid(e) then
        ...
        if r.isLocked ~= nil then e:Fire((tobool(row.locked) and "" or "un") .. "lock", "", 0); end
...
function DB.Log(text, force)
  ...
  if not DB.File then -- The log file of this session, if it's not there then make it!
    if not file.IsDir("DarkRP_logs", "DATA") then file.CreateDir("DarkRP_logs", "DATA"); end
