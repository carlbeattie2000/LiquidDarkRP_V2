...
---------------------------------------------------------------------------]]
hook.Add("DatabaseInitialized", "FAdmin_CreateMySQLTables", function() MySQLite.query("CREATE TABLE IF NOT EXISTS FAdminBans(SteamID VARCHAR(25) NOT NULL PRIMARY KEY, Nick VARCHAR(40), BanDate DATETIME, UnbanDate DATETIME, Reason VARCHAR(100), AdminName VARCHAR(40), Admin_steam VARCHAR(25));", function() hook.Call("FAdmin_RetrieveBans", nil); end); end);
--[[---------------------------------------------------------------------------
...
Unban someone
---------------------------------------------------------------------------]]
hook.Add("FAdmin_UnBan", "FAdmin_MySQLUnban", function(ply, steamID) MySQLite.query("DELETE FROM FAdminBans WHERE steamID = " .. MySQLite.SQLStr(steamID)); end);
--[[---------------------------------------------------------------------------
...
hook.Add("FAdmin_RetrieveBans", "getMySQLBans", function()
  ...
  local diffSeconds = MySQLite.CONNECTED_TO_MYSQL and "TIMESTAMPDIFF(SECOND, NOW(), UnbanDate)" or "strftime('%s', UnbanDate) - strftime('%s','now')";
