...
function initialize()
  if RP_MySQLConfig.EnableMySQL then
    timer.Simple(1, function() connectToMySQL(RP_MySQLConfig.Host, RP_MySQLConfig.Username, RP_MySQLConfig.Password, RP_MySQLConfig.Database_name, RP_MySQLConfig.Database_port); end);
  else
...
function commit(onFinished)
  ...
  call = function(...)
    ...
    if queuePos + 1 > #queue then
      if onFinished then -- All queries have finished
        onFinished();
      end
      return;
    end
...
function query(sqlText, callback, errorCallback)
  if CONNECTED_TO_MYSQL then
    ...
    query.onSuccess = function() if callback then callback(data, query:lastInsert()); end end;
    query:start();
    return;
  end
  ...
  if sql.LastError() and sql.LastError() ~= lastError then error("SQLite error: " .. sql.LastError()); end
...
function queryValue(sqlText, callback, errorCallback)
  if CONNECTED_TO_MYSQL then
    local query = databaseObject:query(sqlText);
    local data;
    query.onData = function(Q, D) data = D; end;
...
function connectToMySQL(host, username, password, database_name, database_port)
  ...
  databaseObject.onConnectionFailed = function(_, msg) Error("Connection failed! " .. tostring(msg)); end;
  databaseObject.onConnected = function()
    ...
    cachedQueries = {};
    timer.Create("darkrp_check_mysql_status", 60, 0, function() if databaseObject and databaseObject:status() == mysqlOO.DATABASE_NOT_CONNECTED then connectToMySQL(RP_MySQLConfig.Host, RP_MySQLConfig.Username, RP_MySQLConfig.Password, RP_MySQLConfig.Database_name, RP_MySQLConfig.Database_port); end end);
...
function SQLStr(str)
  if not CONNECTED_TO_MYSQL then return sql.SQLStr(str); end
