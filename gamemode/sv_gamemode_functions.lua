...
function GM:PlayerSpawnProp(ply, model)
  ...
  if ply:GetCount("Props") > 30 and not ply:IsVIP() then
  ...
  if string.find(model, "//") then
    Notify(ply, 1, 4, "You can't spawn this prop as it contains an invalid path. " .. model);
...
local function canSpawnWeapon(ply, class)
  if (not GAMEMODE.Config.adminweapons == 0 and ply:IsAdmin()) or (GAMEMODE.Config.adminweapons == 1 and ply:IsSuperAdmin()) then return true; end
...
function GM:EntityRemoved(ent)
  ...
  for k, v in pairs(DarkRPEntities or {}) do
    if IsValid(ent) and ent:GetClass() == v.ent and ent.dt and IsValid(ent.dt.owning_ent) and not ent.IsRemoved then
      ...
      if not ply["max" .. cmdname] then ply["max" .. cmdname] = 1; end
...
function addItemToPlayersInventory(ply, cmd, args)
  ...
  if not args[1] or not args[2] or not args[3] or type(tonumber(args[3])) ~= "number" then
  ...
  for k, v in pairs(playersOnServer) do
    if string.lower(v:Nick()) == plyNick then foundMatchingPlayer = v; end
  end

  if not foundMatchingPlayer then
...
function setInvWeight(ply, cmd, args)
  ...
  local foundMatchingPlayer = nil;
  for k, v in pairs(playersOnServer) do
    if string.lower(v:Nick()) == playerName then foundMatchingPlayer = v; end
  end
...
function GM:OnNPCKilled(victim, ent, weapon)
  ...
  if ent then
    ...
    if not ent:IsPlayer() and ent.SID then ent = Player(ent.SID); end
...
function GM:CanTool(ply, trace, mode)
  ...
  if table.HasValue(LDRP_Protector.Restrict.Tools, string.lower(mode)) then
    Notify(ply, 1, 5, "This tool is restricted!");
    return false;
  end
...
function GM:CanProperty(ply, property, ent)
  if allowedProperty[property] and ent:CPPICanTool(ply, "remover") then return true; end
...
function GM:PlayerDeath(ply, weapon, killer)
  ...
  if GAMEMODE.Config.deathpov then SendUserMessage("DarkRPEffects", ply, "deathPOV", "1"); end
  UnDrugPlayer(ply);
  ...
  elseif killer:IsPlayer() then
    for k, v in pairs(player.GetAll()) do
      if v:IsAdmin() or v:IsUserGroup("Moderator") then v:PrintMessage(HUD_PRINTCONSOLE, ply:Nick() .. " was killed by " .. killer:Name() .. " with " .. weapon:GetClass()); end
    end
  end
  ...
  if GAMEMODE.Config.dropmoneyondeath then
    ...
    if not ply:CanAfford(GAMEMODE.Config.deathfee) then amount = ply.DarkRPVars.money; end
    if amount > 0 then
      ply:AddMoney(-amount);
...
function GM:PlayerSetModel(ply)
  ...
  if GAMEMODE.Config.enforceplayermodel then
    ...
    if type(TEAM.model) == "table" then
      ...
      if not found then EndModel = TEAM.model[math.random(#TEAM.model)]; end
    else
      EndModel = TEAM.model;
    end
...
function GM:PlayerInitialSpawn(ply)
  ...
  for k, v in pairs(ents.GetAll()) do
    if IsValid(v) and v.deleteSteamID == ply:SteamID() and v.dt then
      v.SID = ply.SID;
      if v.Setowning_ent then v:Setowning_ent(ply); end
      v.deleteSteamID = nil;
...
local function SendDarkRPVars(ply)
  if ply.DarkRPVarsSent and ply.DarkRPVarsSent > (CurTime() - 1) then --prevent spammers
    return;
  end
  ...
  for k, v in pairs(player.GetAll()) do
    ...
    for a, b in pairs(v.DarkRPVars or {}) do
      if not (v.privateDRPVars or {})[a] or ply == v then sendtable[v][a] = b; end
    end
  end
...
function GM:PlayerSelectSpawn(ply)
  ...
  if GAMEMODE.Config.customspawns and not RPArrestedPlayers[ply:SteamID()] and CustomSpawnPos then POS = CustomSpawnPos[math.random(1, #CustomSpawnPos)]; end
  -- Spawn where died in certain cases
  if GAMEMODE.Config.strictsuicide and ply:GetTable().DeathPos then POS = ply:GetTable().DeathPos; end
  if RPArrestedPlayers[ply:SteamID()] then
  ...
  if not GAMEMODE:IsEmpty(POS) then
    ...
    if not found then POS = POS + Vector(0, 0, 70); end
  end
...
function GM:PlayerSpawn(ply)
  ...
  if not GAMEMODE.Config.showcrosshairs then ply:CrosshairDisable(); end
  ...
  if ply:Team() == TEAM_CHIEF or ply:Team() == TEAM_POLICE then GAMEMODE:SetPlayerSpeed(ply, GAMEMODE.Config.walkspeed, GAMEMODE.Config.runspeed + 10); end
  if ply:getDarkRPVar("Arrested") then GAMEMODE:SetPlayerSpeed(ply, GAMEMODE.Config.arrestspeed, GAMEMODE.Config.arrestspeed); end
  ply:Extinguish();
  if ply:GetActiveWeapon() and IsValid(ply:GetActiveWeapon()) then ply:GetActiveWeapon():Extinguish(); end
...
function GM:PlayerLoadout(ply)
  ...
  if (FAdmin and FAdmin.Access.PlayerHasPrivilege(ply, "rp_tool")) or ply:IsAdmin() then ply:Give("gmod_tool"); end
...
function GM:PlayerDisconnected(ply)
  ...
  for _, v in ipairs(ply:GetWeapons()) do
    ...
    if ply:CanCarry(wepType) then ply:AddItem(wepType, 1); end
  end
  ...
  if ply.SleepRagdoll and IsValid(ply.SleepRagdoll) then ply.SleepRagdoll:Remove(); end
  ply:UnownAll();
...
function GM:GetFallDamage(ply, flFallSpeed)
  if GetConVarNumber("mp_falldamage") == 1 then return flFallSpeed / 15; end
  return 10;
end
