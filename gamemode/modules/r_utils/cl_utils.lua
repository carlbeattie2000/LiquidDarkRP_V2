cUtils = cUtils or {};
cUtils.funcs = cUtils.funcs or {};
cUtils.config = cUtils.config or {};
...
function cUtils.funcs.PrettyComboBox(parent, x, y, w, h, defaultTxt, values, font, boxColor, boxFontColor, vBackColor, vFontColor, vBackHoverColor, vFontHoverColor, onSelect, dock, dockMargin)
  local comboBox = vgui.Create("DComboBox", parent or nil);
  comboBox:SetSize(w, h);
  comboBox:SetPos(x, y);
  comboBox:SetValue(defaultTxt);
  if dock then
    comboBox:Dock(dock);
    if dockMargin then comboBox:DockMargin(dockMargin, dockMargin, dockMargin, dockMargin); end
  end
  ...
  comboBox.OnSelect = function(self, index, value) onSelect(value); end;
  ...
  comboBox.OldOpen = comboBox.OldOpen or comboBox.OpenMenu;
...
function cUtils.funcs.PrettyTextBox(parent, defaultTxt, x, y, w, h, font, bColor, fColor, onValueChange, dock, dockMargin)
  ...
  if dock then
    textInput:Dock(TOP);
    if dockMargin then textInput:DockMargin(dockMargin, dockMargin, dockMargin, dockMargin); end
  end
...
function cUtils.funcs.createMenu(x, y, w, h, title, centered, bgColor, closeBtns)
  ...
  if centered then frame:Center(); end
