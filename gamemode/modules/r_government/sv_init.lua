...
function resetGovernmentFunds()
  R_GOVERNMENT.funds = R_GOVERNMENT.Config.DefaultGovernmentFunds or 0;
  serverDataUpdated("government_values");
end
...
function meta:addPlayerAsCandidate()
  ...
  if not self:CanAfford(entryCost) then
  ...
  if not R_GOVERNMENT.electionRunning then if canStartElection() then startElection(); end end
...
function notifyMayor(msg)
  ...
  if mayor ~= nil then mayor:LiquidChat(R_GOVERNMENT.Config.chatTag, R_GOVERNMENT.Config.chatTagColor, msg); end
end

function canStartElection()
  return tablelength(R_GOVERNMENT.candidates) >= R_GOVERNMENT.Config.VotingSettings["min_candidates"] and not R_GOVERNMENT.mayorActive;
end
...
function findWinner()
  ...
  for _, v in pairs(R_GOVERNMENT.candidates) do
    if v["votes"] == 0 and highestVotes == 0 then continue; end
...
concommand.Add("r_g_join_election", function(ply, cmd, args) ply:addPlayerAsCandidate(); end);
concommand.Add("r_g_vote", function(ply, cmd, args)
  ...
  if not targetPly or not targetPly:isCandidate() then return; end
  targetPly:addVote();
end);
...
function customTaxHook(ply, direction, customTaxMsg, amount, tax_name, customTaxPercentage)
  ...
  if taxAmount == nil then PrintConsoleTaggedMessage(" Custom tax hook could not find a valid tax value."); end
  if amount == nil or amount == 0 then PrintConsoleTaggedMessage(" Invalid amount passed to custom tax hook."); end
  ...
  if customTaxMsg ~= nil or customTaxMsg ~= "" then taxMsg = customTaxMsg; end
...
function requestUpdatedGovernmentDetails(_, ply)
  ...
  local onlinePlayers = player.GetAll();
  for _, v in ipairs(onlinePlayers) do
    if v:isDataSynced("government_values") then continue; end
...
function handlePlayerSalaryPay(ply, salary)
  ...
  if govermentPaychecksFailed > 5 then fireAllGovernmentEmployees(); end
...
function governmentSeizeIllegalEntity(owner, ply, entName, entValue)
  ...
  if IsValid(owner) then owner:LiquidChat(R_GOVERNMENT.Config.chatTag, R_GOVERNMENT.Config.chatTagColor, string.format("Your %s has been seized by the police!", entName)); end
...
function rGovernmentInit()
  ...
  timer.Simple(1, function()
    if not loadMayorCustomCheck() then status = false; end
