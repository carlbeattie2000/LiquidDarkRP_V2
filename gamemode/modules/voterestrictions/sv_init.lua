...
local function decide(ply, target)
  ...
  if GangsterJobs[ply:Team()] and GovernmentJobs[target:Team()] then return false, "Gangsters can't vote for government things!"; end
  if GovernmentJobs[ply:Team()] and GangsterJobs[target:Team()] then return false, "Government officials can't vote for gangster things!"; end
end

hook.Add("CanDemote", "VoteRegulations", decide);
local function canVote(ply, vote)
  if vote.votetype ~= "job" and vote.votetype ~= "demote" then -- only apply to promotions and demotions
    return;
  end
