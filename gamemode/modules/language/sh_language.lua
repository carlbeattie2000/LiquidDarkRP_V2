...
cvars.AddChangeCallback("gmod_language", function(cv, old, new) selectedLanguage = new end)
...
local function getMissingPhrases(ply, cmd, args)
    if not args[1] then
        print("Please run the command with a language code e.g. darkrp_getphrases \"en\"")
        return
    end

    local lang = rp_languages[args[1]]
    if not lang then
        print("This language does not exist! Make sure the casing is right.")
        print("Available languages:")
        for k in pairs(rp_languages) do
            print(k)
        end
        return
    end
...
function DarkRP.getChatCommandDescription(name)
    ...
    return chatCmdDescriptions[selectedLanguage] and chatCmdDescriptions[selectedLanguage][name] or cmd and cmd.description or nil
end
...
local function getMissingCmdTranslations()
    ...
    local count = #cmds
    for i = 1, count do
        if chatCmdDescriptions[selectedLanguage][cmds[i].command] then cmds[i] = nil end
    end
...
local function printMissingChatTranslations()
    ...
    for _, v in pairs(cmds) do
        maxCmdLength = math.Max(maxCmdLength, string.len(v.command))
    end
