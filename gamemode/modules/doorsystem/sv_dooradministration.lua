...
local function ccLock(ply, args)
    ...
    MySQLite.query(string.format([[REPLACE INTO darkrp_door VALUES(%s, %s, %s, 1, %s);]], MySQLite.SQLStr(ent:doorIndex()), MySQLite.SQLStr(string.lower(game.GetMap())), MySQLite.SQLStr(ent:getKeysTitle() or ""), ent:getKeysNonOwnable() and 1 or 0))
...
local function ccUnLock(ply, args)
    ...
    ent:keysUnLock()
    if not ent:CreatedByMap() then return end
    MySQLite.query(string.format([[REPLACE INTO darkrp_door VALUES(%s, %s, %s, 0, %s);]], MySQLite.SQLStr(ent:doorIndex()), MySQLite.SQLStr(string.lower(game.GetMap())), MySQLite.SQLStr(ent:getKeysTitle() or ""), ent:getKeysNonOwnable() and 1 or 0))
