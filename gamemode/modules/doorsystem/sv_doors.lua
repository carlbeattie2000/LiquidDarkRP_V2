...
function meta:keysLock()
    self:Fire("lock", "", 0)
    if isfunction(self.Lock) then -- SCars
        self:Lock(true)
    end

    if IsValid(self.EntOwner) and self.EntOwner ~= self then -- SCars
        return self.EntOwner:keysLock()
    end
...
function meta:keysUnLock()
    self:Fire("unlock", "", 0)
    if isfunction(self.UnLock) then -- SCars
        self:UnLock(true)
    end

    if IsValid(self.EntOwner) and self.EntOwner ~= self then -- SCars
        return self.EntOwner:keysUnLock()
    end
...
function meta:keysOwn(ply)
    ...
    if self:IsVehicle() then self:CPPISetOwner(ply) end
    ...
    if ply.OwnedNumz == 0 and GAMEMODE.Config.propertytax then timer.Create(ply:SteamID64() .. "propertytax", 270, 0, function() ply.doPropertyTax(ply) end) end
...
function pmeta:keysUnOwnAll()
    for entIndex, ent in pairs(self.Ownedz or {}) do
        if not IsValid(ent) or not ent:isKeysOwnable() then
            self.Ownedz[entIndex] = nil
            continue
        end

        if ent:isMasterOwner(self) then ent:Fire("unlock", "", 0.6) end
        ent:keysUnOwn(self)
    end
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        for _, ent in pairs(ply.Ownedz or {}) do
            if IsValid(ent) and ent:isKeysAllowedToOwn(self) then ent:removeKeysAllowedToOwn(self) end
        end
    end
...
local function taxesUnOwnAll(ply, taxables)
    for _, ent in pairs(taxables) do
        if ent:isMasterOwner(ply) then ent:Fire("unlock", "", 0.6) end
        ent:keysUnOwn(ply)
    end
end
...
local function SetDoorOwnable(ply)
    ...
    if IsValid(ent:getDoorOwner()) then ent:keysUnOwn(ent:getDoorOwner()) end
...
local function SetDoorGroupOwnable(ply, arg)
    ...
    if not RPExtraTeamDoors[arg] and arg ~= "" then
        DarkRP.notify(ply, 1, 10, DarkRP.getPhrase("door_group_doesnt_exist"))
        return ""
    end
...
local function SetDoorTeamOwnable(ply, arg)
    ...
    arg = tonumber(arg)
    if not arg then
        DarkRP.notify(ply, 1, 10, DarkRP.getPhrase("job_doesnt_exist"))
        return ""
    end

    if not RPExtraTeams[arg] and arg ~= nil then
        DarkRP.notify(ply, 1, 10, DarkRP.getPhrase("job_doesnt_exist"))
        return ""
    end
...
local function OwnDoor(ply)
    ...
    if ent:isKeysOwnedBy(ply) then
        ...
        if bAllowed == false then
            if strReason and strReason ~= "" then DarkRP.notify(ply, 1, 4, strReason) end
            return ""
        end
        ...
        if not bSuppress then DarkRP.notify(ply, 0, 4, DarkRP.getPhrase("door_sold", DarkRP.formatMoney(GiveMoneyBack))) end
    else
        ...
        ply:addMoney(-iCost)
        if not bSuppress then DarkRP.notify(ply, 0, 4, bVehicle and DarkRP.getPhrase("vehicle_bought", DarkRP.formatMoney(iCost), "") or DarkRP.getPhrase("door_bought", DarkRP.formatMoney(iCost), "")) end
        ent:keysOwn(ply)
...
local function UnOwnAll(ply, cmd, args)
    ...
    for _, otherPly in ipairs(player.GetAll()) do
        ...
        for _, ent in pairs(otherPly.Ownedz or {}) do
            if IsValid(ent) and ent:isKeysOwnedBy(ply) then table.insert(unownables, ent) end
        end
    end
    ...
    for entIndex, ent in pairs(unownables) do
        ...
        if ent:isMasterOwner(ply) then ent:Fire("unlock", "", 0) end
        ent:keysUnOwn(ply)
        amount = amount + 1
        local GiveMoneyBack = math.floor((hook.Call("get" .. (ent:IsVehicle() and "Vehicle" or "Door") .. "Cost", GAMEMODE, ply, ent) * 0.666) + 0.5)
        hook.Call("playerKeysSold", GAMEMODE, ply, ent, GiveMoneyBack)
    ...
    if amount == 0 then
        DarkRP.notify(ply, 1, 4, DarkRP.getPhrase("no_doors_owned"))
        return ""
    end
...
local function SetDoorTitle(ply, args)
    ...
    local function onCAMIResult(allowed)
        ...
        if ent:isKeysOwned() or ent:getKeysNonOwnable() or ent:getKeysDoorGroup() or hasTeams then ent:setKeysTitle(args) end
        if ent:getKeysNonOwnable() or ent:getKeysDoorGroup() or hasTeams then DarkRP.storeDoorData(trace.Entity) end
    end
...
local function RemoveDoorOwner(ply, args)
    ...
    if ent:isKeysAllowedToOwn(target) then ent:removeKeysAllowedToOwn(target) end
    if ent:isKeysOwnedBy(target) then ent:removeKeysDoorOwner(target) end
    return ""
end
