...
hook.Add("Initialize", "FSpectate", function()
    ...
    FAdmin.StartHooks["zzSpectate"] = function()
        ...
        local function calcAccess()
            CAMI.PlayerHasAccess(LocalPlayer(), "FSpectate", function(b, _) canSpectate = b end)
        end
...
startFreeRoam = function()
    ...
    if IsValid(specEnt) then
        if specEnt:IsPlayer() then roamPos = thirdperson and getThirdPersonPos(specEnt) or specEnt:GetShootPos() end
