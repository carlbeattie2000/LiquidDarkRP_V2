...
local function clearInvalidSpectators()
    for ply, _ in pairs(FSpectating) do
        if not IsValid(ply) then FSpectating[ply] = nil end
    end
end
...
local function startSpectating(ply, target)
    ...
    if IsValid(ply.FSpectatingEnt) then net.WriteEntity(ply.FSpectatingEnt) end
    net.Send(ply)
...
local function SpectateVisibility(ply, viewEnt)
    ...
    if IsValid(ply.FSpectatingEnt) then AddOriginToPVS(ply.FSpectatingEnt:IsPlayer() and ply.FSpectatingEnt:GetShootPos() or ply.FSpectatingEnt:GetPos()) end
    if ply.FSpectatePos then AddOriginToPVS(ply.FSpectatePos) end
end
...
local function fixAdminModIncompat()
    if ULib then ULib.removeSayCommand("!spectate") end
    if serverguard then serverguard.command:Remove("spectate") end
end
