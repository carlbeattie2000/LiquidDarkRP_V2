...
local function onDBInitialized()
    ...
    MySQLite.query("SELECT * FROM darkrp_position NATURAL JOIN darkrp_jobspawn WHERE map = " .. map .. ";", function(data) teamSpawns = data or {} end)
...
function DarkRP.retrieveJailPos(index)
    ...
    if index then return jailPos[index] end
...
function DarkRP.storeTeamSpawnPos(t, pos)
    ...
    DarkRP.removeTeamSpawnPos(t, function()
        MySQLite.query([[INSERT INTO darkrp_position(map, type, x, y, z) VALUES(]] .. MySQLite.SQLStr(map) .. [[, "T", ]] .. pos[1] .. [[, ]] .. pos[2] .. [[, ]] .. pos[3] .. [[);]], function()
...
function DarkRP.removeTeamSpawnPos(t, callback)
    ...
    for k, v in pairs(teamSpawns) do
        if v.teamcmd == teamcmd then teamSpawns[k] = nil end
    end
