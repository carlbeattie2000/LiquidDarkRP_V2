...
local function CreateTutorialPoint(ply, cmd, args)
  ...
  function titleValue:OnTextChanged()
    ...
    if #txt > C_MAXTITLECHAR then
      ...
      timer.Simple(2, function() titleChars:SetTextColor(self:GetSkin().text_normal); end);
  ...
  function descValue:OnTextChanged()
    ...
    if #txt > C_MAXDESCCHAR then
      ...
      timer.Simple(2, function() descChars:SetTextColor(self:GetSkin().text_normal); end);
      local newTxt = string.sub(txt, 1, #txt - 1);
      self:SetText(newTxt);
      self:SetValue(newTxt);
      self:SetCaretPos(#newTxt);
  ...
  promptText:SetText("You are about to create a new tutorial point with" .. " the current settings:\nPosition: " .. tostring(camPos) .. "\nAngle: " .. tostring(camAng) .. "\nMap: " .. game.GetMap() .. "\n\nDo you want to create a new point?");
...
local function DeleteTutPoint()
  if not LocalPlayer().InTut then
    LocalPlayer():ChatPrint("You must be viewing a tutorial point in order to delete it!");
    return false;
  end
...
function LDRP.DoTutorial()
  ...
  timer.Simple(3, function()
    if not LocalPlayer().InTut then return; end
    gui.EnableScreenClicker(true);
  end);
  ...
  NextButton.OnCursorEntered = function() CurColor = Color(150, 150, 150, 150); end;
  NextButton.OnCursorExited = function() CurColor = Color(50, 50, 50, 150); end;
  ...
  BackButton:SetText("");
  local CurColor = Color(50, 50, 50, 150);
  BackButton.OnCursorEntered = function() CurColor = Color(150, 150, 150, 150); end;
  BackButton.OnCursorExited = function() CurColor = Color(50, 50, 50, 150); end;
  ...
  BackButton.DoClick = function()
    if LDRP.CurrentPart == 1 then
      LocalPlayer():ChatPrint("You are already at the start of the tutorial!");
      return;
    end
  ...
  NextButton.DoClick = function()
    if not HasP then HasP = true; end
    LDRP.CurrentPart = LDRP.CurrentPart + 1;
    Change = CurTime();
  end;
  ...
  function LDRP.TutorialHUD()
    if LDRP.CurrentPart <= #LDRP.TutorialParts then
      ...
      if not HasP then draw.SimpleTextOutlined("Use the arrow buttons on the left and right to go through the tutorial", "Trebuchet24", ScrW() * .5, ScrH() * .85, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 255)); end
