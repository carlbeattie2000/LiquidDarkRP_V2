...
function LDRP.RepeatTut(ply, cmd, args)
  ...
  if not tutLady or ply:GetPos():Distance(tutLady) > 300 then
    print("Not running");
    return;
  end
...
function LDRP.FinishTut(ply, cmd, args)
  if ply.InTut then ply:DoTutorial("done"); end
end
...
hook.Add("PlayerInitialSpawn", "SendTutToNewPlayer", function(ply) SendTutPartsToPlayer(ply); end);
--[[---------------------------------------------------------------------------
...
local function LoadTutorialsFromDB(result)
  ...
  if type(result) ~= "table" then
    GAMEMODE:WriteOut("Received unexpected result when querying tutorials: " .. type(restult) .. ": " .. tostring(result), Severity.Info);
    return;
  end
  ...
  GAMEMODE:WriteOut("Broadcasting updated master tutorial point table to all players...", Severity.Debug);
...
hook.Add("DarkRPDBInitialized", "Load tutorials from DB", function() InitLoadTutsFromDB(); end);
...
net.Receive("SaveTutorialPoint", function(length, ply)
  ...
  for k, v in pairs(tbl) do
    if type(v) == "string" then tbl[k] = SQLStr(v); end
  end
...
net.Receive("DeleteTutPoint", function(length, ply)
  ...
  DB:DeleteEntry("tutorials", "title = " .. MySQLite.SQLStr(tutName), function(result) InitLoadTutsFromDB(); end);
end);
