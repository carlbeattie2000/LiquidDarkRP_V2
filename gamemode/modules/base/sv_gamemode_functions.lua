...
function GM:calculateNPCKillPay(ply, npc)
    ...
    if npc.KillValue then return npc.KillValue end
...
function GM:PlayerSpawnProp(ply, model)
    ...
    model = string.gsub(tostring(model), "//", "/")
    local jobTable = ply:getJobTable()
    if jobTable.PlayerSpawnProp then jobTable.PlayerSpawnProp(ply, model) end
...
function GM:PlayerSpawnedProp(ply, model, ent)
    ...
    if IsValid(phys) then ent.RPOriginalMass = phys:GetMass() end
...
local function canSpawnWeapon(ply)
    if (GAMEMODE.Config.adminweapons == 0 and ply:IsAdmin()) or (GAMEMODE.Config.adminweapons == 1 and ply:IsSuperAdmin()) or (GAMEMODE.Config.adminweapons == 3) then -- Can't use 2 to maintain compatibility
        return true
    end
...
function GM:OnNPCKilled(victim, ent, weapon)
    ...
    if not ent:IsPlayer() then ent = Player(tonumber(ent.SID) or 0) end
    ...
    for _, ply1 in ipairs(players) do
        ...
        for i = 0, 3 do
            ...
            for _, ply2 in ipairs(cell) do
                local canTalk = ply1Pos:DistToSqr(plyPos[ply2]) < voiceDistance and (not dynv or IsInRoom(
                    -- voiceradius is on and the two are within hearing distance
                    ply1EyePos, eyePos[ply2], ply2))

                -- Dynamic voice is on and players are in the same room
    ...
    for _, row in pairs(grid) do
        for _, cell in pairs(row) do
            ...
            for i = 1, count do
                ...
                for j = i + 1, count do
                    local ply2 = cell[j]
                    local canTalk = plyPos[ply1]:DistToSqr(plyPos[ply2]) < voiceDistance and (not dynv or IsInRoom(
                        -- voiceradius is on and the two are within hearing distance
                        eyePos[ply1], eyePos[ply2], ply2))
...
function GM:CanTool(ply, trace, mode)
    ...
    if IsValid(ent) then
        ...
        if ent.nodupe and (mode == "weld" or mode == "weld_ez" or mode == "spawner" or mode == "duplicator" or mode == "adv_duplicator") then return false end
        if ent:IsVehicle() and mode == "nocollide" and not GAMEMODE.Config.allowvnocollide then return false end
    end
    return true
end
...
function GM:DoPlayerDeath(ply, attacker, dmginfo, ...)
    ...
    if (GAMEMODE.Config.dropweapondeath or ply.dropWeaponOnDeath) and weapon:IsValid() and canDrop then ply:dropDRPWeapon(weapon) end
    self.Sandbox.DoPlayerDeath(self, ply, attacker, dmginfo, ...)
end

function GM:PlayerDeath(ply, weapon, killer)
    local jobTable = ply:getJobTable()
    if jobTable.PlayerDeath then jobTable.PlayerDeath(ply, weapon, killer) end
    ...
    if GAMEMODE.Config.showdeaths then self.Sandbox.PlayerDeath(self, ply, weapon, killer) end
    ply:Extinguish()
    ply:ExitVehicle()
    ...
    if GAMEMODE.Config.dropmoneyondeath then
        ...
        if not ply:canAfford(GAMEMODE.Config.deathfee) then amount = ply:getDarkRPVar("money") end
        if amount > 0 then
            ply:addMoney(-amount)
    ...
    if IsValid(ply) and (ply ~= killer or ply.Slayed) and not ply:isArrested() then
        if not GAMEMODE.Config.wantedrespawn then ply:setDarkRPVar("wanted", nil) end
        ply.DeathPos = nil
        ply.Slayed = false
    end
...
function GM:PlayerCanPickupWeapon(ply, weapon)
    ...
    if GAMEMODE.Config.license and not ply:getDarkRPVar("HasGunlicense") and not ply.RPLicenseSpawn then
        if GAMEMODE.NoLicense[string.lower(weaponClass)] or not weapon:IsWeapon() then return true end
        return false
    end
    return true
end
...
function GM:PlayerSetModel(ply)
    ...
    if GAMEMODE.Config.enforceplayermodel then
        if istable(jobTable.model) then
            ...
            if not found then EndModel = jobTable.model[math.random(#jobTable.model)] end
        else
...
local function restoreReconnectedEnts(ply)
    ...
    for _, e in pairs(queuedForRemoval[sid]) do
        ...
        e.SID = ply.SID
        if e.Setowning_ent then e:Setowning_ent(ply) end
        -- Some entities (e.g. vehicles) have an SID, but do not have a DarkRPItem
        if e.DarkRPItem then ply:addCustomEntity(e.DarkRPItem) end
    end
...
function GM:PlayerInitialSpawn(ply)
    ...
    timer.Simple(1, function()
        ...
        if group then ply:SetUserGroup(group) end
    end)
...
function GM:PlayerSelectSpawn(ply)
    ...
    local jobTable = ply:getJobTable()
    if jobTable.PlayerSelectSpawn then jobTable.PlayerSelectSpawn(ply, spawn) end
    local POS
    ...
    if GAMEMODE.Config.customspawns and not ply:isArrested() and CustomSpawnPos and next(CustomSpawnPos) ~= nil then POS = CustomSpawnPos[math.random(1, #CustomSpawnPos)] end
    -- Spawn where died in certain cases
    if GAMEMODE.Config.strictsuicide and ply.DeathPos then POS = ply.DeathPos end
    if ply:isArrested() then
...
function GM:PlayerSpawn(ply)
    if not ply.DarkRPInitialised then DarkRP.errorNoHalt(string.format("DarkRP was unable to introduce player \"%s\" to the game. Expect further errors and shit generally being fucked!", IsValid(ply) and ply:Nick() or "unknown"), 1, {"This error most likely does not stand on its own, and previous serverside errors have a very good chance of telling you the cause.", "Note that errors from another addon could cause this. Specifically when they're thrown during 'PlayerInitialSpawn'.", "This error can also be caused by some other addon returning a value in 'PlayerInitialSpawn', though that is less likely.", "Errors in your DarkRP configuration (jobs, shipments, etc.) could also cause this. Earlier errors should tell you when this is the case."}) end
    ply:CrosshairEnable()
    ply:UnSpectate()
    -- Kill any colormod
    ...
    if GAMEMODE.Config.babygod and not ply.IsSleeping and not ply.Babygod then enableBabyGod(ply) end
    ply.IsSleeping = false
    ply:Extinguish()
    for i = 0, 2 do
        local vm = ply:GetViewModel(i)
        if IsValid(vm) then vm:Extinguish() end
    end
    ...
    if jobTable.PlayerSpawn then jobTable.PlayerSpawn(ply) end
...
function GM:PlayerDisconnected(ply)
    ...
    if isMayor and GAMEMODE.Config.shouldResetLaws then DarkRP.resetLaws() end
    if IsValid(ply.SleepRagdoll) then ply.SleepRagdoll:Remove() end
    ply:keysUnOwnAll()
    ...
    local jobTable = ply:getJobTable()
    if jobTable.PlayerDisconnected then jobTable.PlayerDisconnected(ply) end
end
...
timer.Simple(0.1, function() if not GAMEMODE.InitPostEntityCalled then GAMEMODE:InitPostEntity() end end)
...
function GM:PlayerLeaveVehicle(ply, vehicle)
    if GAMEMODE.Config.autovehiclelock and vehicle:isKeysOwnedBy(ply) then vehicle:keysLock() end
