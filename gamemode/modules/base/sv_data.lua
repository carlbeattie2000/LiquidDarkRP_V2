...
function DarkRP.retrieveRPNames(name, callback)
    MySQLite.query("SELECT COUNT(*) AS count FROM darkrp_player WHERE rpname = " .. MySQLite.SQLStr(name) .. ";", function(r) callback(tonumber(r[1].count) > 0) end)
end
...
function DarkRP.storeMoney(ply, amount)
    if not isnumber(amount) or amount < 0 or amount >= 1 / 0 then
        DarkRP.errorNoHalt("Some addon attempted to store a invalid money amount " .. tostring(amount) .. " for Player " .. ply:Nick() .. " (" .. ply:SteamID() .. ")", 1, {"This money amount will not be stored in the database, but it may be set in the game.", "The database simply stores the last valid, non-negative amount of money.", "Please try to find the very first time this error happened for this player. Then look at the files mentioned in this error.", "That will tell you which addon is causing this.", "IMPORTANT: This is NOT a DarkRP bug!", "Note: The player can simply rejoin to fix their negative money, until whatever causes this happens again."})
        return
    end
...
function DarkRP.storeOfflineMoney(sid64, amount)
    ...
    if not isnumber(amount) or amount < 0 or amount >= 1 / 0 then
        DarkRP.errorNoHalt("Some addon attempted to store a invalid money amount " .. tostring(amount) .. " for an offline player with steamID64 " .. sid64, 1, {"This money amount will not be stored in the database.", "Please try to find the very first time this error happened for this player. Then look at the files mentioned in this error.", "That will tell you which addon is causing this.", "IMPORTANT: This is NOT a DarkRP bug!"})
        return
    end
...
function meta:restorePlayerData()
    ...
    DarkRP.retrievePlayerData(self, function(data)
        ...
        -- Retrieving data failed, go on without it
        if not IsValid(self) then return end
    ...
    MySQLite.query("SELECT idx, title, isLocked, isDisabled FROM darkrp_door WHERE map = " .. MySQLite.SQLStr(string.lower(game.GetMap())) .. ";", function(r)
        ...
        for _, row in pairs(r) do
            ...
            if e:isKeysOwnable() then
                if tobool(row.isDisabled) then e:setKeysNonOwnable(tobool(row.isDisabled)) end
                if row.isLocked and row.isLocked ~= "NULL" then e:Fire((tobool(row.isLocked) and "" or "un") .. "lock", "", 0) end
...
local function onKeyValue(ent, key, value)
    ...
    if keyValueActions[key] then keyValueActions[key](ent, value) end
end
