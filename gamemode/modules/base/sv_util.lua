...
function DarkRP.talkToRange(ply, PlayerName, Message, size)
    ...
    for _, v in ipairs(ents) do
        if (v:EyePos():DistToSqr(plyPos) <= sizeSqr) and (v == ply or hook.Run("PlayerCanSeePlayersChat", PlayerName .. ": " .. Message, false, v, ply) ~= false) then table.insert(filter, v) end
    end

    if PlayerName == ply:Nick() then -- If it's just normal chat, why not cut down on networking and get the name on the client
        PlayerName = ""
    end
...
function meta:applyPlayerClassVars(applyHealth)
    ...
    if applyHealth then self:SetHealth(playerClass.StartHealth >= 0 and playerClass.StartHealth or (tonumber(GAMEMODE.Config.startinghealth) or 100)) end
