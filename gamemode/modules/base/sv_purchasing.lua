...
local function BuyPistol(ply, args)
    ...
    if shipment.onBought then shipment.onBought(ply, shipment, weapon) end
...
function DarkRP.hooks:canBuyShipment(ply, shipment)
    ...
    if not ply:canAfford(cost) then return false, false, DarkRP.getPhrase("cant_afford", DarkRP.getPhrase("shipment")) end
    if not shipment.allowPurchaseWhileDead and not ply:Alive() then return false, false, DarkRP.getPhrase("must_be_alive_to_do_x", DarkRP.getPhrase("buy_x", DarkRP.getPhrase("shipments"))) end
    return true
end
...
local function BuyShipment(ply, args)
    ...
    if CustomShipments[foundKey].onBought then CustomShipments[foundKey].onBought(ply, CustomShipments[foundKey], crate) end
...
function DarkRP.hooks:canBuyVehicle(ply, vehicle)
    if not vehicle.allowPurchaseWhileDead and not ply:Alive() then return false, false, DarkRP.getPhrase("must_be_alive_to_do_x", DarkRP.getPhrase("buy_x", vehicle.name)) end
    if not GAMEMODE:CustomObjFitsMap(vehicle) then return false, false, "Custom object does not fit map" end
    if ply:isArrested() then return false, false, DarkRP.getPhrase("unable", "/buyvehicle", "") end
    ...
    if GAMEMODE.Config.maxvehicles and ply.Vehicles >= GAMEMODE.Config.maxvehicles then return false, false, DarkRP.getPhrase("limit", DarkRP.getPhrase("vehicle")) end
    local cost = vehicle.getPrice and vehicle.getPrice(ply, vehicle.price) or vehicle.price
    if not ply:canAfford(cost) then return false, false, DarkRP.getPhrase("cant_afford", DarkRP.getPhrase("vehicle")) end
    return true
end
...
local function BuyVehicle(ply, args)
    ...
    if found.onBought then found.onBought(ply, found, ent) end
    return ""
end
...
function DarkRP.hooks:canBuyAmmo(ply, ammo)
    ...
    if not ply:canAfford(cost) then return false, false, DarkRP.getPhrase("cant_afford", DarkRP.getPhrase("ammo")) end
    return true
end
