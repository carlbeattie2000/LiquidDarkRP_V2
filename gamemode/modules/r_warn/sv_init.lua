...
function meta:playerHasPerms()
  ...
  for k, v in pairs(aloudGroups) do
    if self:IsUserGroup(k) then aloud = true; end
  end
  return aloud;
end

function meta:warnsPunishment()
  local playerWarns = findPlayersWarnsSQL(self:SteamID()) or {};
  local activeWarns = 0;
...
function sendHWarnMenuOpenMessage(ply)
  if not ply:playerHasPerms() then return; end
...
function newWarnSQL(warner, steam_id, reason)
  ...
  sql.Query("INSERT INTO r_warns (steam_id, reason, warn_time, active, warner_steam_id, warner_nick) VALUES (" .. sql.SQLStr(steam_id) .. "," .. sql.SQLStr(reason) .. "," .. sql.SQLStr(currentTimeMilliseconds) .. "," .. sql.SQLStr(1) .. "," .. sql.SQLStr(warner:SteamID()) .. "," .. sql.SQLStr(warner:Nick()) .. ")");
end
...
function warnPlayer(warner, ply, reason)
  ...
  for i, v in ipairs(playersOnline) do
    ...
    if v:Nick() == ply:Nick() then message = playerWarnedChatString; end
...
function getPlayerWarnsCommands(ply, cmd, args)
  ...
  net.WriteTable(playersWarns or {});
  net.Send(ply);
end
...
function checkIfPlayersWarnsHaveExpired()
  ...
  for i, ply in ipairs(players) do
    ...
    for i1, warn in ipairs(playerWarns) do
      ...
      if timeDifference > R_WARN_CONFIG["warns_decay_seconds"] then sql.Query("UPDATE r_warns SET active = " .. sql.SQLStr(0) .. " WHERE id = " .. sql.SQLStr(warn["id"]) .. ";"); end
    end
  end
end
