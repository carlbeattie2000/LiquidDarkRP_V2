...
function writeEvent(content)
  ...
  if tonumber(todayDate["hour"]) < 10 then todayDate["hour"] = "0" .. tostring(todayDate["hour"]); end
  if tonumber(todayDate["min"]) < 10 then todayDate["min"] = "0" .. tostring(todayDate["min"]); end
  if tonumber(todayDate["sec"] < 10) then todayDate["sec"] = "0" .. tostring(todayDate["sec"]); end
  ...
  if not file.Exists("p_logger/", "DATA") then file.CreateDir("p_logger/"); end
...
hook.Add("governmentPlayerTaxesChanged", "logGTaxesChanged", function(newValues)
  local loggingText = string.format("{player_tax: %f} \n {sales_tax: %f} \n {trading_tax: %d}", newValues["player_tax"], newValues["sales_tax"], newValues["trading_tax"]);
  writeEvent(loggingText);
end);
...
hook.Add("playerFamilySharing", "logAFamilySharing", function(ply) if IsValid(ply) then writeEvent(string.format("%s is family sharing", ply:Nick())); end end);
hook.Add("playerUsingVPN", "logAUsingVPN", function(ply) if IsValid(ply) then writeEvent(string.format("%s is using a vpn", ply:Nick())); end end);
hook.Add("playerVPNCheckingFailed", "logAVPNCheckFailed", function(ply) if IsValid(ply) then writeEvent(string.format("VPN Checking for %s failed", ply:Nick())); end end);
