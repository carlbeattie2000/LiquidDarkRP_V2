...
local function onRagdollArrested(arrestee, _, arrester)
    ...
    if arrester.SteamName then DarkRP.log(arrester:Nick() .. " (" .. arrester:SteamID() .. ") arrested " .. arrestee:Nick(), Color(0, 255, 255)) end
end

local hookCanSleep = {
    canSleep = function(_, ply, force)
...
function DarkRP.toggleSleep(player, command)
    ...
    if not player.SleepSound then player.SleepSound = CreateSound(player, "npc/ichthyosaur/water_breath.wav") end
    ...
    if player.Sleeping and IsValid(player.SleepRagdoll) then
        ...
        if string.lower(model) == "models/humans/corpse1.mdl" then model = "models/player/corpse1.mdl" end
        ...
        if player.WeaponsForSleep and player:GetTable().BeforeSleepTeam == player:Team() then
            ...
            if player:HasWeapon(cl_defaultweapon) then player:SelectWeapon(cl_defaultweapon) end
        ...
        if command == true then player:arrest() end
        ...
        if player:isArrested() then GAMEMODE:SetPlayerSpeed(player, GAMEMODE.Config.arrestspeed, GAMEMODE.Config.arrestspeed) end
        ...
        if not player:isArrested() then
            ...
            for k, v in ipairs(player:GetWeapons()) do
                player.WeaponsForSleep[k] = {v:GetClass(), player:GetAmmoCount(v:GetPrimaryAmmoType()), v:GetPrimaryAmmoType(), player:GetAmmoCount(v:GetSecondaryAmmoType()), v:GetSecondaryAmmoType(), v:Clip1(), v:Clip2()}
