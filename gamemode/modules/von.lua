...
function d_findVariable(s, i, len, lastType)
  ...
  while true do
    ...
    if i > len then error("vON: Reached end of string, cannot form proper variable."); end
...
_deserialize = {
  ...
  ["Vector"] = function(s, i, len)
    ...
    a = find(s, "[;:}~]", i);
    if a then z = tonumber(sub(s, i, a - 1)); end
  ...
  ["Angle"] = function(s, i, len)
    ...
    a = find(s, "[;:}~]", i);
    if a then r = tonumber(sub(s, i, a - 1)); end
...
_serialize = {
  ...
  ["table"] = function(data, mustInitiate, isNumeric, isKey, isLast, first)
    ...
    for k, v in next, data do
      ...
      if type(k) ~= "number" or k < 1 or k > len then keyvals[#keyvals + 1] = k; end
    end
