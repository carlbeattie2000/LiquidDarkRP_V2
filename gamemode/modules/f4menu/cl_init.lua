...
function DarkRP.closeF4Menu()
    if f4Frame then f4Frame:Hide() end
end
...
function DarkRP.hooks.F4MenuTabs()
    ...
    if not table.IsEmpty(shipments) then DarkRP.addF4MenuTab(DarkRP.getPhrase("Shipments"), vgui.Create("F4MenuShipments")) end
    local guns = fn.Filter(fn.Curry(fn.GetValue, 2)("separate"), CustomShipments)
    if not table.IsEmpty(guns) then DarkRP.addF4MenuTab(DarkRP.getPhrase("F4guns"), vgui.Create("F4MenuGuns")) end
    if not table.IsEmpty(GAMEMODE.AmmoTypes) then DarkRP.addF4MenuTab(DarkRP.getPhrase("F4ammo"), vgui.Create("F4MenuAmmo")) end
    if not table.IsEmpty(CustomVehicles) then DarkRP.addF4MenuTab(DarkRP.getPhrase("F4vehicles"), vgui.Create("F4MenuVehicles")) end
end
