...
hook.Add("InitPostEntity", "DarkRP_SetupTremor", function() createTremor() end)
...
local function EarthQuakeTest()
    ...
    if GAMEMODE.Config.quakechance and math.random(0, GAMEMODE.Config.quakechance) < 1 then
        ...
        for _, e in ipairs(en) do
            ...
            if e:IsOnGround() then e:TakeDamage((force / 100) + 15, game.GetWorld()) end
        end
    end
end
...
for k in pairs(flammablePropsKV) do
    table.insert(flammableProps, k)
end
...
local function FireSpread(ent, chanceDiv)
    ...
    if ent:isMoneyBag() then ent:Remove() end
