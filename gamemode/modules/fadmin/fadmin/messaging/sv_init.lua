...
receiversToPlayers = {
    ...
    involved = function(i, t)
        local res = table.Copy(istable(t) and t or {t})
        table.insert(res, i)
        return res
    end,
...
function FAdmin.Messages.FireNotification(name, instigator, targets, extraInfo)
    ...
    if not notId then error(string.format("Notification '%s' does not exist!", name), 2) end
    ...
    net.Send(receivers)
    if notification.logging then logNotification(notification, instigator, targets, extraInfo) end
end
