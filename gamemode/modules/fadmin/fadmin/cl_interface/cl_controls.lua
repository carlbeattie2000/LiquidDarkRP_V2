...
function PANEL:SizeToContents()
    ...
    if #self:GetChildren() == 1 then h = math.Max(0, 120) end
    self:SetSize(w, h)
    self:PerformLayout()
end
...
function PANEL3:Paint()
    if self.CatagoryColor then draw.RoundedBox(4, 0, 0, self:GetWide(), self.Header:GetTall(), self.CatagoryColor) end
end
...
function PANEL:Paint()
    ...
    if GAMEMODE.Name == "Sandbox" then
        ...
        if self.Player:GetFriendStatus() == "friend" then color = Color(236, 181, 113, 255) end
    else
    ...
    if self.Player == LocalPlayer() or self.Player:GetFriendStatus() == "friend" then surface.SetDrawColor(255, 255, 255, 50 + math.sin(RealTime() * 2) * 50) end
...
function PANEL6:Paint()
    ...
    if self.Hovered then BorderColor = Color(math.Min(BorderColor.r + 40, 255), math.Min(BorderColor.g + 40, 255), math.Min(BorderColor.b + 40, 255), BorderColor.a) end
    if self.Depressed then BorderColor = color_transparent end
