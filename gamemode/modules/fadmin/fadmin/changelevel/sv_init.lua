...
local function ChangeLevel(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "changelevel") then
    ...
    if gameMode then RunConsoleCommand("gamemode", gameMode) end
...
local mapNames = {}
local mapPatterns = {}
local ignorePatterns = {"^asi-", "^background", "^c[%d]m", "^devtest", "^ep1_background", "^ep2_background", "^mp_coop_", "^sp_a", "^styleguide"}
local ignoreMaps = {
    -- Prefixes
    ["ddd_"] = true,
    ["sdk_"] = true,
...
hook.Add("PlayerInitialSpawn", "FAdmin_ChangelevelInfo", function(ply)
    ...
    for _, v in ipairs(maps) do
        ...
        for _, ignore in ipairs(ignorePatterns) do
            if string.find(name, ignore) then goto mapContinue end
        end
        ...
        if not mapCategory then
            for pattern, category in pairs(mapPatterns) do
                if string.find(name, pattern) then mapCategory = category end
            end
        end
        ...
        if mapCategory == "Counter-Strike" and not file.Exists("maps/" .. name .. ".bsp", "cstrike") then continue end
        if not mapList[mapCategory] then mapList[mapCategory] = {} end
