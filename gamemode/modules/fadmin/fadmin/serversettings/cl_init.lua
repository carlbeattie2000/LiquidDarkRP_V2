    ...
    for _, v in SortedPairs(Settings.settings or {}) do
        if v.type == "Numeric" then
            ...
            function left:OnValueChanged(val)
                if val == GetConVar(v.name):GetFloat() then return end
...
FAdmin.StartHooks["ServerSettings"] = function()
    ...
    FAdmin.ScoreBoard.Server:AddServerSetting(function() return (sbox_godmode:GetBool() and "Disable" or "Enable") .. " global god mode" end, function() return "fadmin/icons/god", sbox_godmode:GetBool() and "fadmin/icons/disable" end, Color(0, 0, 155, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(ply, "ServerSetting") end, function(button)
    ...
    FAdmin.ScoreBoard.Server:AddServerSetting(function() return (sbox_playershurtplayers:GetBool() and "Disable" or "Enable") .. " player vs player damage" end, function() return "fadmin/icons/weapon", sbox_playershurtplayers:GetBool() and "fadmin/icons/disable" end, Color(0, 0, 155, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(ply, "ServerSetting") end, function(button)
    ...
    FAdmin.ScoreBoard.Server:AddServerSetting(function() return (sbox_noclip:GetBool() and "Disable" or "Enable") .. " global noclip" end, function() return "fadmin/icons/noclip", sbox_noclip:GetBool() and "fadmin/icons/disable" end, Color(0, 0, 155, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(ply, "ServerSetting") end, function(button)
