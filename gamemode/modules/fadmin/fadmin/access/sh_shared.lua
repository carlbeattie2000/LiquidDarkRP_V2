...
function FAdmin.Access.AddGroup(name, admin_access, privs, immunity, fromCAMI, CAMIsrc) --[[0 = not admin, 1 = admin, 2 = superadmin]]
    FAdmin.Access.Groups[name] = FAdmin.Access.Groups[name] or {
        ADMIN = admin_access,
        PRIVS = privs or {},
        immunity = immunity
    }
    ...
    if not isstring(CAMIsrc) then CAMIsrc = "" end
    ...
    MySQLite.queryValue("SELECT COUNT(*) FROM FADMIN_GROUPS WHERE NAME = " .. MySQLite.SQLStr(name) .. ";", function(val)
        ...
        MySQLite.query("REPLACE INTO FADMIN_GROUPS VALUES(" .. MySQLite.SQLStr(name) .. ", " .. tonumber(admin_access) .. ");", function()
            ...
            if fromCAMI then MySQLite.query("REPLACE INTO FADMIN_GROUPS_SRC VALUES(" .. MySQLite.SQLStr(name) .. ", " .. MySQLite.SQLStr(CAMIsrc) .. ");") end
        end)
    end)

    if immunity then MySQLite.query("REPLACE INTO FAdmin_Immunity VALUES(" .. MySQLite.SQLStr(name) .. ", " .. tonumber(immunity) .. ");") end
...
function FAdmin.Access.RemoveGroup(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "ManageGroups") then
...
function PLAYER:IsAdmin(...)
    ...
    if (FAdmin.Access.Groups[usergroup] and FAdmin.Access.Groups[usergroup].ADMIN >= 1) or (self.IsListenServerHost and self:IsListenServerHost()) then --[[1 = admin]]
        return true
    end
...
function PLAYER:IsSuperAdmin(...)
    ...
    if (FAdmin.Access.Groups[usergroup] and FAdmin.Access.Groups[usergroup].ADMIN >= 2) or (self.IsListenServerHost and self:IsListenServerHost()) then --[[2 = superadmin]]
        return true
    end
...
hook.Add("CAMI.OnPrivilegeUnregistered", "FAdmin", function(privilege) FAdmin.Access.Privileges[privilege.Name] = nil end)
...
function FAdmin.Access.PlayerHasPrivilege(ply, priv, target, ignoreImmunity)
    ...
    if canTarget ~= nil then return canTarget end
    if FAdmin.GlobalSetting.Immunity and not ignoreImmunity and not isstring(target) and IsValid(target) and target ~= ply and FAdmin.Access.Groups[Usergroup] and FAdmin.Access.Groups[target:GetUserGroup()] and FAdmin.Access.Groups[Usergroup].immunity and FAdmin.Access.Groups[target:GetUserGroup()].immunity and FAdmin.Access.Groups[target:GetUserGroup()].immunity >= FAdmin.Access.Groups[Usergroup].immunity then return false end
    -- Defer answer when usergroup is unknown
    if not FAdmin.Access.Groups[Usergroup] then return end
    if FAdmin.Access.Groups[Usergroup].PRIVS[priv] then return true end
...
hook.Add("CAMI.SteamIDHasAccess", "FAdmin", function(actorSteam, privilegeName, callback, targetSteam, extraInfo)
    ...
    if not targetSteam or extraInfo and extraInfo.IgnoreImmunity then
        MySQLite.query(string.format([[SELECT COUNT(*) AS c
            FROM FAdmin_PlayerGroup l
            JOIN FADMIN_PRIVILEGES r ON l.groupname = r.NAME
            WHERE l.steamid = %s AND r.PRIVILEGE = %s]], MySQLite.SQLStr(actorSteam), MySQLite.SQLStr(privilegeName)), function(res) callback(tonumber(res[1].c) > 0) end)
        return true
    end

    MySQLite.query(string.format([[SELECT ll.i AND rr.c AS res
        ...
        JOIN (SELECT COUNT(*) AS c
            FROM FAdmin_PlayerGroup l
            JOIN FADMIN_PRIVILEGES r ON l.groupname = r.NAME
            WHERE l.steamid = %s AND r.PRIVILEGE = %s) AS rr]], MySQLite.SQLStr(actorSteam), MySQLite.SQLStr(targetSteam), MySQLite.SQLStr(actorSteam), MySQLite.SQLStr(privilegeName)), function(res) callback(res and res[1] and tobool(res[1].res) or false) end)
    return true
end)
