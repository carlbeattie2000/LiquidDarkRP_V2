        ...
        MySQLite.query("SELECT g.NAME, g.ADMIN_ACCESS, p.PRIVILEGE, i.immunity, s.src FROM FADMIN_GROUPS g LEFT OUTER JOIN FADMIN_PRIVILEGES p ON g.NAME = p.NAME LEFT OUTER JOIN FAdmin_Immunity i ON g.NAME = i.groupname LEFT OUTER JOIN FADMIN_GROUPS_SRC s ON g.NAME = s.NAME;", function(data)
            ...
            for _, v in pairs(data) do
                FAdmin.Access.Groups[v.NAME] = FAdmin.Access.Groups[v.NAME] or {
                    ADMIN = tonumber(v.ADMIN_ACCESS),
                    PRIVS = {}
                }

                if v.PRIVILEGE and v.PRIVILEGE ~= "NULL" then FAdmin.Access.Groups[v.NAME].PRIVS[v.PRIVILEGE] = true end
                if v.immunity and v.immunity ~= "NULL" then FAdmin.Access.Groups[v.NAME].immunity = tonumber(v.immunity) end
...
function FAdmin.Access.PlayerSetGroup(ply, group)
    ...
    if not isstring(ply) and IsValid(ply) then ply:SetUserGroup(group) end
end
...
hook.Add("CAMI.PlayerUsergroupChanged", "FAdmin", function(ply, old, new, source) MySQLite.query("REPLACE INTO FAdmin_PlayerGroup VALUES(" .. MySQLite.SQLStr(ply:SteamID()) .. ", " .. MySQLite.SQLStr(new) .. ");") end)
hook.Add("CAMI.SteamIDUsergroupChanged", "FAdmin", function(steamId, old, new, source) MySQLite.query("REPLACE INTO FAdmin_PlayerGroup VALUES(" .. MySQLite.SQLStr(steamId) .. ", " .. MySQLite.SQLStr(new) .. ");") end)
...
local function AddGroup(ply, cmd, args)
    ...
    if not args[1] or not admin then
        FAdmin.Messages.SendMessage(ply, 5, "Incorrect arguments!")
        return false
    end
...
local function AddPrivilege(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "ManagePrivileges") then
        FAdmin.Messages.SendMessage(ply, 5, "No access!")
        return false
    end
...
function FAdmin.Access.SetAccess(ply, cmd, args)
    ...
    if not targets and (string.find(args[1], "^STEAM_[0-9]:[01]:[0-9]+$") or args[1] == "BOT" or (string.find(args[1], "STEAM_") and #args == 6)) then
        ...
        MySQLite.queryValue(string.format("SELECT groupname FROM FAdmin_PlayerGroup WHERE steamid = %s", MySQLite.SQLStr(target)), function(val) CAMI.SignalSteamIDUserGroupChanged(target, val or "user", groupname, "FAdmin") end)
...
hook.Add("PlayerInitialSpawn", "FAdmin_SetAccess", function(ply)
    MySQLite.queryValue("SELECT groupname FROM FAdmin_PlayerGroup WHERE steamid = " .. MySQLite.SQLStr(ply:SteamID()) .. ";", function(Group)
        ...
        if FAdmin.Access.Groups[Group] then ply:FAdmin_SetGlobal("FAdmin_admin", FAdmin.Access.Groups[Group].ADMIN_ACCESS) end
    end, function(err)
        ErrorNoHalt(err)
        MsgN()
    end)
...
local function toggleImmunity(ply, cmd, args)
    ...
    if not args[1] then
        FAdmin.Messages.SendMessage(ply, 5, "Invalid argument!")
        return false
    end
...
FAdmin.StartHooks["Access"] = function()
    --Run all functions that depend on other plugins
