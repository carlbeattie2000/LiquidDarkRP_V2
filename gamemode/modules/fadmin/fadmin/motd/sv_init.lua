...
hook.Add("InitPostEntity", "PlaceMOTD", function()
    ...
    ent:Spawn()
    ent:Activate()
    if file.Exists("FAdmin/CurMOTDPage.txt", "DATA") and file.Read("FAdmin/CurMOTDPage.txt", "DATA") ~= "" then game.ConsoleCommand("_FAdmin_MOTDPage \"" .. file.Read("FAdmin/CurMOTDPage.txt", "DATA") .. "\"\n") end
end)

function FAdmin.MOTD.SaveMOTD(ent, ply)
    local pos = ent:GetPos()
    local ang = ent:GetAngles()
    local map, x, y, z, pitch, yaw, roll = string.lower(game.GetMap()), pos.x, pos.y, pos.z, ang.p, ang.y, ang.r
    if MOTD then
        sql.Query([[UPDATE FADMIN_MOTD SET ]] .. "x = " .. MySQLite.SQLStr(x) .. ", " .. "y = " .. MySQLite.SQLStr(y) .. ", " .. "z = " .. MySQLite.SQLStr(z) .. ", " .. "pitch = " .. MySQLite.SQLStr(pitch) .. ", " .. "yaw = " .. MySQLite.SQLStr(yaw) .. ", " .. "roll = " .. MySQLite.SQLStr(roll) .. " WHERE map = " .. MySQLite.SQLStr(map) .. ";")
    else
        sql.Query([[INSERT INTO FADMIN_MOTD VALUES(]] .. MySQLite.SQLStr(map) .. ", " .. MySQLite.SQLStr(x) .. ", " .. MySQLite.SQLStr(y) .. ", " .. MySQLite.SQLStr(z) .. ", " .. MySQLite.SQLStr(pitch) .. ", " .. MySQLite.SQLStr(yaw) .. ", " .. MySQLite.SQLStr(roll) .. ");")
    end
...
function FAdmin.MOTD.SetMOTDPage(ply, cmd, args)
    ...
    if ply:EntIndex() ~= 0 and (not ply.IsSuperAdmin or not ply:IsSuperAdmin()) then
...
local function CreateMOTD(ply)
    if ply ~= game.GetWorld() and not ply:IsSuperAdmin() then
        FAdmin.Messages.SendMessage(ply, 5, "No access!")
        return false
    end
