...
local function Jail(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        ...
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Jail", target) then
        ...
        elseif JailType == "big" then
            -- Requires CSS but it's really funny
            jailDistance = 80
            table.insert(JailProps, {
                pos = Vector(0, 0, -5),
                ang = Angle(0, 0, 0),
        ...
        if not target:FAdmin_GetGlobal("fadmin_jailed") and JailType ~= "unjail" and string.lower(cmd) ~= "unjail" then
            ...
            timer.Create("FAdmin_jail_watch" .. target:UserID(), 1, 0, function()
                ...
                if target:GetPos():DistToSqr(target.FAdminJailPos) > jailDistance then target:SetPos(target.FAdminJailPos) end
            end)
...
hook.Add("PlayerSpawn", "FAdmin_jail", function(ply) if ply:FAdmin_GetGlobal("fadmin_jailed") then timer.Simple(0.1, function() if IsValid(ply) then ply:SetPos(ply.FAdminJailPos) end end) end end)
hook.Add("PlayerSpawnObject", "FAdmin_jailed", function(ply) if ply:FAdmin_GetGlobal("fadmin_jailed") then return false end end)
hook.Add("CanPlayerEnterVehicle", "FAdmin_jailed", function(ply) if ply:FAdmin_GetGlobal("fadmin_jailed") then return false end end)
--Kill stupid addons that does not call CanPlayerEnterVehicle (like Sit Anywhere script)
hook.Add("PlayerEnteredVehicle", "FAdmin_jailed", function(ply) if ply:FAdmin_GetGlobal("fadmin_jailed") then ply:ExitVehicle() end end)
