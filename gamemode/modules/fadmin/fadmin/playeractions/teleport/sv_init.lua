local function zapEffect(target)
    ...
    for i = 1, 100 do
        timer.Simple(1 / i, function() util.Effect("TeslaHitBoxes", effectdata, true, true) end)
    end
...
local function TPToPos(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "Teleport") then
...
local function Teleport(ply, cmd, args)
    ...
    if not targets or #targets == 1 and not IsValid(targets[1]) then targets = {ply} end
    for _, target in pairs(targets) do
        if IsValid(target) and target:Alive() then
            ...
            if trace.HitNormal ~= Vector(0, 0, 1) then offset = trace.HitNormal * 16 end
