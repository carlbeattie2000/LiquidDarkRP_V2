local function MuteChat(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if not FAdmin.Access.PlayerHasPrivilege(ply, "Chatmute", target) then
...
local function UnMuteChat(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        ...
        if IsValid(target) and target:FAdmin_GetGlobal("FAdmin_chatmuted") then target:FAdmin_SetGlobal("FAdmin_chatmuted", false) end
    end
...
hook.Add("PlayerSay", "FAdmin_Chatmute", function(ply, text, Team, dead) if ply:FAdmin_GetGlobal("FAdmin_chatmuted") then return "" end end)
