    ...
    for _, target in pairs(targets) do
        ...
        if IsValid(target) then
            ...
            elseif stage == "update" then
                ...
                game.ConsoleCommand(string.format("kickid %s %s\n", target:UserID(), "Kicked by " .. name .. " (" .. Reason .. ")"))
                ply.FAdminKickReason = nil
            end
        end
    end

    if stage ~= "start" and stage ~= "cancel" and stage ~= "update" then FAdmin.Messages.FireNotification("kick", ply, nil, {FAdmin.TargetsToString(targets), Reason}) end
...
timer.Create("FAdminCheckBans", 60, 0, function()
    for k, v in pairs(FAdmin.BANS) do
        if v.time and not isstring(v.time) and tonumber(v.time) < os.time() and v.time ~= 0 then FAdmin.BANS[k] = nil end
    end
end)
...
local function Ban(ply, cmd, args)
    ...
    if stage ~= "start" and stage ~= "cancel" and stage ~= "update" then FAdmin.Messages.FireNotification("ban", ply, nil, {FAdmin.TargetsToString(targets), time, Reason}) end
    return true, targets, stage, Reason
end
...
hook.Add("PlayerInitialSpawn", "FAdmin_Bans", function(ply)
    ...
    if allowedIn == false then ply:Kick(reason) end
end)
