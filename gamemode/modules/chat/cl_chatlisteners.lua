...
local currentConfig = {
    text = "", -- Default config is not loaded yet
    hearFunc = fn.Id
}
...
local function loadChatReceivers()
    ...
    DarkRP.addChatReceiver("", DarkRP.getPhrase("talk"), function(ply)
        ...
        return LocalPlayer():GetPos():DistToSqr(ply:GetPos()) < GAMEMODE.Config.talkDistance * GAMEMODE.Config.talkDistance
    end)
    ...
    DarkRP.addChatReceiver("/g", DarkRP.getPhrase("talk_to_your_group"), function(ply)
        for _, func in pairs(GAMEMODE.DarkRPGroupChats) do
            if func(LocalPlayer()) and func(ply) then return true end
        end
        return false
    end)
    ...
    DarkRP.addChatReceiver("/pm", "PM", function(ply, text)
        ...
        return string.find(string.lower(ply:Nick()), text[2], 1, true) ~= nil or string.find(string.lower(ply:SteamName()), text[2], 1, true) ~= nil or string.lower(ply:SteamID()) == text[2]
    end)
