...
function plyMeta:arrest(time, arrester)
    ...
    if GAMEMODE.Config.teletojail and DarkRP.jailPosCount() ~= 0 then self:Spawn() end
end
...
function DarkRP.arrestedPlayerCount()
    local count = 0
    for _ in DarkRP.iterateArrestedPlayers() do
        count = count + 1
    end
    return count
end
...
local function CombineRequest(ply, args)
    ...
    local DoSay = function(text)
        ...
        for _, v in ipairs(player.GetAll()) do
            if v:isCP() or v == ply then DarkRP.talkToPerson(v, col, phrase .. " " .. name, col2, text, ply) end
        end
    end
    return args, DoSay
end
...
local function warrantCommand(ply, args)
    ...
    if not RPExtraTeams[Team] or not RPExtraTeams[Team].mayor then -- No need to search through all the teams if the player is a mayor
        ...
        for k, v in pairs(RPExtraTeams) do
            if v.mayor then table.Add(mayors, team.GetPlayers(k)) end
        end
...
function finishWarrantRequest(choice, mayor, initiator, suspect, reason)
    ...
    if IsValid(suspect) then suspect:warrant(initiator, reason) end
end
...
function DarkRP.hooks:canArrest(arrester, arrestee)
    if IsValid(arrestee) and arrestee:IsPlayer() and arrestee:isCP() and not GAMEMODE.Config.cpcanarrestcp then return false, DarkRP.getPhrase("cant_arrest_other_cp") end
    if not GAMEMODE.Config.npcarrest and arrestee:IsNPC() then return false, DarkRP.getPhrase("unable", "arrest", "NPC") end
    if GAMEMODE.Config.needwantedforarrest and not arrestee:IsNPC() and not arrestee:getDarkRPVar("wanted") then return false, DarkRP.getPhrase("must_be_wanted_for_arrest") end
    if arrestee:IsPlayer() and arrestee.FAdmin_GetGlobal and arrestee:FAdmin_GetGlobal("fadmin_jailed") then return false, DarkRP.getPhrase("cant_arrest_fadmin_jailed") end
    local jpc = DarkRP.jailPosCount()
    if not jpc or jpc == 0 then return false, DarkRP.getPhrase("cant_arrest_no_jail_pos") end
    if arrestee.Babygod then return false, DarkRP.getPhrase("cant_arrest_spawning_players") end
    return true
end
...
function DarkRP.hooks:playerArrested(ply, time, arrester)
    ...
    if not job or not job.hasLicense then ply:setDarkRPVar("HasGunlicense", nil) end
    ply:StripWeapons()
    ply:StripAmmo()
    if ply:isArrested() then -- hasn't been arrested before
        return
    end
...
function DarkRP.hooks:playerUnArrested(ply, actor, teleportOverride)
    ...
    if ply.Sleeping then DarkRP.toggleSleep(ply, "force") end
    if not ply:Alive() and not GAMEMODE.Config.respawninjail then ply.NextSpawnTime = CurTime() end
