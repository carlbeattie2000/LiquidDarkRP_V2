...
local function DoLottery(ply, amount)
    ...
    for k, v in ipairs(player.GetAll()) do
        if v ~= ply then DarkRP.createQuestion(phrase, "lottery_" .. tostring(k), v, 30, EnterLottery, ply, v) end
    end
...
local function RequestLicense(ply)
    ...
    for teamNr, jobTable in pairs(RPExtraTeams) do
        if jobTable.mayor then table.insert(mayors, jobTable.name) end
        if jobTable.chief then table.insert(chiefs, jobTable.name) end
        if GAMEMODE.CivilProtection[teamNr] then table.insert(cops, jobTable.name) end
    end
...
local function rp_GiveLicense(ply, arg)
    ...
    if ply ~= target then DarkRP.notify(ply, 0, 4, DarkRP.getPhrase("gunlicense_granted", nick, target:Nick())) end
...
local function rp_RevokeLicense(ply, arg)
    ...
    if ply ~= target then DarkRP.notify(ply, 1, 4, DarkRP.getPhrase("gunlicense_denied", nick, target:Nick())) end
