...
function hasPlayerExceededWarnLimit(ply, warns)
  ...
  if warns >= warnLimit then ply:Kick("You exceeded your warn limit for attempted Mass RDM"); end
end
...
function savePlayerWarns(ply)
  ...
  if playerWarnsFound then
    ...
    if timeNow > lastWarnTime + (warnClearTimeInDays * dayInSeconds) then updatedWarns = 1; end
