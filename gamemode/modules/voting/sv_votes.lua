...
function Vote:handleNewVote(ply, choice)
    ...
    if voteCount >= player.GetCount() - excludeCount then self:handleEnd() end
end
...
function DarkRP.hooks:canStartVote(newvote)
    if player.GetCount() <= table.Count(newvote.exclude) then
        ...
        if ply and ply:IsPlayer() then DarkRP.notify(ply, 0, 4, DarkRP.getPhrase("vote_alone")) end
        return false, true
    end
    return true
end
...
function DarkRP.createVote(question, voteType, target, time, callback, excludeVoters, fail, extraInfo)
    ...
    if not canStart then
        if callSuccess then
            ...
            if ply:IsPlayer() then DarkRP.notify(ply, 1, 4, message or DarkRP.getPhrase("unable", DarkRP.getPhrase("create_vote_for_job"), "")) end
            newvote:fail()
        end
        return
    end
    ...
    if ply:IsPlayer() then DarkRP.notify(ply, 0, 4, DarkRP.getPhrase("vote_started")) end
...
local function CancelVote(ply)
    ...
    if result then
        ...
        if ply:EntIndex() == 0 then print(DarkRP.getPhrase("x_cancelled_vote", "Console")) end
    else
