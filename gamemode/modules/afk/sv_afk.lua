...
timer.Create("DarkRPKeyPressedCheck", 1, 0, function() KillAFKTimer() end)
...
hook.Add("playerGetSalary", "AFKGetSalary", function(ply, amount) if ply:getDarkRPVar("AFK") then return true, "", 0 end end)
...
local function unAFKPlayer(ply)
    if ply:getDarkRPVar("AFK") then SetAFK(ply) end
end
