...
function plyMeta:stopHitInfo()
    ...
    if table.IsEmpty(activeHitmen) then hook.Remove("PostPlayerDraw", "drawHitInfo") end
end
...
hook.Add("EntityRemoved", "hitmenu", function(ent) activeHitmen[ent] = nil end)
...
hook.Add("InitPostEntity", "HitmanMenu", function()
    for _, v in ipairs(player.GetAll()) do
        if IsValid(v) and v:isHitman() and v:hasHit() then v:drawHitInfo() end
    end
end)
...
Networking
---------------------------------------------------------------------------]]
net.Receive("onHitAccepted", function(len) hook.Call("onHitAccepted", DarkRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadEntity()) end)
net.Receive("onHitCompleted", function(len) hook.Call("onHitCompleted", DarkRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadEntity()) end)
net.Receive("onHitFailed", function(len) hook.Call("onHitFailed", DarkRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadString()) end)
