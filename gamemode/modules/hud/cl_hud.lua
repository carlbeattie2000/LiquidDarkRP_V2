...
local function DrawMainHud()
  ...
  draw.RoundedBox(10, 25, ScrH() - 150, 500, 130, Color(25, 25, 25, 200))

  if (not run) then
    -- DPanel is global to allow the job to be changed outside of this statement
    -- Need to either figure out a way to change it outside without 
    -- DPanel being global, or have a hook so Change model is being
    -- Called so much
    DPanel = vgui.Create("DModelPanel")
    DPanel:SetPos(40, ScrH() - 120)
    DPanel:SetSize(70, 70)
    DPanel:SetModel(LocalPlayer():GetModel())
    local eyepos = DPanel.Entity:GetBonePosition(DPanel.Entity:LookupBone("ValveBiped.Bip01_Head1"))

    eyepos:Add(Vector(0, 0, 2))	-- Move up slightly

    DPanel:SetLookAt(eyepos)

    DPanel:SetCamPos(eyepos-Vector(-12, 0, 0))	-- Move cam in front of eyes

    DPanel.Entity:SetEyeTarget(eyepos-Vector(-12, 0, 0))
    function DPanel:LayoutEntity( ent ) return end
  end

  -- Update player model when job changes etc
  DPanel:SetModel(LocalPlayer():GetModel())

  run = true

  surface.SetDrawColor( Color(255, 0, 0, 255) )
  surface.DrawOutlinedRect(200, ScrH() - 100, 200, 15)

  surface.SetDrawColor( Color(0, 0, 255, 255) )
  surface.DrawOutlinedRect(200, ScrH() - 60, 200, 15)
end
