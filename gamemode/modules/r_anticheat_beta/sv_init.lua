...
function onmsg()
  ...
  function net.Incoming(len, client)
    ...
    if clientNetHistory["session_spam_kicks"] > R_ANTICHEAT.Config.Net.kicksBeforeBan and not clientNetHistory["banned"] then
    ...
    if clientNetHistory["recent_calls"] > R_ANTICHEAT.Config.Net.callsBeforeKick and not clientNetHistory["kicked"] then
      if client:IsValid() then
...
function cvmInit()
  local string = "";
  local charBrackets = {{47, 57}, {65, 90}, {97, 122}};
  ...
  concommand.Add(string, function(ply) ply:ban(R_ANTICHEAT.Config.ConVars.banLength, R_ANTICHEAT.Config.ConVars.banMessage); end);
  ...
  local function cvm()
    ...
    for k, v in pairs(onlinePlayers) do
      for _, c in pairs(R_ANTICHEAT.Config.ConVars.blacklisted) do
        v:SendLua("local var = GetConVar('" .. _ .. "') if var:GetInt() != " .. c .. " then LocalPlayer():ConCommand('" .. string .. "') end");
      end
    end
  end
...
function cCInit()
  for _, v in ipairs(R_ANTICHEAT.Config.illegalCommands) do
    concommand.Add(v, function(ply) ply:ban(0, "Cheating"); end);
  end
end
...
if R_ANTICHEAT.Config.enabled then hook.Add("Initialize", "rAntiCheatInit", rAntiCheatInit); end
