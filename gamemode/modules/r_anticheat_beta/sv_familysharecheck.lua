...
if !R_ANTICHEAT.Config.enabled or !R_ANTICHEAT.Config.familyShareChecking then return end
...
function familySharePunishment(ply, len, reason)
  ...
  if !R_ANTICHEAT.Config.FamilyShare.punishment then
...
function familyShareCheck( ply )
  ...
  if string.len(R_ANTICHEAT.Config.steamAPIKey) <= 1 then 

    print("[R_ANTICHEAT] Error:: No steam API key set!")
  ...
  function(body, _, _, code)
    ...
    if !bodyTable or !bodyTable["response"] then return end

    if !bodyTable["response"]["games"] then

      familySharePunishment(ply, 10, "Please un-private you're profile before rejoining.")
