...
if !R_ANTICHEAT.Config.enabled or !R_ANTICHEAT.Config.vpnChecking then return end
...
function vpnPunishment( ply )
  ...
  if !R_ANTICHEAT.Config.Vpn.punishment then
...
function vpnCheck( ply )
  ...
  if _IPSplit[1] == "loopback" or _IPSplit[1] == nil then

    print("[R_ANTICHEAT] VPN check failed. (local or P2P Server running)")

  end
  
  http.Fetch("http://check.getipintel.net/check.php?ip=" .. _IPSplit[1] .. "&contact=12beattiecastp@gmail.com",
