...
local function RP_PlayerChat(ply, text)
  ...
  for k, v in pairs(ChatCommands) do
    if string.lower(v.cmd) == string.Explode(" ", string.lower(text))[1] then
      ...
      if callback == "" then return "", "", DoSayFunc; end
...
function GM:PlayerSay(ply, text, isTeamChat) -- We will make the old hooks run AFTER DarkRP's playersay has been run.
  ...
  for k, v in SortedPairs(otherhooks, false) do
    if type(v) == "function" then text2 = v(ply, text2) or text2; end
  end

  if game.IsDedicated() then ServerLog("\"" .. ply:Nick() .. "<" .. ply:UserID() .. ">" .. "<" .. ply:SteamID() .. ">" .. "<" .. team.GetName(ply:Team()) .. ">\" say \"" .. text .. "\"\n"); end
  if DoSayFunc then
    DoSayFunc(text2);
    return "";
  end
...
hook.Add("InitPostEntity", "DarkRP_ChatCommands", function()
  ...
  for a, b in pairs(otherhooks) do
    if type(b) ~= "function" then otherhooks[a] = nil; end
  end
end);
...
function TalkToRange(ply, PlayerName, Message, size)
  ...
  for k, v in pairs(ents) do
    if v:IsPlayer() then filter:AddPlayer(v); end
  end
...
function GM:TalkToPerson(receiver, col1, text1, col2, text2, sender)
  ...
  umsg.Short(col1.b);
  umsg.String(text1);
  if sender then umsg.Entity(sender); end
...
function ConCommand(ply, _, args)
  ...
  if tbl.delay and ply.DrpCommandDelays[cmd] and ply.DrpCommandDelays[cmd] > time - tbl.delay then return; end
