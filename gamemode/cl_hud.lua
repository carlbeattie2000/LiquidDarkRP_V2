...
usermessage.Hook("AdminTell", function(msg)
  ...
  timer.Simple(10, function() AdminTell = function() end; end);
end);
...
local function DrawHUD()
  if not LocalPlayer().DarkRPVars or LocalPlayer().InTut then return; end
  Scrw, Scrh = ScrW(), ScrH();
...
local function LDRP_PlayerInfo(ply)
  ...
  local longest = 0;
  for _, a in pairs(long) do
    local c = string.len(a);
    if c > longest then longest = c; end
  end
  ...
  if GAMEMODE.Config.showjob then draw.SimpleTextOutlined(Team, "Trebuchet22", x, y + 20, team.GetColor(ply:Team()), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255)); end
...
local function DrawEntityDisplay()
  ...
  if e and e.IsValid and e:IsValid() and e:GetPos():Distance(LocalPlayer():GetPos()) < 400 then
    if tr.Entity:IsPlayer() then LDRP_PlayerInfo(tr.Entity); end
    if tr.Entity:IsOwnable() then tr.Entity:DrawOwnableInfo(); end
  end
end
