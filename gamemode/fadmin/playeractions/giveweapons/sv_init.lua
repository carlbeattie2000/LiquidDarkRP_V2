local function GiveWeapon(ply, cmd, args)
  if not FAdmin.Access.PlayerHasPrivilege(ply, "giveweapon") then
  ...
  if table.HasValue(FAdmin.HL2Guns, args[2]) then
    weapon = args[2];
  elseif weapon and weapon.ClassName then
    weapon = weapon.ClassName;
  end

  if not weapon then return; end
  for _, target in pairs(targets) do
    if IsValid(target) then target:Give(weapon); end
  end
...
local function GiveAmmo(ply, cmd, args)
  ...
  for _, target in pairs(targets) do
    if IsValid(target) then target:GiveAmmo(amount, ammo); end
  end
