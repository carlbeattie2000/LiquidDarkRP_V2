...
usermessage.Hook("FAdmin_kick_update", function(um) KickText = um:ReadString(); end);
...
local function showBanWindow(SteamID, NICK, time, reason)
  ...
  local M, H, D, W, Y = BanTime % 60, math.floor(BanTime / 60) % 24, math.floor(BanTime / 1440) % 7, math.floor(BanTime / 10080) % 53, math.floor(BanTime / 525948);
  ...
  TextEntry.OnEnter = function()
    Window:Close();
    RunConsoleCommand("_FAdmin", "ban", SteamID, "execute", BanTime, TextEntry:GetValue());
  end;
  ...
  Years:SetValue(Y);
  Years:SetDecimals(0);
  local MinLabel, HourLabel, DayLabel, WeekLabel, YearLabel = vgui.Create("DLabel", TimePanel), vgui.Create("DLabel", TimePanel), vgui.Create("DLabel", TimePanel), vgui.Create("DLabel", TimePanel), vgui.Create("DLabel", TimePanel);
  ...
  function Minutes:OnValueChanged(val)
    if val == M then return; end
    M = val;
    update();
  end

  function Hours:OnValueChanged(val)
    if val == H then return; end
    H = val;
    update();
  end

  function Days:OnValueChanged(val)
    if val == D then return; end
    D = val;
    update();
  end

  function Weeks:OnValueChanged(val)
    if val == W then return; end
    W = val;
    update();
  end

  function Years:OnValueChanged(val)
    if val == Y then return; end
    Y = val;
    update();
  end
  ...
  ButtonCancel.DoClick = function()
    Window:Close();
    RunConsoleCommand("_FAdmin", "ban", SteamID, "cancel");
  end;
...
FAdmin.StartHooks["CL_KickBan"] = function()
  ...
  FAdmin.ScoreBoard.Player:AddActionButton("Kick", "FAdmin/icons/kick", nil, function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Kick", ply); end, function(ply)
    ...
    TextEntry.OnEnter = function()
      Window:Close();
      RunConsoleCommand("_FAdmin", "kick", SteamID, "execute", TextEntry:GetValue());
    end;
    ...
    ButtonCancel.DoClick = function()
      Window:Close();
      RunConsoleCommand("FAdmin", "kick", SteamID, "cancel");
    end;
  ...
  FAdmin.ScoreBoard.Server:AddPlayerAction("Unban", function() return "FAdmin/icons/Ban", "FAdmin/icons/disable"; end, Color(0, 155, 0, 255), true, function(button)
    ...
    local function RetrieveBans(len)
      ...
      for k, v in pairs(bans) do
        local Line = BanList:AddLine(k, v.name or "N/A", (tonumber(v.time or "") and FAdmin.PlayerActions.ConvertBanTime((tonumber(v.time) - os.time()) / 60)) or "N/A", v.reason or "", v.adminname or "", v.adminsteam or "");
        Line.name = v.name;
        Line.time = v.time;
