  ...
  for _, target in pairs(targets) do
    ...
    if IsValid(target) then
      ...
      elseif stage == "update" then
        ...
        game.ConsoleCommand(string.format("kickid %s %s\n", target:UserID(), "Kicked by " .. name .. " (" .. (Reason or "No reason provided") .. ")"));
        ply.FAdminKickReason = nil;
      end
    end
  end
...
hook.Add("PlayerAuthed", "FAdmin_LeavingBeforeBan", function(ply, SteamID, ...) if table.HasValue(StartBannedUsers, SteamID) then game.ConsoleCommand(string.format("kickid %s %s\n", ply:UserID(), "Getting banned")); end end);
-- Banning
...
timer.Create("FAdminCheckBans", 60, 0, function()
  for k, v in pairs(FAdmin.BANS) do
    if v.time and type(v.time) ~= "string" and tonumber(v.time) < os.time() and v.time ~= 0 then FAdmin.BANS[k] = nil; end
  end
end);
...
local function Ban(ply, cmd, args)
  ...
  for _, target in pairs(targets) do
    if (type(target) == "string" and not FAdmin.Access.PlayerHasPrivilege(ply, "Ban")) or not FAdmin.Access.PlayerHasPrivilege(ply, "Ban", target) then
      FAdmin.Messages.SendMessage(ply, 5, "No access!");
      return;
    end
    ...
    elseif stage == "cancel" then
      if type(target) ~= "string" and IsValid(target) then
        ...
        for k, v in pairs(StartBannedUsers) do
          if v == target:SteamID() then table.remove(StartBannedUsers, k); end
        end
      else -- If he left and you want to cancel
        for k, v in pairs(StartBannedUsers) do
          if v == args[1] then table.remove(StartBannedUsers, k); end
        end
      end
    elseif stage == "update" then
      -- Update reason text
