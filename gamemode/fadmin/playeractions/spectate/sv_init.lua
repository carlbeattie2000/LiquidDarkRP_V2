...
local function Spectate(ply, cmd, args)
  if not FAdmin.Access.PlayerHasPrivilege(ply, "Spectate") then
...
local function SpectateVisibility(ply, viewEnt)
  ...
  if IsValid(ply.FAdminSpectatingEnt) then AddOriginToPVS(ply.FAdminSpectatingEnt:GetShootPos()); end
  if ply.FAdminSpectatePos then AddOriginToPVS(ply.FAdminSpectatePos); end
end
...
OnPlayerSay = function(spectator, sender, message, isTeam)
  ...
  if spectator.FAdminSpectatePos and sender:GetShootPos():Distance(spectator.FAdminSpectatePos) <= 400 and sender:GetShootPos():Distance(spectator:GetShootPos()) > 250 then -- Make sure you don't get it twice
  ...
  if IsValid(spectator.FAdminSpectatingEnt) and sender:GetShootPos():Distance(spectator.FAdminSpectatingEnt:GetShootPos()) <= 300 and sender:GetShootPos():Distance(spectator:GetShootPos()) > 250 then GAMEMODE:TalkToPerson(spectator, team.GetColor(sender:Team()), sender:Nick(), Color(255, 255, 255, 255), message, sender); end
end;
