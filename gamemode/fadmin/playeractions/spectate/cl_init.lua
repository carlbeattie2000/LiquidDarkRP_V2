...
FAdmin.StartHooks["zzSpectate"] = function()
  ...
  -- Right click option
  FAdmin.ScoreBoard.Main.AddPlayerRightClick("Spectate", function(ply) RunConsoleCommand("_FAdmin", "Spectate", ply:SteamID()); end);
  -- Slap option in player menu
  FAdmin.ScoreBoard.Player:AddActionButton("Spectate", "FAdmin/icons/spectate", Color(0, 200, 0, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "Spectate") and ply ~= LocalPlayer(); end, function(ply) RunConsoleCommand("_FAdmin", "Spectate", ply:SteamID()); end);
end;
...
local function specCalcView(ply, origin, angles, fov)
  ...
  if IsValid(specEnt) then specEnt:SetNoDraw(not thirdperson); end
  return view;
end
...
local function specBinds(ply, bind, pressed)
  if bind == "+jump" then
    ...
    if keysDown["ATTACK2"] then
      ...
      RunConsoleCommand("FAdmin", "TPToPos", string.format("%d, %d, %d", pos.x, pos.y, pos.z), string.format("%d, %d, %d", roamVelocity.x, roamVelocity.y, roamVelocity.z));
    end
    return true;
...
stopSpectating = function()
  ...
  if IsValid(specEnt) then specEnt:SetNoDraw(false); end
