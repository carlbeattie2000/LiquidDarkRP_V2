local function SetNoclip(ply, cmd, args)
  if not FAdmin.Access.PlayerHasPrivilege(ply, "SetNoclip") then
  ...
  for _, target in pairs(targets) do
    if IsValid(target) then
      if Toggle then
        ...
        if not FAdmin.Access.PlayerHasPrivilege(target, "Noclip") then FAdmin.Messages.SendMessage(ply, 3, "This is not permanent! Make it permanent with a custom group in SetAccess!"); end
      else
        target:FAdmin_SetGlobal("FADmin_CanNoclip", false);
        target:FAdmin_SetGlobal("FADmin_DisableNoclip", true);
        if target:GetMoveType() == MOVETYPE_NOCLIP then target:SetMoveType(MOVETYPE_WALK); end
      end
    end
  end
...
hook.Add("PlayerNoClip", "FAdmin_noclip", function(ply)
  if not util.tobool(GetConVarNumber("sbox_noclip")) and ((FAdmin.Access.PlayerHasPrivilege(ply, "Noclip") and not ply:FAdmin_GetGlobal("FADmin_DisableNoclip")) or ply:FAdmin_GetGlobal("FADmin_CanNoclip")) then
