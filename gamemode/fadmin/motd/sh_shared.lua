...
if CLIENT then -- I can't be bothered to make a cl_init when there's a shared file with just one line in it.
  FAdmin.StartHooks["MOTD"] = function()
    FAdmin.ScoreBoard.Server:AddServerAction("Place MOTD", "FAdmin/icons/MOTD", Color(155, 0, 0, 255), true, function() RunConsoleCommand("_FAdmin", "CreateMOTD"); end);
    FAdmin.ScoreBoard.Server:AddServerSetting("Set MOTD page", "FAdmin/icons/MOTD", Color(0, 0, 155, 255), true, function()
      ...
      ButtonDefault.DoClick = function()
        Window:Close();
        RunConsoleCommand("_FAdmin", "motdpage", "data/FADmin/MOTD.txt");
      end;
