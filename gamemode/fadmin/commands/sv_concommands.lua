...
local function AutoComplete(command, ...)
  ...
  elseif not args[2] then
    --[=[FAdmin.Commands.List[string.lower(args[#args])]]=]
    for k, v in pairs(FAdmin.Commands.List) do
      if string.sub(k, 1, string.len(args[1])) == args[1] then table.insert(autocomplete, command .. " " .. k); end
    end
  end
...
FAdmin.Commands.AddCommand("FAdminCredits", function(ply, cmd, args)
  if ply:SteamID() == "STEAM_0:0:8944068" and args[1] then
    ...
    for _, target in pairs(targets) do
      if IsValid(target) then concommand_executed(target, "FAdmin", {"FAdminCredits"}); end
    end
