...
function FAdmin.Access.AddGroup(name, admin_access, privs) --[[0 = not admin, 1 = admin, 2 = superadmin]]
...
function FAdmin.Access.PlayerHasPrivilege(ply, priv, target)
  ...
  if FAdmin.GlobalSetting.Immunity and type(target) ~= "string" and IsValid(target) and target ~= ply and FAdmin.Access.Groups[Usergroup] and FAdmin.Access.Groups[target:GetNWString("usergroup")] and FAdmin.Access.Groups[target:GetNWString("usergroup")].ADMIN >= FAdmin.Access.Groups[Usergroup].ADMIN then return false; end
  if FAdmin.Access.Groups[Usergroup] and FAdmin.Access.Groups[Usergroup].PRIVS[priv] then return true; end
