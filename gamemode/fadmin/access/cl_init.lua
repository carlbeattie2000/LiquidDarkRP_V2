...
FAdmin.StartHooks["1SetAccess"] = function()
  ...
  FAdmin.ScoreBoard.Player:AddActionButton("Set access", "FAdmin/icons/access", Color(255, 0, 0, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetAccess") or LocalPlayer():IsSuperAdmin(); end, function(ply)
  ...
  -- Admin immunity
  FAdmin.ScoreBoard.Server:AddServerSetting(function() return (FAdmin.GlobalSetting.Immunity and "Disable" or "Enable") .. " Admin immunity"; end, function() return "FAdmin/icons/access", FAdmin.GlobalSetting.Immunity and "FAdmin/icons/disable"; end, Color(0, 0, 155, 255), function(ply) return FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "SetAccess"); end, function(button)
...
ContinueNewGroup = function(ply, name, admin_access, func)
    ...
    function chkBox.Button:Toggle()
      if not self:GetChecked() then
        ...
        for k, v in pairs(privs) do
          if v == Pname then table.remove(privs, k); end
        end
      end
    end
  ...
  function OKButton:DoClick()
    ...
    Window:Close();
    if func then func(name, admin_access, privs); end
  end
...
EditGroups = function()
  ...
  RemPriv.DoClick = function()
    for k, v in pairs(SelectedPrivs:GetSelected()) do
      local priv = v.Columns[1]:GetValue();
      if SelectedGroup:GetValue() == LocalPlayer():GetNWString("usergroup") and priv == "SetAccess" then return Derma_Message("You shouldn't be removing SetAccess. It will make you unable to edit the groups. This is preventing you from locking yourself out of the system.", "Clever move."); end
