...
local function RetrieveRestricted()
  ...
  for k, v in pairs(Query) do
    if Restricted[v.TYPE] then Restricted[v.TYPE][v.ENTITY] = v.ADMIN_GROUP; end
  end
end

RetrieveRestricted();
local function DoRestrictWeapons(ply, cmd, args)
  if not FAdmin.Access.PlayerHasPrivilege(ply, "Restrict") then
    print("no1");
  ...
  local Weapon = args[1];
  local Group = args[2];
  if not Group or not FAdmin.Access.Groups[Group] or not Weapon then
    print("no2");
    return;
  end
...
local function RestrictWeapons(ply, Weapon, WeaponTable)
  ...
  if not FAdmin or not FAdmin.Access or not FAdmin.Access.Groups or not FAdmin.Access.Groups[Group] or not FAdmin.Access.Groups[Restricted.Weapons[Weapon]] then return; end
