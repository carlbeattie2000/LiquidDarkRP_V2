FAdmin.StartHooks["Logging"] = function()
  ...
  FAdmin.Commands.AddCommand("Logging", function(ply, cmd, args)
    if not FAdmin.Access.PlayerHasPrivilege(ply, "Logging") then
...
function FAdmin.Log(text, preventServerLog)
  ...
  if not LogFile then -- The log file of this session, if it's not there then make it!
    if not file.IsDir("FAdmin_logs", "DATA") then file.CreateDir("FAdmin_logs"); end
...
hook.Add("PlayerSpawnProp", "FAdmin_Log", function(ply, class)
  ...
  for k, v in pairs(player.GetAll()) do
    if v:IsAdmin() then v:PrintMessage(HUD_PRINTCONSOLE, ply:Nick() .. " (" .. ply:SteamID() .. ") Spawned a " .. class); end
  end
...
hook.Add("PlayerDeath", "FAdmin_Log", function(ply, inflictor, Killer)
  local Nick, SteamID, KillerName, InflictorName = IsValid(ply) and ply:Nick() or "N/A", IsValid(ply) and ply:SteamID() or "N/A", IsValid(Killer) and (Killer:IsPlayer() and Killer:Nick() or Killer:GetClass()) or "N/A", IsValid(inflictor) and inflictor:GetClass() or "N/A";
