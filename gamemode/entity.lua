...
function meta:IsOwnable()
  ...
  if (class == "func_door" or class == "func_door_rotating" or class == "prop_door_rotating") or (GAMEMODE.Config.allowvehicleowning and self:IsVehicle()) then return true; end
  return false;
end
...
function meta:AllowedToOwn(ply)
  ...
  if self.DoorData.AllowedToOwn and string.find(self.DoorData.AllowedToOwn, ply:UserID()) then return true; end
  return false;
end
...
if CLIENT then
  function meta:DrawOwnableInfo()
    ...
    local ownerstr = "";
    if IsValid(self:GetDoorOwner()) and self:GetDoorOwner().Nick then ownerstr = self:GetDoorOwner():Nick() .. "\n"; end
    for k, v in pairs(player.GetAll()) do
      if self:OwnedBy(v) and v ~= self:GetDoorOwner() then ownerstr = ownerstr .. v:Nick() .. "\n"; end
    end
    ...
    if type(self.DoorData.AllowedToOwn) == "string" and self.DoorData.AllowedToOwn ~= "" and self.DoorData.AllowedToOwn ~= ";" then
      ...
      for a, b in pairs(string.Explode(";", self.DoorData.AllowedToOwn)) do
        if IsValid(Player(b)) then table.insert(names, Player(b):Nick()); end
      end
        ...
        if superAdmin then
          if blocked then
            ...
            if self.DoorData.GroupOwn then
              whiteText = true;
              st = self.DoorData.title .. "\n" .. LANGUAGE.keys_owned_by .. "\n" .. self.DoorData.GroupOwn;
              if not self:IsVehicle() then st = st .. "\n" .. LANGUAGE.keys_everyone; end
            else
              st = LANGUAGE.keys_unowned .. "\n" .. LANGUAGE.keys_disallow_ownership;
              if not self:IsVehicle() then st = st .. "\n" .. LANGUAGE.keys_cops; end
            end
          end
        else
          if blocked then
...
local function SetDoorGroupOwnable(ply, arg)
  ...
  if not RPExtraTeamDoors[arg] and arg ~= "" then
    Notify(ply, 1, 10, "Door group does not exist!");
    return "";
  end
...
local function OwnDoor(ply)
  ...
  if IsValid(trace.Entity) and trace.Entity:IsOwnable() and ply:GetPos():Distance(trace.Entity:GetPos()) < 200 then
    ...
    if trace.Entity:OwnedBy(ply) then
      ...
      if ply:GetTable().OwnedNumz == 0 then timer.Create(ply:SteamID() .. "propertytax", 270, 0, ply.DoPropertyTax, ply); end
...
function meta:Own(ply)
  ...
  if not self:IsOwned() and not self:OwnedBy(ply) then self.DoorData.Owner = ply; end
end
...
local function RemoveDoorOwner(ply, args)
  ...
  if IsValid(trace.Entity) and trace.Entity:IsOwnable() and ply:GetPos():Distance(trace.Entity:GetPos()) < 110 then
    ...
    if trace.Entity.DoorData.NonOwnable then Notify(ply, 1, 4, LANGUAGE.door_rem_owners_unownable); end
    if target then
      if trace.Entity:OwnedBy(ply) then
        if trace.Entity:AllowedToOwn(target) then trace.Entity:RemoveAllowed(target); end
        if trace.Entity:OwnedBy(target) then trace.Entity:RemoveOwner(target); end
      else
